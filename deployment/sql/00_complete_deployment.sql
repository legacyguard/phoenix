-- ========================================
-- LegacyGuard Error Logging System
-- Complete Deployment Script
-- ========================================
-- This script combines all necessary steps for deploying the error logging system
-- Run this in your Supabase SQL Editor

-- Step 1: Create the error_logs table and all related objects
-- ========================================
...
CREATE INDEX IF NOT EXISTS idx_error_logs_unresolved ON public.error_logs(created_at DESC) WHERE resolved_at IS NULL;
...
-- Create or replace the error statistics view
...
-- Create or replace the function to check for critical error threshold
...
-- Create or replace the function to log errors
...
BEGIN
    ...
    IF NOT table_exists THEN
        RAISE EXCEPTION 'Error: error_logs table was not created';
    END IF;
    ...
    IF function_count != 2 THEN
        RAISE EXCEPTION 'Error: Not all functions were created (found % of 2)', function_count;
    END IF;
    ...
    IF policy_count < 3 THEN
        RAISE EXCEPTION 'Error: Not all policies were created (found % of 3)', policy_count;
    END IF;
    
    RAISE NOTICE 'Success: Error logging system deployed successfully!';
    RAISE NOTICE '- Table created: error_logs';
    RAISE NOTICE '- Functions created: log_error, check_critical_error_threshold';
    RAISE NOTICE '- Policies created: % policies', policy_count;
    RAISE NOTICE '- View created: error_statistics';
END $$;

-- Step 3: Test the deployment
-- ========================================

-- Insert a test error to verify everything works
DO $$
DECLARE
    test_error_id UUID;
BEGIN
    ...
    SELECT public.log_error(
        'error',
        'Deployment test - this can be deleted',
        'Test stack trace',
        '{"test": true, "deployment": true}'::jsonb,
        '/deployment-test',
        'Mozilla/5.0 Test Agent',
        '{"browser": "test"}'::jsonb
    ) INTO test_error_id;
    
    IF test_error_id IS NULL THEN
        RAISE EXCEPTION 'Error: Failed to insert test error';
    END IF;
    
    RAISE NOTICE 'Test error created with ID: %', test_error_id;
    RAISE NOTICE 'You can delete this test error with: DELETE FROM error_logs WHERE id = ''%'';', test_error_id;
END $$;
...
SELECT 
    ...
    COUNT(DISTINCT error_level) as error_levels,
...
-- 
-- Next steps:
-- 1. Deploy the Edge Function for email alerts
-- 2. Test error logging from your application
-- 3. Monitor the error_logs table
-- ========================================
