describe("Heritage Vault Critical Path E2E Tests", () => {
  ...
  const assetStory =
    "This home has been in our family for three generations. My grandparents built it in 1952 with their own hands. Every room holds precious memories of family gatherings, celebrations, and the everyday moments that make a house a home.";
  ...
  describe("Golden Path Onboarding", () => {
    it("should complete full onboarding journey and verify dashboard updates", () => {
      ...
      cy.get("[data-cy=next-step]").should(
        "contain",
      ...
      cy.get("[data-cy=person-relationship]").select(
        trustedPerson.relationship,
      );
  ...
  describe("Asset Story Feature", () => {
    ...
    it("should add, edit, and display asset stories", () => {
      ...
      cy.get("[data-cy=story-prompt]").should(
        "contain",
      ...
      cy.get("[data-cy=char-counter]").should(
        "contain",
        `${assetStory.length}/2000`,
      );
      ...
      const additionalText =
        " It is my hope that this home continues to shelter and nurture our family for generations to come.";
      ...
      cy.get("[data-cy=story-text]").should(
        "contain",
        assetStory + additionalText,
      );
