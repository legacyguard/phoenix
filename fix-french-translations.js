...
function generateFixTemplate(analysis) {
  ...
  if (
    analysis.identicalTranslations &&
    analysis.identicalTranslations.length > 0
  ) {
...
function generateFixTemplates() {
  ...
  try {
      ...
      if (fs.existsSync(frPath)) {
        ...
        if (analysis.totalMissing > 0 || analysis.totalIdentical > 0) {
          ...
          const templatePath = path.join(
            __dirname,
            `fix-template-${filename.replace(".json", ".txt")}`,
          );
    ...
    console.log(
      `Templates generated: ${results.filter((r) => r.totalMissing > 0 || r.totalIdentical > 0).length}`,
    );
    ...
    const priorityFiles = results
      ...
      .sort(
        (a, b) =>
          b.totalMissing +
          b.totalIdentical -
          (a.totalMissing + a.totalIdentical),
      );

    priorityFiles.forEach((result) => {
      console.log(
        `  - ${result.filename}: ${result.totalMissing} missing, ${result.totalIdentical} identical`,
      );
    });
  } catch (error) {
