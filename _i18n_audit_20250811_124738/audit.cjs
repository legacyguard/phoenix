...
const locFiles = fs
  .readFileSync(locListFile, "utf8")
  .trim()
  .split("\n")
  .filter(Boolean);
const srcFiles = fs
  .readFileSync(srcListFile, "utf8")
  .trim()
  .split("\n")
  .filter(Boolean);
...
for (const f of locFiles) {
  ...
  try {
    ...
    for (const k of keys) {
      const val = k
        .split(".")
        .reduce((o, seg) => (o && o[seg] != null ? o[seg] : undefined), json);
  ...
  for (const loc of locales) {
    ...
    if (a !== b) {
      placeholderMismatch.push({
        key: fullKey,
        base: BASE,
        basePlaceholders: [...baseSet],
        locale: loc,
        localePlaceholders: set ? [...set] : [],
      });
    }
  }
}
...
// Outputs
fs.writeFileSync(
  path.join(outDir, "missing_by_locale.json"),
  JSON.stringify(missingByLocale, null, 2),
);
fs.writeFileSync(
  path.join(outDir, "unused_in_base.json"),
  JSON.stringify({ base: BASE, unused: unusedInBase.sort() }, null, 2),
);
fs.writeFileSync(
  path.join(outDir, "unknown_namespace_usages.json"),
  JSON.stringify(unknownNs, null, 2),
);
fs.writeFileSync(
  path.join(outDir, "invalid_json.json"),
  JSON.stringify(invalidJson, null, 2),
);
fs.writeFileSync(
  path.join(outDir, "placeholder_mismatch.json"),
  JSON.stringify(placeholderMismatch, null, 2),
);

// Summary
const sum = [];
sum.push(
  `Locales dir: ${process.cwd() + "/" + path.relative(process.cwd(), "${LOC}")}`,
); // filled by shell, not perfect
sum.push(`Base locale: ${BASE}`);
sum.push(`Locales found: ${locales.length}`);
sum.push(`Used keys detected: ${used.size}`);
for (const loc of locales)
  sum.push(`Missing in ${loc}: ${missingByLocale[loc].length}`);
