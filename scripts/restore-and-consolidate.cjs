...
const finalStructure = {
  ...
  'dashboard': ['dashboard', 'annual-review', 'progress', 'notifications', 'alerts', 'admin', 'calculator', 'recommendations', 'reminders'],
  'assets': ['assets', 'asset-management', 'documents', 'upload', 'vault'],
  'family': ['family', 'guardians', 'guardian', 'executor', 'beneficiary', 'invitations', 'crisis', 'playbook'],
...
function loadFromBackup(fileName) {
  const backupPath = path.join(__dirname, '../src/i18n/locales/en', `${fileName}.json.backup`);
  const originalPath = path.join(__dirname, '../src/i18n/locales/en', `${fileName}.json`);
  ...
  for (const [targetFile, sourceFiles] of Object.entries(finalStructure)) {
    ...
    for (const sourceFile of sourceFiles) {
      ...
      if (Object.keys(data).length > 0) {
        ...
        console.log(`  ✅ Loaded ${sourceFile}.json (${Object.keys(data).length} keys)`);
      } else {
    ...
    if (objectsToMerge.length > 0) {
      ...
      console.log(`  📄 Created ${targetFile}.json (${Object.keys(mergedData).length} keys)`);
...
function updateI18nConfig(consolidatedFiles) {
  ...
  const newNamespacesString = newNamespaces.map(ns => `'${ns}'`).join(',\n  ');
  ...
  console.log(`✅ Updated i18n.ts with ${newNamespaces.length} consolidated namespaces`);
...
function cleanupOldFiles() {
  ...
  for (const file of files) {
    if (file.endsWith('.json') && !file.includes('backup')) {
      ...
      if (!allSourceFiles.includes(fileName) && !Object.keys(finalStructure).includes(fileName)) {
