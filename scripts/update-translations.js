...
async function processNamespace(namespace, usedKeys, stats) {
  ...
  const currentKeys = new Set(
    extractKeys(currentTranslations).map((item) => item.key),
  );
  ...
  console.log(
    `  ✓ Updated ${languages.length} language files for ${namespace}`,
  );
}
...
async function main() {
  ...
  if (Object.keys(usedKeys).length === 0) {
    console.error(
      "No extracted keys found. Please run extract-used-translations.js first.",
    );
    process.exit(1);
  }
...
`;
  ...
  if (stats.totalMissing > 0) {
    ...
    console.log(
      "   You should send these files to translators for proper translation.",
    );
  }
}
