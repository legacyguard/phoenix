...
const KEY_MAPPINGS = {
  'ui': {
    'ui-common': ['actions', 'buttons', 'labels', 'status', 'validation', 'placeholders'],
    'ui-navigation': ['navigation', 'menu', 'breadcrumb', 'sidebar', 'header', 'footer'],
  ...
  'family': {
    'family-core': ['access', 'actions', 'beneficiaryCommunications', 'common', 'roles', 'relationships'],
    'family-emergency': ['emergency', 'crisis', 'crisisAssessment', 'crisisSituations', 'crisisPrevention', 'protocols'],
    'family-communication': ['*'] // Everything else
  },
  'dashboard': {
    'dashboard-main': ['dashboard', 'welcome', 'metadata', 'documentCard', 'guardianCard', 'firstTimeGuide', 'notifications', 'complexProfile', 'progress'],
    'dashboard-review': ['annualReview', 'review', 'status', 'planning', 'assessment']
  }
};
...
function updateFileContent(filePath, content) {
  ...
  updated = updated.replace(useTranslationPattern, (match, namespace) => {
    ...
    if (newNamespace && typeof newNamespace === 'string') {
      hasChanges = true;
      console.log(`  Updating useTranslation('${namespace}') → useTranslation('${newNamespace}')`);
  ...
  updated = updated.replace(tFunctionPattern, (match, namespace, key) => {
    ...
    if (newNamespace !== namespace) {
      hasChanges = true;
      console.log(`  Updating t('${namespace}:${key}') → t('${newNamespace}:${key}')`);
  ...
  updated = updated.replace(dollarTPattern, (match, namespace, key) => {
    ...
    if (newNamespace !== cleanNamespace) {
      hasChanges = true;
      console.log(`  Updating $t(${namespace}:${key}) → $t(${newNamespace}:${cleanKey})`);
  ...
  updated = updated.replace(i18nTPattern, (match, namespace, key) => {
    ...
    if (newNamespace !== namespace) {
      hasChanges = true;
      console.log(`  Updating i18n.t('${namespace}:${key}') → i18n.t('${newNamespace}:${key}')`);
  ...
  const namespaceArrayPattern = /(['"])(ui|family|dashboard|ai|assistant|upload)(['"])/g;
  updated = updated.replace(namespaceArrayPattern, (match, quote1, namespace, quote2) => {
    // Only replace in specific contexts (namespace arrays)
    const lineContext = updated.substring(Math.max(0, updated.indexOf(match) - 50), updated.indexOf(match) + 50);
    if (lineContext.includes('namespace') || lineContext.includes('ns:') || lineContext.includes('loadNamespaces')) {
      const newNamespace = NAMESPACE_MAPPING[namespace];
      if (newNamespace) {
        if (typeof newNamespace === 'string') {
          hasChanges = true;
          console.log(`  Updating namespace '${namespace}' → '${newNamespace}'`);
          ...
          console.log(`  Note: '${namespace}' splits into multiple namespaces: ${newNamespace.join(', ')}`);
        }
      }
    }
    return match;
...
function updateI18nConfig() {
  ...
  if (fs.existsSync(i18nPath)) {
    ...
    content = content.replace(namespaceArrayRegex, 
...
function main() {
  ...
  console.log('📌 Note: Some complex namespace splits may require manual review.');
}
