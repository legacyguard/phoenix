...
function addRetryToAsyncFunctions(content) {
  ...
  const asyncFunctionRegex =
    /const\s+(\w+)\s*=\s*async\s*\([^)]*\)\s*=>\s*{[\s\S]*?try\s*{([\s\S]*?)}[\s\S]*?catch/g;
...
function processFile(filePath, componentName) {
  try {
    ...
    if (hasRetryMechanism(content)) {
      console.log(
        `⏭️  Preskočené (už má retry): ${path.relative(process.cwd(), filePath)}`,
      );
      return false;
    }
    ...
    if (content === originalContent) {
      console.log(
        `⏭️  Žiadne zmeny: ${path.relative(process.cwd(), filePath)}`,
      );
      return false;
    }
    ...
    console.log(
      `✅ Pridaný retry mechanizmus: ${path.relative(process.cwd(), filePath)}`,
    );
    return true;
  } catch (error) {
