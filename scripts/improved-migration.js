...
const MIGRATION_CONFIG = {
  splits: {
    'ui.json': {
      'ui-common.json': [
        'common', 'general', 'global', 'shared', 'buttons', 
        'labels', 'messages', 'placeholders', 'tooltips'
      ],
      'ui-forms.json': [
        'forms', 'inputs', 'validation', 'fields', 'formLabels',
        'formErrors', 'formSuccess', 'formHelp'
      ],
      'ui-navigation.json': [
        'navigation', 'menu', 'breadcrumbs', 'tabs', 'links',
        'nav', 'header', 'footer', 'sidebar'
      ],
      'ui-professional.json': [
        'professional', 'advisor', 'lawyer', 'accountant', 'expert',
    ...
    'family.json': {
      'family-core.json': [
        'members', 'relationships', 'roles', 'basic', 'core',
        'family', 'relatives'
      ],
      'family-emergency.json': [
        'emergency', 'urgent', 'critical', 'contacts', 'crisis'
      ],
    ...
    'dashboard.json': {
      'dashboard-main.json': [
        'main', 'overview', 'summary', 'stats', 'activity',
        'dashboard', 'home', 'widgets', 'cards'
      ],
      'dashboard-review.json': [
        'review', 'annual', 'periodic', 'check', 'audit',
        'annualReview', 'reviewProcess'
      ]
    }
  },
  merges: {
...
function processLanguage(lang) {
  ...
  // Process splits
  for (const [sourceFile, targetFiles] of Object.entries(MIGRATION_CONFIG.splits)) {
    ...
    try {
      ...
      for (const [keyPath, value] of Object.entries(flatKeys)) {
        ...
        if (!assigned) {
          const catchAllFile = Object.keys(targetFiles).find(f => targetFiles[f].length === 0);
          if (catchAllFile) {
      ...
      for (const [targetFile, keys] of Object.entries(keyAssignments)) {
        if (Object.keys(keys).length > 0) {
          ...
          console.log(`    ✅ Created ${targetFile} with ${Object.keys(keys).length} keys`);
  ...
  // Process merges
  for (const [targetFile, sourceFiles] of Object.entries(MIGRATION_CONFIG.merges)) {
    const mergedContent = {};
    ...
    if (hasContent) {
      ...
      fs.writeFileSync(targetPath, JSON.stringify(nestedObject, null, 2));
      console.log(`    ✅ Created ${targetFile} with ${Object.keys(mergedContent).length} keys`);
  ...
  for (const lang of languages) {
    ...
    for (const file of files.sort()) {
      ...
      try {
        ...
        const status = lines < 100 ? '⚠️  (small)' : lines > 800 ? '❌ (too large)' : '✅';
        console.log(`  ${status} ${file.padEnd(30)} ${lines.toString().padStart(4)} lines, ${keys.toString().padStart(4)} keys`);
    ...
    console.log(`  📊 Total: ${totalKeys} keys in ${totalLines} lines across ${files.length} files`);
...
async function main() {
  ...
  if (isValid) {
    ...
    console.log('⚠️  Migration completed with issues. Please review the file sizes above.');
  }
