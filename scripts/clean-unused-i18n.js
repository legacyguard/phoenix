...
const languages = readdirSync(localesDir).filter(lang => statSync(join(localesDir, lang)).isDirectory() && !lang.startsWith('_'));
const namespaces = ['assets', 'auth', 'dashboard', 'documents', 'emails', 'errors', 'family', 'guardians', 'help', 'onboarding', 'settings', 'subscription', 'ui', 'wills'];
...
function findSourceFiles(dir, files = []) {
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      if (!entry.includes('node_modules') && !entry.startsWith('.') && !entry.includes('dist') && !entry.includes('build')) {
  ...
  sourceFiles.forEach(file => {
    ...
    dynamicPatterns.forEach(pattern => {
      if (pattern.test(content)) {
        console.log(`Warning: Found dynamic translation keys in ${file}. Manual review recommended.`);
      }
    });
  });
...
async function cleanupTranslations() {
  ...
  const backupDir = join(localesDir, '_backup_' + new Date().toISOString().replace(/:/g, '-'));
  ...
  for (const lang of languages) {
    ...
    for (const namespace of namespaces) {
      ...
      try {
        ...
        const cleanedTranslations = removeUnusedKeys(translations, usedKeysInNamespace);
        
        // Count keys
        const originalKeyCount = allKeys.length;
        const remainingKeyCount = getTranslationKeys(cleanedTranslations).length;
        ...
        // Write cleaned file
        writeFileSync(filePath, JSON.stringify(cleanedTranslations, null, 2) + '\n');
        
        console.log(`  ${namespace}: ${originalKeyCount} → ${remainingKeyCount} keys (removed ${removedKeyCount})`);
  ...
  console.log(`Reduction: ${((totalRemovedKeys / totalOriginalKeys) * 100).toFixed(1)}%`);
