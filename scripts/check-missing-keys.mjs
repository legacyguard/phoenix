...
const backupDir = path.join(__dirname, '..', 'src', 'i18n', 'locales.backup.20250810_182743', 'en');
...
function compareTranslations() {
  console.log(`${colors.bold}${colors.blue}=== Checking for Missing English Translation Keys ===${colors.reset}\n`);
  
  // Get all backup files
  const backupFiles = fs.readdirSync(backupDir).filter(file => file.endsWith('.json'));
  console.log(`${colors.cyan}Found ${backupFiles.length} backup files${colors.reset}`);
  
  // Get all current files
  const currentFiles = fs.readdirSync(currentDir).filter(file => file.endsWith('.json'));
  console.log(`${colors.cyan}Found ${currentFiles.length} current files${colors.reset}\n`);
  ...
  for (const [key, sourceFiles] of backupKeys.entries()) {
    if (!currentKeys.has(key)) {
      ...
      const moved = !sourceFiles.some(sf => currentFiles.some(cf => cf === sf));
      if (moved) {
  ...
  console.log(`Total unique keys in backup: ${colors.cyan}${backupKeys.size}${colors.reset}`);
  console.log(`Total unique keys in current: ${colors.cyan}${currentKeys.size}${colors.reset}`);
  console.log(`Total key occurrences in backup files: ${colors.cyan}${totalBackupKeys}${colors.reset}`);
  console.log(`Total key occurrences in current files: ${colors.cyan}${totalCurrentKeys}${colors.reset}`);
  
  if (missingKeys.length > 0) {
    console.log(`\n${colors.bold}${colors.red}⚠️  MISSING KEYS: ${missingKeys.length}${colors.reset}`);
    console.log(`${colors.red}The following keys exist in backup but are missing from current files:${colors.reset}\n`);
    ...
    for (const [file, keys] of Object.entries(missingByFile)) {
      console.log(`${colors.yellow}From ${file} (${keys.length} keys):${colors.reset}`);
    ...
    console.log(`${colors.yellow}Full report saved to: ${reportPath}${colors.reset}`);
  } else {
  ...
  if (foundInDifferentFile.length > 0) {
    ...
    if (foundInDifferentFile.length > 5) {
      console.log(`  ... and ${foundInDifferentFile.length - 5} more moved keys`);
    }
  }
  ...
  if (duplicatesAcrossFiles.length > 0) {
    ...
    if (duplicatesAcrossFiles.length > 5) {
      console.log(`  ... and ${duplicatesAcrossFiles.length - 5} more duplicates`);
    }
  }
  ...
  if (missingKeys.length === 0 && duplicatesAcrossFiles.length === 0) {
    console.log(`${colors.green}✅ Migration successful! No keys lost and no duplicates found.${colors.reset}`);
  } else if (missingKeys.length > 0) {
    console.log(`${colors.red}❌ Migration has issues: ${missingKeys.length} keys are missing.${colors.reset}`);
    console.log(`${colors.yellow}Action needed: Review the missing-keys-report.json file and restore missing keys.${colors.reset}`);
  } else if (duplicatesAcrossFiles.length > 0) {
    console.log(`${colors.yellow}⚠️  Migration complete but duplicates found across files.${colors.reset}`);
    console.log(`${colors.yellow}Action needed: Review and remove duplicate keys.${colors.reset}`);
  }
}
