...
async function validateTranslations() {
  console.log(
    `${colors.blue}🔍 Starting translation validation...${colors.reset}\n`,
  );
  ...
  for (const lang of languages) {
      ...
      if (data) {
        ...
        keys.forEach((key) => {
          if (currentKeys.has(key)) {
            console.error(
              `  ${colors.red}❌ Duplicate key found: "${key}" in both ${file} and ${currentKeys.get(key)}${colors.reset}`,
            );
            report.summary.duplicateKeys.push({
              lang,
              key,
              files: [file, currentKeys.get(key)],
            });
            hasErrors = true;
          }
    ...
    if (missingKeys.length > 0) {
      console.error(
        `  ${colors.red}❌ Missing ${missingKeys.length} keys:${colors.reset}`,
      );
      ...
      report.summary.missingKeys.push(
        ...missingKeys.map((k) => ({ lang, ...k })),
      );
      hasErrors = true;
    }
    ...
    if (newKeys.length > 0 && lang === "en") {
      console.warn(
        `  ${colors.yellow}⚠️  Found ${newKeys.length} new keys (unexpected in migration)${colors.reset}`,
      );
    ...
    console.log(
      `  ${colors.green}✅ Keys: ${backupKeys.size} → ${currentKeys.size}${colors.reset}`,
    );
    console.log(
      `  ${colors.green}✅ Files: ${backupFiles.length} → ${currentFiles.length}${colors.reset}`,
    );
    ...
    if (removedFiles.length > 0) {
      console.log(
        `  ${colors.yellow}📄 Removed files: ${removedFiles.join(", ")}${colors.reset}`,
      );
    ...
    if (addedFiles.length > 0) {
      console.log(
        `  ${colors.blue}📄 New files: ${addedFiles.join(", ")}${colors.reset}`,
      );
  ...
  console.log(
    `Total keys (English): ${report.summary.totalOriginalKeys} → ${report.summary.totalNewKeys}`,
  );
  ...
  for (const file of enFiles.sort()) {
    ...
    console.log(
      `  ${status} ${file.padEnd(30)} ${lines.toString().padStart(4)} lines, ${keys.toString().padStart(4)} keys${colors.reset}`,
    );
  }
  ...
  if (hasErrors) {
    ...
    console.error(
      `${colors.red}Please check the errors above and fix them manually.${colors.reset}\n`,
    );
    ...
    console.log(
      `${colors.green}The migration was successful.${colors.reset}\n`,
    );
  }
}
