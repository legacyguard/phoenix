...
function checkPrerequisites() {
  ...
  const backupDirs = fs.readdirSync(path.join(PROJECT_DIR, 'src/i18n'))
...
function showSummary() {
  ...
  log('   • ui.json → ui-common, ui-forms, ui-navigation, ui-professional, ui-misc', 'white');
  log('   • family.json → family-core, family-emergency, family-communication', 'white');
...
async function main() {
  ...
  if (!checkPrerequisites()) {
    log('\n❌ Prerequisites check failed. Please ensure all scripts are present.', 'red');
    process.exit(1);
  }
