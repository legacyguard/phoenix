...
async function migrateTranslations() {
  ...
  for (const lang of languages) {
    ...
    // Process splits
    for (const [sourceFile, splitConfigs] of Object.entries(
      MIGRATION_PLAN.splits,
    )) {
          ...
          for (const [targetFile, targetData] of Object.entries(splitResults)) {
            ...
            if (Object.keys(targetData).length > 0) {
              ...
              if (lang === "en") {
                ...
                keys.forEach((key) =>
                  newKeys.set(`${targetFile}:${key}`, true),
                );
              }

              console.log(
                `    ✅ Created ${targetFile} with ${Object.keys(targetData).length} top-level keys`,
              );
            }
          }
    ...
    for (const merge of MIGRATION_PLAN.merges) {
      ...
      if (sourceDataArray.every((data) => data !== null)) {
        ...
        console.log(
          `    ✅ Created ${merge.target} with ${Object.keys(mergedData).length} top-level keys`,
        );
