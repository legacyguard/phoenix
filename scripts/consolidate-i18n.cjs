...
const consolidationMap = {
  ...
  'dashboard': ['dashboard', 'annual-review', 'progress', 'notifications', 'alerts'],
  'assets': ['assets', 'asset-management', 'documents', 'upload', 'vault'],
  'family': ['family', 'guardians', 'guardian', 'executor', 'beneficiary', 'invitations'],
  ...
  for (const { sources } of consolidatedFiles) {
    for (const source of sources) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        console.log(`🗑️  Removed ${source}.json (backup: ${source}.json.backup)`);
