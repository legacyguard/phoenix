...
function getSplitCategories() {
  ...
  const enComponentsContent = JSON.parse(
    fs.readFileSync(enComponentsPath, "utf8"),
  );
...
async function main() {
  ...
  console.log(
    `📊 Using split pattern from English files (${group1Categories.size} categories for ui-components.json)\n`,
  );
