...
const backupDir = path.join(__dirname, '..', 'src', 'i18n', 'locales.backup.20250810_182743');
...
async function restoreMissingKeys() {
  console.log(`${colors.bold}${colors.blue}=== Restoring Missing Translation Keys ===${colors.reset}\n`);
  
  // Get list of languages
  const languages = fs.readdirSync(backupDir).filter(lang => 
    fs.statSync(path.join(backupDir, lang)).isDirectory()
  );
  ...
  for (const lang of languages) {
      ...
      if (backupData) {
        ...
        ['dashboard-main.json', 'dashboard-review.json'].forEach(file => {
          ...
          if (fs.existsSync(currentPath)) {
            ...
            if (currentData) {
              getAllKeys(currentData).forEach((val, key) => currentDashboardKeys.add(key));
            }
          }
        });
        ...
        backupKeys.forEach((value, key) => {
          if (!currentDashboardKeys.has(key)) {
            ...
            const targetFile = keyMappings['dashboard.json'][topLevel] || 'dashboard-main.json';
    ...
    if (fs.existsSync(familyBackupPath)) {
      ...
      if (backupData) {
        ...
        ['family-core.json', 'family-emergency.json', 'family-communication.json'].forEach(file => {
          const currentPath = path.join(currentLangDir, file);
          if (fs.existsSync(currentPath)) {
            const currentData = loadJsonFile(currentPath);
            if (currentData) {
              getAllKeys(currentData).forEach((val, key) => currentFamilyKeys.add(key));
            }
          }
        });
        ...
        backupKeys.forEach((value, key) => {
          if (!currentFamilyKeys.has(key)) {
            const topLevel = key.split('.')[0];
            const targetFile = keyMappings['family.json'][topLevel] || keyMappings['family.json']['_default'];
  ...
  console.log(`Total keys restored: ${colors.cyan}${stats.totalRestored}${colors.reset}\n`);
  ...
  console.log(`${colors.yellow}Next step: Run validation to ensure no duplicates were created.${colors.reset}`);
}
