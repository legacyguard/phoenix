...
async function reportUnusedTranslations() {
  ...
  for (const [fileName, keys] of Object.entries(unusedByFile)) {
    ...
    keys.slice(0, 20).forEach(key => { // Show first 20 keys
      console.log(`  - ${key}`);
    });
    if (keys.length > 20) {
  ...
  console.log(`   - Keys marked as "potentially unused" may still be used in ways not detected`);
  console.log(`   - Always manually review before removing any keys`);
  console.log(`   - Check for dynamic key usage, template literals, and other patterns`);
  console.log(`   - Consider keeping keys that might be used in future features`);
}
