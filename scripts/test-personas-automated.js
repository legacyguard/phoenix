...
const testPersonas = [
  ...
  },
  {
    ...
    password: 'TestPassword123!',
    name: 'Jordan Journey',
    tests: ['login', 'checkDashboardProgress', 'checkAssets', 'checkTrustedPeople']
  },
  {
    email: 'premium_user@test.com',
    password: 'TestPassword123!',
    name: 'Patricia Premium',
    tests: ['login', 'checkPreservationMode', 'checkTimeCapsules', 'checkPremiumFeatures']
  },
  {
...
const tests = {
  async login(page, persona) {
    ...
    try {
      await page.waitForSelector('a[href*="login"], button:has-text("Sign In")', { timeout: 5000 });
  ...
  async checkEmptyDashboard(page, persona) {
    ...
    try {
      ...
      const planStrength = await page.$eval('[data-testid="plan-strength"], [data-cy="plan-strength"], .plan-strength', 
        el => el.textContent);
      ...
      const hasOnboardingPrompt = await page.$('text=/get started|begin|start your journey/i');
  ...
  async startOnboarding(page, persona) {
    ...
    try {
      ...
      const startButton = await page.$('button:has-text("Get Started"), button:has-text("Begin"), button:has-text("Start")');
      if (startButton) {
  ...
  async checkAssets(page, persona) {
    ...
    try {
      ...
      await page.click('a[href*="vault"], nav :has-text("Vault"), nav :has-text("Assets")');
      await page.waitForTimeout(2000);
      
      // Check for asset cards
      const assetCount = await page.$$eval('[data-testid="asset-card"], .asset-card', els => els.length);
  ...
  async checkTrustedPeople(page, persona) {
    ...
    try {
      ...
      await page.click('a[href*="trusted"], nav :has-text("Trusted"), nav :has-text("Circle")');
      await page.waitForTimeout(2000);
      
      // Check for people cards
      const peopleCount = await page.$$eval('[data-testid="person-card"], .person-card, .trusted-person', els => els.length);
  ...
  async checkTimeCapsules(page, persona) {
    ...
    try {
      ...
      await page.click('a[href*="time-capsule"], nav :has-text("Time Capsule")');
      await page.waitForTimeout(2000);
      
      // Check for capsule list
      const capsuleCount = await page.$$eval('[data-testid="capsule-card"], .capsule-card', els => els.length);
  ...
  async checkPremiumFeatures(page, persona) {
    ...
    try {
      ...
      const premiumBadge = await page.$(':has-text("Premium"), :has-text("PRO"), .premium-badge');
      ...
      const videoButton = await page.$('button:has-text("Record Video"), button:has-text("Video Message")');
      if (videoButton) {
  ...
  async checkFreeLimitations(page, persona) {
    ...
    try {
      ...
      const premiumButton = await page.$('button:has-text("Record Video"), button:has-text("Premium Feature")');
      if (premiumButton) {
        await premiumButton.click();
        await page.waitForTimeout(1000);
        
        // Check for upgrade prompt
        const upgradePrompt = await page.$(':has-text("Upgrade"), :has-text("Premium"), :has-text("Subscribe")');
        if (upgradePrompt) {
          console.log(`    ✅ Free limitations confirmed (upgrade prompt shown)`);
          return true;
        }
      }
  ...
  async checkExecutorDashboard(page, persona) {
    ...
    try {
      ...
      console.log(`    ⚠️  Executor elements not found (may need different navigation)`);
      return true;
    } catch (error) {
  ...
  async checkAnnualReviewPrompt(page, persona) {
    ...
    try {
      ...
      const notification = await page.$('.notification:has-text("review"), .alert:has-text("review")');
      if (notification) {
