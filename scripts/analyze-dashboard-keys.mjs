...
const backupPath = path.join(__dirname, '..', 'src', 'i18n', 'locales.backup.20250810_182743', 'en', 'dashboard.json');
...
for (const file of currentFiles) {
  const filePath = path.join(__dirname, '..', 'src', 'i18n', 'locales', 'en', file);
...
if (missingKeys.length > 0) {
  console.log(`Missing keys from dashboard.json (${missingKeys.length} total):`);
