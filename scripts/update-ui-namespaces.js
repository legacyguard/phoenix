...
function getNamespaceForKey(key) {
  ...
  const componentsPath = path.join(
    PROJECT_ROOT,
    "src/i18n/locales/en/ui-components.json",
  );
...
function updateFileReferences(filePath) {
  ...
  if (hasHookUsage) {
    ...
    if (usedNamespaces.size === 1) {
      ...
      console.log(
        `  ⚠️  ${path.relative(PROJECT_ROOT, filePath)}: Uses keys from both ui-components and ui-elements`,
      );
...
async function main() {
  console.log(
    "🚀 Updating namespace references from ui-misc to ui-components/ui-elements...\n",
  );
  ...
  for (const file of files) {
    ...
    if (updates > 0) {
      console.log(
        `  ✅ ${path.relative(PROJECT_ROOT, file)}: ${updates} references updated`,
      );
