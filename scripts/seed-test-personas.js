...
if (!supabaseUrl || !supabaseServiceKey) {
  ...
  console.log(
    "Please ensure VITE_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are set in .env.local",
  );
  process.exit(1);
}
...
async function createTestData(userId, persona) {
  try {
      ...
      const assets = [
        {
          ...
          asset_story:
            persona.profile.plan_strength >= 75
              ? "This home has been our sanctuary for the past 15 years. Every room holds precious memories of our children growing up."
              : null,
        },
        {
          user_id: userId,
    ...
    if (persona.profile.plan_strength >= 25) {
      ...
      if (peopleError)
        console.error("Error creating trusted people:", peopleError);
    }
    ...
    if (persona.email === "premium_user@test.com") {
      const timeCapsules = [
        {
          ...
          message:
            "Dear children, if you are reading this, it means I am no longer with you...",
      ...
      if (capsuleError)
        console.error("Error creating time capsules:", capsuleError);
    }
      ...
      const lifeEvents = [
        {
          ...
          ).toISOString(), // 5 years ago
          description: "Birth of first child",
          actions_taken: [
            "Updated will",
            "Added child as beneficiary",
...
async function seedDatabase() {
  ...
  if (
    createdUsers["premium_user@test.com"] &&
    createdUsers["executor@test.com"]
  ) {
    try {
      const { error } = await supabase.from("executor_assignments").insert({
