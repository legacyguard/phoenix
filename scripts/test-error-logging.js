...
async function testErrorLogging() {
  ...
  try {
    const { data, error } = await supabase.rpc('log_error', {
      ...
      p_error_stack: 'Error: Test error\n    at testErrorLogging (test-error-logging.js:50:15)',
      p_error_context: { test: true, automated: true, timestamp: new Date().toISOString() },
    ...
    results.passed++;
    results.tests.push({ name: 'Log regular error', status: 'passed', errorId: data });
  } catch (error) {
    log(`❌ Test 1 failed: ${error.message}`, 'red');
    results.failed++;
    results.tests.push({ name: 'Log regular error', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    results.passed++;
    results.tests.push({ name: 'Log warning', status: 'passed', errorId: data });
  } catch (error) {
    log(`❌ Test 2 failed: ${error.message}`, 'red');
    results.failed++;
    results.tests.push({ name: 'Log warning', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    log(`✅ Test 3 passed: ${criticalErrorIds.length} critical errors logged`, 'green');
    results.passed++;
    results.tests.push({ name: 'Log critical errors', status: 'passed', errorIds: criticalErrorIds });
  } catch (error) {
    log(`❌ Test 3 failed: ${error.message}`, 'red');
    results.failed++;
    results.tests.push({ name: 'Log critical errors', status: 'failed', error: error.message });
  }

  // Test 4: Check critical error threshold
  log('\nTest 4: Checking critical error threshold...', 'blue');
  try {
    const { data, error } = await supabase.rpc('check_critical_error_threshold');
    ...
    results.passed++;
    results.tests.push({ name: 'Check threshold', status: 'passed', data: result });
  } catch (error) {
    log(`❌ Test 4 failed: ${error.message}`, 'red');
    results.failed++;
    results.tests.push({ name: 'Check threshold', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    results.passed++;
    results.tests.push({ name: 'Query error logs', status: 'passed', count: data.length });
  } catch (error) {
    log(`❌ Test 5 failed: ${error.message}`, 'red');
    results.failed++;
    results.tests.push({ name: 'Query error logs', status: 'failed', error: error.message });
  }
  ...
  try {
    ...
    if (data.length > 0) {
      ...
      data.forEach(stat => {
        log(`   - ${stat.hour}: ${stat.error_level} (${stat.error_count} errors)`, 'cyan');
      });
    }
    results.passed++;
    results.tests.push({ name: 'Check error statistics', status: 'passed', stats: data.length });
  } catch (error) {
    log(`❌ Test 6 failed: ${error.message}`, 'red');
    results.failed++;
    results.tests.push({ name: 'Check error statistics', status: 'failed', error: error.message });
  }
...
testErrorLogging()
  .then(results => {
    if (results.failed === 0) {
      log('\n✨ All tests passed! Error logging system is working correctly.', 'green');
      process.exit(0);
    } else {
