  ...
  for (const [namespace, keys] of Object.entries(reorganizationMap)) {
    ...
    if (Object.keys(extractedData).length > 0) {
      ...
      if (!fs.existsSync(filePath)) {
        ...
        console.log(
          `✅ Created ${namespace}.json (${Object.keys(extractedData).length} keys)`,
        );
...
function updateI18nConfig(newNamespaces) {
  ...
  const namespaceMatch = i18nContent.match(
  ...
  if (namespaceMatch) {
    ...
    // Add new namespaces
    const allNamespaces = [
      ...new Set([...existingNamespaces, ...newNamespaces]),
    ].sort();

    const newNamespacesString = allNamespaces
      .map((ns) => `'${ns}'`)
      .join(",\n  ");
    ...
    console.log(
      `✅ Updated i18n.ts with ${newNamespaces.length} new namespaces`,
    );
  }
}
    ...
    if (componentNamespace) {
      ...
      if (useTranslationRegex.test(content)) {
        content = content.replace(
          useTranslationRegex,
          `useTranslation('${componentNamespace}')`,
        );
        updated = true;
      }
      ...
      if (updated) {
        ...
        console.log(
          `✅ Updated ${file} to use ${componentNamespace} namespace`,
        );
...
function cleanupCommonJson() {
  ...
  const backupPath = path.join(
    __dirname,
    "../src/i18n/locales/en/common.json.backup",
  );
  ...
  console.log(
    `✅ Cleaned common.json (removed ${Object.keys(commonData).length - Object.keys(cleanedData).length} keys)`,
  );
...
function main() {
  ...
  try {
    ...
    console.log(
      `   - Removed ${Object.keys(commonData).length - remainingKeys} keys from common.json`,
    );
