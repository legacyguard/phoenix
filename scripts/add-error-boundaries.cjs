...
function processFile(filePath, componentName, isAsync = false) {
  try {
    ...
    if (hasErrorBoundary(content)) {
      console.log(
        `⏭️  Preskočené (už má Error Boundary): ${path.relative(process.cwd(), filePath)}`,
      );
      return false;
    }
    ...
    console.log(
      `✅ Pridaná ${isAsync ? "AsyncErrorBoundary" : "ErrorBoundary"}: ${path.relative(process.cwd(), filePath)}`,
    );
    return true;
  } catch (error) {
