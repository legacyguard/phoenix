...
function extractTranslationUsages(content, filePath) {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      } else if (pattern.source.includes('useTranslation') || pattern.source.includes('withTranslation')) {
...
function analyzeNamespaceUsage() {
  console.log(`${colors.bold}${colors.blue}=== Verifying All Namespace References ===${colors.reset}\n`);
  
  const { keyToNamespace, namespaceToKeys } = loadTranslationKeys();
  const codeFiles = findAllCodeFiles(srcDir);
  
  console.log(`${colors.cyan}Loaded ${keyToNamespace.size} unique translation keys from ${namespaceMap.size} namespaces${colors.reset}`);
  console.log(`${colors.cyan}Found ${codeFiles.length} code files to analyze${colors.reset}\n`);
  ...
  // Generate report
  console.log(`${colors.bold}${colors.yellow}=== ANALYSIS RESULTS ===${colors.reset}\n`);
  
  // Summary statistics
  console.log(`${colors.bold}Summary:${colors.reset}`);
  console.log(`  Total unique keys: ${colors.cyan}${keyToNamespace.size}${colors.reset}`);
  console.log(`  Keys with correct references: ${colors.green}${keyToNamespace.size - issues.unreferencedKeys.size}${colors.reset}`);
  console.log(`  Unreferenced keys: ${colors.yellow}${issues.unreferencedKeys.size}${colors.reset}`);
  console.log(`  Wrong namespace references: ${colors.red}${issues.wrongNamespace.length}${colors.reset}`);
  console.log(`  Non-existent key references: ${colors.red}${issues.keyNotFound.length}${colors.reset}`);
  console.log(`  Duplicate keys across files: ${colors.yellow}${issues.duplicateKeys.length}${colors.reset}`);
  console.log(`  Invalid namespace references: ${colors.red}${issues.namespaceNotFound.length}${colors.reset}\n`);
  
  // Wrong namespace issues
  if (issues.wrongNamespace.length > 0) {
    console.log(`${colors.bold}${colors.red}❌ Wrong Namespace References (${issues.wrongNamespace.length}):${colors.reset}`);
    issues.wrongNamespace.slice(0, 10).forEach(issue => {
      console.log(`  ${issue.file}:${issue.line}`);
      console.log(`    Key: ${colors.yellow}${issue.key}${colors.reset}`);
      console.log(`    Used: ${colors.red}${issue.usedNamespace}:${issue.key}${colors.reset}`);
      console.log(`    Should be: ${colors.green}${issue.correctNamespaces[0]}:${issue.key}${colors.reset}\n`);
    });
  ...
  if (issues.keyNotFound.length > 0) {
    console.log(`${colors.bold}${colors.red}❌ Non-existent Key References (${issues.keyNotFound.length}):${colors.reset}`);
    issues.keyNotFound.slice(0, 10).forEach(issue => {
      console.log(`  ${issue.file}:${issue.line}`);
      console.log(`    ${colors.red}${issue.namespace}:${issue.key}${colors.reset} - Key does not exist\n`);
    });
  ...
  if (issues.namespaceNotFound.length > 0) {
    console.log(`${colors.bold}${colors.red}❌ Invalid Namespace References (${issues.namespaceNotFound.length}):${colors.reset}`);
    issues.namespaceNotFound.slice(0, 10).forEach(issue => {
      console.log(`  ${issue.file}:${issue.line}`);
      console.log(`    ${colors.red}${issue.namespace}${colors.reset} - Namespace does not exist\n`);
    });
  ...
  if (issues.duplicateKeys.length > 0) {
    console.log(`${colors.bold}${colors.yellow}⚠️  Duplicate Keys Across Namespaces (${issues.duplicateKeys.length}):${colors.reset}`);
  ...
  if (issues.unreferencedKeys.size > 0) {
    console.log(`${colors.bold}${colors.yellow}⚠️  Potentially Unreferenced Keys (${issues.unreferencedKeys.size}):${colors.reset}`);
    console.log(`${colors.dim}  Note: These might be used dynamically or in external systems${colors.reset}\n`);
  ...
  if (issues.wrongNamespace.length === 0 && issues.keyNotFound.length === 0 && issues.namespaceNotFound.length === 0) {
    console.log(`${colors.green}✅ All namespace references are correct!${colors.reset}`);
    console.log(`${colors.green}   ${keyToNamespace.size - issues.unreferencedKeys.size} keys are properly referenced.${colors.reset}`);
  } else {
    console.log(`${colors.red}❌ Found ${issues.wrongNamespace.length + issues.keyNotFound.length + issues.namespaceNotFound.length} namespace reference issues that need fixing.${colors.reset}`);
  }
