...
function extractStringsFromFile(filePath) {
  ...
  while ((match = jsxTextRegex.exec(content)) !== null) {
    ...
    if (text && text.length > 2 && !IGNORE_PATTERNS.some(pattern => pattern.test(text))) {
  ...
  while ((match = jsxAttrRegex.exec(content)) !== null) {
    ...
    if (text && !IGNORE_PATTERNS.some(pattern => pattern.test(text))) {
      const key = generateTranslationKey(text, `${fileName}.${attr}`, namespace);
      extracted.set(key, text);
    }
  }
  ...
  for (const [namespace, strings] of namespaceGroups) {
    ...
    if (fs.existsSync(namespaceFile)) {
      try {
        ...
        console.warn(`Warning: Could not parse ${namespaceFile}:`, error.message);
      }
    }
...
function main() {
  ...
  for (const file of files) {
    try {
      ...
      if (extracted.size > 0) {
        console.log(`📄 ${path.relative(SRC_DIR, file)}: ${extracted.size} strings`);
      }
    } catch (error) {
