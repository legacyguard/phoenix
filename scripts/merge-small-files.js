      ...
      if (stat.isDirectory()) {
        ...
        for (const [oldNs, newNs] of Object.entries(namespaceMap)) {
          ...
          const hookPattern = new RegExp(
            `useTranslation\\(['"]${oldNs}['"]\\)`,
            "g",
          );
          if (hookPattern.test(content)) {
            content = content.replace(
              hookPattern,
              `useTranslation('${newNs}')`,
            );
            modified = true;
            totalReplacements++;
          }
...
function validateMerge() {
  ...
  files.sort().forEach((file) => {
    ...
    try {
      ...
      console.log(
        `  ${file.padEnd(30)} ${lines.toString().padStart(5)} ${keys.toString().padStart(7)}   ${status}`,
      );
    } catch (e) {
  ...
  console.log("  " + "─".repeat(60));
  console.log(
    `  Total: ${files.length} files              ${totalLines.toString().padStart(5)} ${totalKeys.toString().padStart(7)}\n`,
  );
...
async function main() {
  ...
  for (const lang of languages) {
    ...
    if (result.errors && result.errors.length > 0) {
      ...
      console.log(
        `    ✅ Merged ${result.merged.length} files, deleted ${result.deleted.length} files`,
      );
    }
  }
