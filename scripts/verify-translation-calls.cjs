  ...
  for (const call of calls) {
    if (call.type === 'useTranslation') {
      ...
      if (newNamespace && newNamespace !== oldNamespace) {
        ...
        updated = true;
        issues.push(`Updated useTranslation: '${oldNamespace}' → '${newNamespace}'`);
...
function verifyKeyExists(key, namespace) {
  const translationPath = path.join(__dirname, '../src/i18n/locales/en', `${namespace}.json`);
  
  if (!fs.existsSync(translationPath)) {
    return { exists: false, error: `Translation file not found: ${namespace}.json` };
  }
  ...
  try {
    ...
    for (const part of keyParts) {
      if (current && typeof current === 'object' && part in current) {
        ...
        return { exists: false, error: `Key not found: ${key} in ${namespace}.json` };
      }
    }
    
    return { exists: true, value: current };
  } catch (error) {
    return { exists: false, error: `Error reading translation file: ${error.message}` };
  }
}
