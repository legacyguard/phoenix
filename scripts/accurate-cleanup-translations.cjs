...
// Function to search for key usage in files with better pattern matching
function searchForKeyUsage(key, files) {
  ...
  for (const file of files) {
    try {
      ...
      // More precise patterns for key usage
      const patterns = [
        // Exact key matches
        `t('${key}')`,
        `t("${key}")`,
      ...
      // Also check for dynamic key usage patterns
      const dynamicPatterns = [
        // Template literals with key
        new RegExp(`t\\(\\s*[\`'"]\\s*\\$\\{[^}]*\\}\\s*[\`'"]\\s*\\)`, "g"),
        // Key in variables
...
// Main function
async function accurateCleanupTranslations() {
  ...
  const localeDirs = glob
    .sync(path.join(localesDir, "*/"))
  ...
  for (const locale of localeDirs) {
    ...
    for (const [fileName, keysToRemove] of Object.entries(unusedByFile)) {
      ...
      try {
        ...
        console.error(
          `  ❌ Error processing ${fileName}.json in ${locale}:`,
          error.message,
        );
      }
    }
