...
function generateImprovedCatch(fileName, functionName, indentLevel = 2) {
  ...
  const fileKey = Object.keys(fileContextMap).find(key => fileName.includes(key));
  const fileContext = fileContextMap[fileKey] || { context: 'Aplikácia', operations: {} };
...
function improveCatchBlocks(filePath) {
  ...
  const catchRegex = /} catch \((error|e|err)\s*(:?\s*any)?\)\s*{\s*([^}]*(?:toast\.error\([^)]+\)|console\.(error|log)\([^)]+\))?\s*[^}]*)?}/g;
  
  let improved = content;
  let changesMade = false;
  
  // Nájdeme názvy funkcií pre kontext
  const functionMatches = content.matchAll(/(?:const|function)\s+(\w+)\s*=?\s*(?:async)?\s*(?:\([^)]*\)|function\s*\([^)]*\))\s*(?:=>|{)/g);
  ...
  while ((match = catchRegex.exec(content)) !== null) {
    ...
    const hasDetailedLogging = catchContent.includes('console.error') && 
    ...
    if (!hasDetailedLogging || !hasToastError) {
      ...
      const improvedCatch = generateImprovedCatch(fileName, functionName, baseIndent);
      
      // Nahradíme starý catch blok novým
      improved = improved.replace(catchBlock, improvedCatch + '\n' + ' '.repeat(baseIndent * 2) + '}');
      changesMade = true;
    }
  }
...
async function main() {
  ...
  for (const file of files) {
    ...
    if (changesMade) {
      // Vytvoríme backup
      const backupPath = path.join(config.backupDir, path.relative(config.srcPath, file));
