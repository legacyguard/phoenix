...
export class ESignatureService {
  ...
  private skribbleApiBase = process.env.SKRIBBLE_API_BASE || 'https://api.skribble.com';
  ...
  async verifySimpleSignature(willId: string, signature: string): Promise<boolean> {
    try {
      const { data, error } = await supabase
        .from('generated_wills')
        .update({
  ...
  async requestSkribbleSignature(willId: string, documentBase64: string): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return { error: error.message || 'Failed to create signature request', status: response.status };
      }
