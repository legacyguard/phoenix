...
const languages = [
  'bg', 'cs', 'cy', 'da', 'de', 'el', 'es', 'et', 'fi',
  'fr', 'ga', 'hr', 'hu', 'is', 'it', 'lt', 'lv', 'me', 'mk',
  'mt', 'nl', 'no', 'pl', 'pt', 'ro', 'ru', 'sk', 'sl', 'sq',
  'sr', 'sv', 'tr', 'uk'
];
const modules = [
  'common','auth','subscription','dashboard','documents','assets','family','guardians','wills','settings','onboarding','help','ui','errors','emails'
];

// Helper: Merge translations, giving precedence to existing
function mergeTranslations(english, existing) {
  if (typeof english !== 'object' || typeof existing !== 'object') return english;
  let merged = { ...english };
...
languages.forEach(lang => {
  ...
  modules.forEach(module => {
    ...
    if (fs.existsSync(destFile)) {
      ...
      try {
        const allCommon = JSON.parse(fs.readFileSync(path.join(langDir, 'common.json'), 'utf8'));
        // Only get keys that are in the English file
        existingTranslations = {};
        for (const k of Object.keys(englishTranslations)) {
          if (allCommon[k] !== undefined) existingTranslations[k] = allCommon[k];
        }
      } catch {}
    }
    // Merge, giving precedence to existing translations
    const mergedTranslations = mergeTranslations(englishTranslations, existingTranslations);
    fs.writeFileSync(destFile, JSON.stringify(mergedTranslations, null, 2), 'utf8');
