...
function validateTerminology() {
  ...
  const translationFile = path.join(process.cwd(), 'src/i18n/locales/en/common.json');
  ...
  if (allIssues.length === 0) {
    console.log('✅ All terminology has been successfully transformed to human-friendly language!');
    ...
    if (groupedIssues.technical_term_found) {
      ...
      groupedIssues.technical_term_found.forEach(issue => {
        console.log(`  - "${issue.term}" found ${issue.count} times in ${issue.file}`);
      });
      console.log('');
    }
