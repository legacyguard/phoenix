...
export function ExecutorSelector({
  ...
  const [hasAlternative, setHasAlternative] = useState(!!executor?.alternativeExecutor);
  ...
  const handleAlternativeUpdate = (updates: Partial<Executor['alternativeExecutor']>) => {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('will.executor.title')}
          </CardTitle>
          <CardDescription>
            {t('will.executor.description')}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {/* Primary Executor */}
          <div className="space-y-4">
            <h4 className="font-medium">{t('will.executor.primary')}</h4>
            
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="executor-name">
                  {t('will.executor.name')} *
                </Label>
                <Input
                  id="executor-name"
                  value={executor?.name || ''}
                  onChange={(e) => handleExecutorUpdate({ name: e.target.value })}
                  placeholder={t('will.executor.namePlaceholder')}
                />
                {errors.executor && (
              ...
              <div className="space-y-2">
                <Label htmlFor="executor-relationship">
                  {t('will.executor.relationship')} *
                </Label>
                <Input
                  id="executor-relationship"
                  value={executor?.relationship || ''}
                  onChange={(e) => handleExecutorUpdate({ relationship: e.target.value })}
                  placeholder={t('will.executor.relationshipPlaceholder')}
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="executor-address">
                {t('will.executor.address')} *
              </Label>
              <Textarea
                id="executor-address"
                value={executor?.address || ''}
                onChange={(e) => handleExecutorUpdate({ address: e.target.value })}
                placeholder={t('will.executor.addressPlaceholder')}
                rows={3}
              />
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="executor-phone">
                {t('will.executor.phone')}
              </Label>
              <Input
                id="executor-phone"
                type="tel"
                value={executor?.phone || ''}
                onChange={(e) => handleExecutorUpdate({ phone: e.target.value })}
                placeholder={t('will.executor.phonePlaceholder')}
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between py-4 border-t">
            <div className="space-y-0.5">
              <Label htmlFor="has-alternative">
                {t('will.executor.addAlternative')}
              </Label>
              <p className="text-sm text-muted-foreground">
                {t('will.executor.alternativeDescription')}
              </p>
            </div>
            <Switch
              id="has-alternative"
            ...
            <div className="space-y-4 pt-4 border-t">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleAlternativeUpdate({ name: e.target.value })}
                    placeholder={t('will.executor.namePlaceholder')}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleAlternativeUpdate({ relationship: e.target.value })}
                    placeholder={t('will.executor.relationshipPlaceholder')}
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => handleAlternativeUpdate({ address: e.target.value })}
                  placeholder={t('will.executor.addressPlaceholder')}
                  rows={3}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => handleAlternativeUpdate({ phone: e.target.value })}
                  placeholder={t('will.executor.phonePlaceholder')}
                />
              </div>
            </div>
      ...
      <Card>
        <CardContent className="pt-6">
          <h4 className="font-medium mb-2">{t('will.executor.responsibilities')}</h4>
          <ul className="text-sm text-muted-foreground space-y-1">
            <li>• {t('will.executor.responsibility1')}</li>
            <li>• {t('will.executor.responsibility2')}</li>
            <li>• {t('will.executor.responsibility3')}</li>
            <li>• {t('will.executor.responsibility4')}</li>
          </ul>
        </CardContent>
      </Card>
    </div>
