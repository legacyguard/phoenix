...
class ContentAuditor {
  ...
  scanFile(filePath, baseDir) {
    try {
      ...
      if (relativePath.includes('.test.') || relativePath.includes('.spec.') || relativePath.includes('.stories.')) {
        return;
      }
  ...
  checkToneIssues(text, file, line) {
    // Too casual
    const casualTerms = ['hey', 'cool', 'awesome', 'stuff', 'things', 'basically', 'kinda', 'sorta', 'yeah'];
    ...
    // Too alarmist
    const alarmistTerms = ['urgent', 'critical', 'emergency', 'immediately', 'now!', 'asap'];
  ...
  checkClarityIssues(text, file, line) {
    // Technical jargon
    const jargonTerms = ['API', 'JSON', 'URL', 'HTTP', 'SQL', 'CSS', 'HTML', 'OAuth', 'JWT', 'UUID'];
    jargonTerms.forEach(term => {
      if (text.includes(term) && !text.includes('error') && !text.includes('Error')) {
        this.addIssue({
          type: 'UNCLEAR_JARGON',
          file,
          line,
  ...
  suggestProfessionalAlternative(text, casualTerm) {
    ...
    return text.replace(new RegExp(casualTerm, 'gi'), alternatives[casualTerm] || casualTerm);
  }
  ...
  suggestCalmerAlternative(text, alarmistTerm) {
    ...
    return text.replace(new RegExp(alarmistTerm, 'gi'), alternatives[alarmistTerm] || alarmistTerm);
  }
