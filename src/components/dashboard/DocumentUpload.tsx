...
export const DocumentUpload: React.FC<DocumentUploadProps> = ({
  ...
  const validateDocumentName = useDebouncedCallback((name: string) => {
    if (!name.trim()) {
      setNameError(t("dashboard-main:validation.nameRequired"));
    } else if (name.trim().length < 2) {
      setNameError(t("dashboard-main:validation.nameMinLength"));
    } else if (name.trim().length > 255) {
      setNameError(t("dashboard-main:validation.nameMaxLength", { max: 255 }));
    } else {
      setNameError("");
    }
  }, 300);
  ...
  const documentTypes = [
    {
      key: "realEstateDeed",
      value: t("dashboard-main:types.deed"),
      suggestContact: true,
    },
    {
      key: "birthCertificate",
      value: t("dashboard-main:types.birthCertificate"),
    },
    {
      key: "insurancePolicy",
      value: t("dashboard-main:types.policy"),
      suggestContact: true,
      suggestRenewal: true,
    ...
    {
      key: "lastWill",
      value: t("dashboard-main:types.will"),
      suggestContact: true,
    },
    {
      key: "employmentContract",
      value: t("dashboard-main:types.contract"),
      suggestContact: true,
    },
    {
      key: "passport",
      value: t("dashboard-main:types.passport"),
      suggestRenewal: true,
    },
    {
      key: "drivingLicense",
      value: t("dashboard-main:types.license"),
      suggestRenewal: true,
    },
    {
      key: "bankStatement",
      value: t("dashboard-main:types.bankStatement"),
      suggestContact: true,
      suggestSubscription: true,
    },
    {
      key: "investmentCertificate",
      value: t("dashboard-main:types.certificate"),
      suggestContact: true,
    },
    {
      key: "energyContract",
    ...
    { key: "other", value: t("dashboard-main:types.other") },
  ];
  ...
  const handleFileSelect = useCallback(
    (selectedFile: File) => {
      ...
      if (selectedFile.size > maxFileSize) {
        ...
        toast.error(
          t("dashboard-main:errors.fileTooLarge", { size: maxSizeMB }),
        );
        return;
      }
      ...
      if (
        storageUsage &&
        !canAddFile(storageUsage.used, selectedFile.size, plan)
      ) {
        toast.error(
          t("dashboard-main:errors.storageLimitReached", {
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      setShowForm(true);
      toast.success(t("dashboard-main:ui.uploaded"));

      return data.path;
    } catch (error: Record<string, unknown>) {
      console.error("Upload error:", error);
      toast.error(t("dashboard-main:errors.uploadFailed"));
      return null;
    } finally {
      setIsUploading(false);
      setUploadProgress(0);
  ...
  const handleSaveDocument = async () => {
    if (!uploadedFilePath || !documentName.trim()) {
      toast.error(t("dashboard-main:validation.nameRequired"));
      return;
    }
    ...
    try {
      ...
      if (!user) {
        toast.error(t("dashboard-main:errors.accessDenied"));
        return;
      }
        ...
        <div className="space-y-4">
          ...
          <div>
            <Label htmlFor="expirationDate">
              {t("dashboard-main:details.expirationDate")}
            </Label>
            <Popover>
      ...
      {!file ? (
        ...
        >
          ...
          <p className="text-xs text-gray-400">
            {t("upload.maxSize", {
              size: MAX_FILE_SIZES[plan] / (1024 * 1024),
            })}
          </p>
          <input
            id="fileInput"
