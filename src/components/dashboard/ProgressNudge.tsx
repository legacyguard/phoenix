...
export const ProgressNudge: React.FC<ProgressNudgeProps> = ({
  ...
  const showWelcomeBack =
    lastVisit &&
    new Date().getTime() - lastVisit.getTime() > 24 * 60 * 60 * 1000;
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex-1">
            <h3 className="text-lg font-semibold mb-2">
              {showWelcomeBack
                ? t("dashboard-main:dashboard.nudge.welcomeBack")
                : t("dashboard-main:dashboard.nudge.progressTitle", {
            ...
            <p className="text-sm text-muted-foreground">
              {remainingTasks === 1
                ? t("dashboard-main:dashboard.nudge.oneTaskLeft")
                : t("dashboard-main:dashboard.nudge.tasksRemaining", {
                    count: remainingTasks,
                  })}{" "}
              {showWelcomeBack
                ? t("dashboard-main:dashboard.nudge.continueSecuring")
                : t("dashboard-main:dashboard.nudge.finishToday")}
            </p>
          </div>
