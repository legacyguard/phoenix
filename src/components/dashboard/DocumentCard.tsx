...
export const DocumentCard: React.FC<DocumentCardProps> = ({
  ...
  const getExpirationStatus = () => {
    ...
    if (daysUntilExpiration < 0) {
      return {
        status: "expired",
        text: t("dashboard-main:dashboard.expired"),
        variant: "destructive" as const,
      };
    } else if (daysUntilExpiration <= 30) {
      return {
        status: "expiring",
        text: t("dashboard-main:dashboard.expiresSoon"),
        ...
        <div className="space-y-3">
          <div className="flex items-start justify-between">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  {document.importance_level && (
                    <Badge
                      variant={getImportanceBadgeColor(
                        document.importance_level,
                      )}
                      className="text-xs"
                    >
                      {t(
                        `dashboard.metadata.importance.${document.importance_level}`,
                      )}
                    </Badge>
                  )}
                  {expirationStatus && (
            ...
            <div className="flex items-center space-x-1">
              <span className="text-sm">
                {getCountryFlag(document.countryCode)}
              </span>
              <Button
                variant="ghost"
                size="sm"
          ...
          {hasMetadata && (
            <div className="pt-2 border-t space-y-2">
                ...
                <div className="text-xs space-y-1">
                  ...
                  {document.contact_phone && (
                    <div className="flex items-center gap-2">
                      ...
                      <a
                        href={`tel:${document.contact_phone}`}
                ...
                <div className="text-xs space-y-1">
                  {document.renewal_date && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-muted-foreground">
                        {t("dashboard.metadata.renewsOn")}:
                      </span>
                      <span className="font-medium">
                        {format(new Date(document.renewal_date), "PP")}
                      </span>
                    </div>
                  )}
                  {document.renewal_action && (
              ...
              {document.subscription_type &&
                document.subscription_type !== "none" && (
                  <div className="text-xs space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      {document.renewal_cost && (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-muted-foreground">
                            /
                            {document.subscription_type === "monthly"
                              ? t(
                                  "dashboard-main:subscription.types.monthly",
                                ).toLowerCase()
                              : t(
                                  "dashboard-main:subscription.types.yearly",
                                ).toLowerCase()}
                          </span>
                        </div>
                      )}
                      {document.auto_renewal !== null && (
                        <Badge
                          variant={
                            document.auto_renewal ? "default" : "outline"
                          }
                          className="text-xs"
                        >
                          <RotateCw className="h-3 w-3 mr-1" />
                          {document.auto_renewal
                            ? t("dashboard-main:subscription.autoRenewal")
                            : t("dashboard-main:subscription.manual")}
                        </Badge>
                      )}
                    </div>
                    {document.cancellation_notice_period &&
                      document.cancellation_notice_period > 0 && (
          ...
          {onToggleKeyDocument && (
            <div className="flex items-center justify-between py-2 border-t">
              <Label
                htmlFor={`key-doc-${document.id}`}
                className="text-xs text-muted-foreground"
              >
                {t("dashboard.documentCard.include_in_survivor_s_manual_1")}
              </Label>
              <Switch
                id={`key-doc-${document.id}`}
                checked={document.is_key_document || false}
                onCheckedChange={(checked) =>
                  onToggleKeyDocument(document.id, checked)
                }
              />
            </div>
          )}
