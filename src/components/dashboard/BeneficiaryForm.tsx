...
const createBeneficiaryFormSchema = (t: Record<string, unknown>) =>
  z.object({
    name: z
      .string()
      .min(2, { message: t("dashboard-main:validation.errors.nameMinLength") })
      .max(100, {
        message: t("dashboard-main:validation.errors.nameMaxLength", {
          max: 100,
        }),
      })
      .trim(),
    allocation: z
      .string()
      .min(1, {
        message: t("dashboard-main:validation.errors.allocationRequired"),
      })
      .refine((val) => !isNaN(Number(val)), {
        message: t("dashboard-main:validation.errors.allocationMustBeNumber"),
      })
      .refine(
        (val) => {
        ...
        {
          message: t("dashboard-main:validation.errors.allocationRange"),
        },
      ),
  });
...
export const BeneficiaryForm: React.FC<BeneficiaryFormProps> = ({
  ...
  const onFormSubmit = async (data: BeneficiaryFormData) => {
    try {
      ...
      toast.error(t("dashboard-main:dashboard.errors.failedToSaveBeneficiary"));
    }
  };
      ...
      <div className="flex gap-3 pt-4">
        <Button type="submit" disabled={isSubmitting} className="flex-1">
          {isSubmitting
            ...
            : isEditing
              ? t("dashboard-main:dashboard.updateBeneficiary")
              : t("dashboard-main:dashboard.saveBeneficiary")}
        </Button>
        <Button
          type="button"
          variant="outline"
