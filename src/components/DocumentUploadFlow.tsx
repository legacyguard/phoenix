...
type UploadStep = 'modeSelection' | 'upload' | 'analyzing' | 'classified' | 'extracting' | 'linking' | 'complete';
...
const DocumentUploadFlow: React.FC<DocumentUploadFlowProps> = ({
  ...
  const getStepIndex = (step: UploadStep) => steps.findIndex((s) => s.id === step);
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    if (!file.type.startsWith('image/') && file.type !== 'application/pdf') {
      setError(t("documentUploader.please_upload_an_image_or_pdf__1"));
      return;
    }
    ...
    if (processingMode === 'local') {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : 'Failed to process document locally');
    ...
    try {
      if (processingMode === 'hybrid') {
        ...
        const metadata = await extractDocumentMetadata(classificationResult, base64);
        setExtractedData(metadata);
        setCurrentStep('extracting');
  ...
  const findRelatedItems = async (classification: ClassificationResult, metadata: any): Promise<any> => {
    ...
    if (classification.category === 'property_deed' || classification.category === 'vehicle_title') {
      const relatedPossession = existingPossessions.find((p) => p.type === 'home' || p.type === 'vehicle');
    ...
    // Find related people
    if (classification.category === 'insurance_policy' || classification.category === 'will_or_trust') {
      const spouse = trustedPeople.find((p) => p.relationship.toLowerCase() === 'spouse' || p.relationship.toLowerCase() === 'wife');
      if (spouse) {
  ...
  return (
    <div className="p-4 md:p-6 font-sans max-w-screen-xl mx-auto">
      <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-2">{t("documentUploader.smart_document_upload_2")}</h2>
      <p className="text-sm md:text-base text-gray-600 mb-4 md:mb-6">{t("documentUploadFlow.upload_any_document_and_watch__3")}
      ...
      <div className="mb-6 md:mb-8 overflow-x-auto">
        <div className="flex items-center justify-between min-w-fit px-2">
          ...
          <div key={step.id} className="flex items-center">
              <div className={`flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-full flex-shrink-0 ${
            ...
            <div className={`h-1 w-12 md:w-16 mx-1 md:mx-2 ${
        ...
        <div className="flex justify-between mt-2 min-w-fit px-2">
          {steps.map((step, index) =>
          <div key={step.id} className="text-xs text-gray-600 text-center w-16 md:w-20">
      ...
      <div className="mb-4">
          <h3 className="text-base md:text-lg font-semibold text-gray-800 mb-3 md:mb-4">{t("documentUploadFlow.select_processing_mode_4")}</h3>
          <div className="space-y-3 md:space-y-4">
            ...
            'border-gray-300 hover:border-gray-400'}`
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-sm md:text-base text-gray-900">{t("settings.privacySettings.hybrid_mode_recommended_5")}</h4>
                  <p className="text-xs md:text-sm text-gray-600 mt-1">{t("documentUploadFlow.combines_local_and_cloud_ai_fo_6")}
            ...
            'border-gray-300 hover:border-gray-400'}`
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h4 className="font-semibold text-sm md:text-base text-gray-900">{t("settings.privacySettings.local_only_privacy_mode_7")}</h4>
                  <p className="text-xs md:text-sm text-gray-600 mt-1">{t("documentUploadFlow.processes_your_document_entire_8")}
          ...
          <div className="mt-4 flex justify-between items-center">
            <p className="text-xs text-gray-500">{t("documentUploadFlow.your_default_is_set_to_9")}
            <strong>{defaultProcessingMode === 'hybrid' ? 'Hybrid Mode' : 'Local-Only Mode'}</strong>
            </p>
            <button
      ...
      <div
        ...
        onDrop={handleDrop}>
          ...
          className="cursor-pointer">
            ...
            <p className="text-sm md:text-base text-gray-700 font-medium mb-1">{t("documentUploader.drop_your_document_here_or_cli_7")}

          </p>
            <p className="text-xs md:text-sm text-gray-500">{t("documentUploader.supports_images_and_pdfs_up_to_8")}
      ...
      {/* Processing Status */}
      {currentStep !== 'upload' && currentStep !== 'complete' && currentStep !== 'modeSelection' &&
          ...
          <div className="flex items-start mb-4">
            ...
            <div className="flex-1">
              <h3 className="text-base md:text-lg font-semibold text-gray-800">
                ...
                {currentStep === 'classified' && 'Identifying document type...'}
                {currentStep === 'extracting' && 'Extracting key information...'}
                {currentStep === 'linking' && 'Finding related items...'}
              </h3>
              <p className="text-xs md:text-sm text-gray-600 mt-1">
                {currentStep === 'analyzing' && 'Our AI is examining the document structure and content'}
                {currentStep === 'classified' && `Document identified as: ${classification?.category.replace(/_/g, ' ')}`}
                {currentStep === 'extracting' && 'Reading important details from your document'}
                {currentStep === 'linking' && 'Connecting to your existing possessions and people'}
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border p-4 md:p-6">
          <div className="flex items-start mb-4">
            ...
            <div className="flex-1">
              <h3 className="text-base md:text-lg font-semibold text-gray-800">{t("documentUploadFlow.document_processing_complete_16")}</h3>
              <p className="text-xs md:text-sm text-gray-600 mt-1">{t("documentUploadFlow.your_document_has_been_analyze_17")}</p>
            </div>
          </div>
          ...
          <div className="space-y-4 mt-6">
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <h4 className="font-semibold text-gray-800 mb-2">{t("documentUploadFlow.document_information_18")}</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-600">{t("documentUploadFlow.type_19")}</span>
                  <span className="font-medium capitalize">{classification.category.replace(/_/g, ' ')}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">{t("documentUploadFlow.suggested_title_20")}</span>
                  <span className="font-medium">{classification.suggestedTitle}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">{t("ai.documentUploadExample.confidence_7")}</span>
          ...
          <div className="p-4 bg-blue-50 rounded-lg">
                <h4 className="font-semibold text-blue-900 mb-2">{t("documentUploadFlow.extracted_information_22")}</h4>
                <div className="space-y-1 text-sm text-blue-800">
                  {Object.entries(extractedData.specificMetadata).slice(0, 3).map(([key, value]) =>
            ...
            {/* Suggested Links */}
            {suggestedLinks && (suggestedLinks.relatedPossessions?.length > 0 || suggestedLinks.relatedPeople?.length > 0) &&
          ...
          <div className="mt-4 md:mt-6 flex flex-col sm:flex-row gap-3">
            <button className="flex-1 px-4 py-2.5 bg-blue-600 text-white text-sm md:text-base font-semibold rounded-md hover:bg-blue-700 transition-colors">{t("documentUploadFlow.save_document_26")}

          </button>
            <button
            onClick={resetFlow}
            className="px-4 py-2.5 border border-gray-300 text-gray-700 text-sm md:text-base font-semibold rounded-md hover:bg-gray-50 transition-colors">{t("documentUploadFlow.upload_another_27")}
      ...
      <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
          ...
          <button
          onClick={resetFlow}
          className="mt-2 text-sm text-red-600 underline hover:text-red-800">{t("documentUploadFlow.try_again_28")}
