...
const LogLifeEvent: React.FC<LogLifeEventProps> = ({ open, onClose, onEventLogged }) => {
  ...
  const lifeEvents = [
    { id: 'married_divorced', label: t('logLifeEvent.eventTypes.married_divorced') },
    { id: 'new_child', label: t('logLifeEvent.eventTypes.new_child') },
    { id: 'bought_sold_home', label: t('logLifeEvent.eventTypes.bought_sold_home') },
    { id: 'started_business', label: t('logLifeEvent.eventTypes.started_business') },
    { id: 'trusted_person_passed', label: t('logLifeEvent.eventTypes.trusted_person_passed') },
    { id: 'job_change', label: t('logLifeEvent.eventTypes.job_change') },
    { id: 'moved_location', label: t('logLifeEvent.eventTypes.moved_location') },
    ...
    { id: 'child_graduated', label: t('logLifeEvent.eventTypes.child_graduated') },
    { id: 'career_advancement', label: t('logLifeEvent.eventTypes.career_advancement') },
    { id: 'financial_windfall', label: t('logLifeEvent.eventTypes.financial_windfall') },
    { id: 'debt_change', label: t('logLifeEvent.eventTypes.debt_change') },
    { id: 'insurance_change', label: t('logLifeEvent.eventTypes.insurance_change') }
  ];
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>{t('logLifeEvent.form.description')}</DialogDescription>
        </DialogHeader>
        <div className="py-4">
          <div className="mb-4">
          ...
          <ul className="space-y-2 max-h-60 overflow-y-auto">
            {lifeEvents.map((event) => (
              <li key={event.id}
                ...
                <div className={`p-4 rounded-lg border-2 transition-all duration-150 ease-in-out
