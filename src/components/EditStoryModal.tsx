...
export const EditStoryModal: React.FC<EditStoryModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!user) {
        toast.error(t("common:editStoryModal.errors.notAuthenticated"));
        return;
      }
      ...
      toast.success(t("common:editStoryModal.messages.saved"));
      onStorySaved(story.trim());
      onClose();
    } catch (error) {
      console.error("Error saving story:", error);
      toast.error(t("common:editStoryModal.errors.saveFailed"));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {currentStory
              ? t("common:editStoryModal.editStory")
              : t("common:editStoryModal.addStory")}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4 pt-4">
          <div className="space-y-2">
            <Label htmlFor="story">
              {t("editStoryModal.fields.yourStory")}
            </Label>
            <Textarea
              id="story"
              value={story}
            ...
            <div className="flex justify-between text-sm text-muted-foreground">
              ...
              <span
                className={
                  characterCount > maxCharacters * 0.9 ? "text-orange-600" : ""
                }
              >
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            >
              {isSaving ? (
                ...
                t("common:editStoryModal.saveStory")
              )}
            </Button>
          </div>
        </div>
