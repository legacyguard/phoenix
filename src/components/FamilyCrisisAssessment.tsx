...
export const FamilyCrisisAssessment: React.FC<FamilyCrisisAssessmentProps> = ({
  ...
  const capabilities = useMemo<FamilyCapability[]>(() => {
    const hasBankAccess = userAssets.some(
      (a) =>
    ...
    const hasLegalContacts = userGuardians.some(
      (g) => g.roles?.includes("lawyer") || g.roles?.includes("attorney"),
    );

    const hasDigitalAccess = userDocuments.some(
      (d) =>
        d.type?.toLowerCase().includes("password") ||
        d.type?.toLowerCase().includes("digital"),
    );

    const hasMedicalDocs = userDocuments.some(
      (d) =>
    ...
    const hasWillOrTrust = userDocuments.some(
      (d) =>
    ...
    return [
      {
        id: "pay_bills",
        capability: t("common:familyCapabilities.payBills"),
        ...
        details: hasBankAccess
          ? t("common:familyCapabilities.canAccessBankAccounts")
          : t("common:familyCapabilities.cannotAccessBankAccounts"),
      },
      {
        id: "insurance_claims",
        capability: t("common:familyCapabilities.insuranceClaims"),
        ...
        details: hasInsurance
          ? t("common:familyCapabilities.canFileInsuranceClaims")
          : t("common:familyCapabilities.cannotFileInsuranceClaims"),
      },
      {
        id: "legal_matters",
        capability: t("common:familyCapabilities.legalMatters"),
        ...
        details: hasLegalContacts
          ? t("common:familyCapabilities.knowsLegalContacts")
          : t("common:familyCapabilities.noLegalContacts"),
      },
      {
        id: "digital_accounts",
        capability: t("common:familyCapabilities.digitalAccounts"),
        ...
        details: hasDigitalAccess
          ? t("common:familyCapabilities.canAccessDigitalAccounts")
          : t("common:familyCapabilities.cannotAccessDigitalAccounts"),
      },
      {
        id: "medical_decisions",
        capability: t("common:familyCapabilities.medicalDecisions"),
        status: hasMedicalDocs
          ? "capable"
        ...
        details: hasMedicalDocs
          ? t("common:familyCapabilities.knowsMedicalWishes")
          : userGuardians.length > 0
            ? t("common:familyCapabilities.partialMedicalInfo")
            : t("common:familyCapabilities.noMedicalInfo"),
      },
      {
        id: "business_operations",
        capability: t("common:familyCapabilities.businessOperations"),
        status:
        ...
        details:
          hasBusinessInfo && userAssets.length > 0
            ? t("common:familyCapabilities.canManageBusiness")
            : hasBusinessInfo
              ? t("common:familyCapabilities.partialBusinessInfo")
              : t("common:familyCapabilities.noBusinessInfo"),
      },
      {
        id: "inheritance",
        capability: t("common:familyCapabilities.inheritance"),
        status:
        ...
        details:
          hasWillOrTrust && userBeneficiaries.length > 0
            ? t("common:familyCapabilities.clearInheritancePlan")
            : userBeneficiaries.length > 0
              ? t("common:familyCapabilities.partialInheritancePlan")
              : t("common:familyCapabilities.noInheritancePlan"),
      },
    ];
    ...
    return [
      {
        ...
        title: t("common:crisisSituations.emergencyHospital.title"),
        description: t("common:crisisSituations.emergencyHospital.description"),
        ...
        isPrepared: [
          ...
          (cap) =>
            capabilityMap[cap] === "capable" ||
            capabilityMap[cap] === "partial",
        ),
        missingCapabilities: [
          "pay_bills",
          "insurance_claims",
          "medical_decisions",
        ].filter((cap) => capabilityMap[cap] === "vulnerable"),
      },
      {
        id: "sudden_death",
        title: t("common:crisisSituations.suddenDeath.title"),
        description: t("common:crisisSituations.suddenDeath.description"),
        ...
        isPrepared: [
          ...
          "digital_accounts",
          "inheritance",
        ].every((cap) => capabilityMap[cap] === "capable"),
        missingCapabilities: [
          "pay_bills",
          "insurance_claims",
          "legal_matters",
          "digital_accounts",
          "inheritance",
        ].filter((cap) => capabilityMap[cap] !== "capable"),
      },
      {
        id: "extended_travel",
        title: t("common:crisisSituations.extendedTravel.title"),
        description: t("common:crisisSituations.extendedTravel.description"),
      ...
      },
      {
        ...
        title: t("common:crisisSituations.businessEmergency.title"),
        description: t("common:crisisSituations.businessEmergency.description"),
  ...
  const preparedSituationsCount = crisisSituations.filter(
    (s) => s.isPrepared,
  ).length;
  const criticalVulnerabilities = capabilities.filter(
  ...
  const getMostCriticalGap = () => {
    const vulnerableCapabilities = capabilities.filter(
      (c) => c.status === "vulnerable",
    );
              ...
              <AlertDescription className="text-red-900">
                ...
                {mostCriticalGap && (
                  <span className="block mt-1">
                    {t("crisisAssessment.mostUrgent")}:{" "}
                    {mostCriticalGap.capability}
                  </span>
                )}
              </AlertDescription>
            </Alert>
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="space-y-1">
              <p className="text-2xl font-bold text-green-600">
                {capabilities.filter((c) => c.status === "capable").length}
              </p>
              <p className="text-sm text-muted-foreground">
                {t("crisisAssessment.protected")}
              </p>
            </div>
            <div className="space-y-1">
                  ...
                  {situation.missingCapabilities.length > 0 && (
                    <div className="text-sm">
                      <span className="text-muted-foreground">
                        {t("crisisAssessment.missing")}:{" "}
                      </span>
                      <span className="text-red-600 font-medium">
                        {situation.missingCapabilities
                          .map(
                            (capId) =>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-red-700 mt-1">
                  {t("crisisAssessment.fixCapability", {
                    capability: mostCriticalGap.capability,
                  })}
                </p>
                <p className="text-xs text-red-600 mt-1">
                  {t("crisisAssessment.timeEstimate")}
                  {t("familyCrisisAssessment.15_1")}
                  {t("ui.minutes")}
                </p>
              </div>
