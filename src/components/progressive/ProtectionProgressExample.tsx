...
export const ProtectionProgressExample: React.FC = () => {
  ...
  const [activeFeature, setActiveFeature] = useState<ProtectionFeature | null>(null);
  ...
  const handleStepComplete = useCallback((stepId: string) => {
    setUserProgress(prev => {
      ...
      if (newCompletedSteps.length >= 4 && newCompletedSteps.includes('basic-will')) {
      ...
      const newAssetCount = stepId === 'basic-asset-documentation' 
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {userProgress.completedSteps.length > 0 ? (
                  ...
                  <span className="text-sm text-muted-foreground">No steps completed yet</span>
                )}
              </div>
            </div>

            <div>
              <h4 className="text-sm font-medium mb-1">Next Recommended Step:</h4>
              {(() => {
