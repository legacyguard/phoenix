...
export const FamilySituationDashboard: React.FC<FamilySituationDashboardProps> = ({
  familyData,
  userPreferences,
  culturalContext,
  ...
  const analyzeFamilySituation = useCallback(() => {
    ...
    const generatedInsights = FamilySituationAnalysisService.generateFamilyInsights(
      situation,
      familyData
    );
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold">{familyData.members.length}</div>
              <div className="text-sm text-gray-600">{t('family.familyMembers')}</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold">{familySituation.specialNeeds.length}</div>
              <div className="text-sm text-gray-600">{t('family.specialConsiderations')}</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold">{familySituation.recommendedFeatures.length}</div>
              <div className="text-sm text-gray-600">{t('family.recommendedFeatures')}</div>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{t('family.keyConsiderations')}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {familySituation.keyConsiderations.map((consideration, index) => (
                  <motion.div
                    key={index}
          ...
          {familySituation.specialNeeds.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{t('family.specialNeeds')}</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  >
                    <Card>
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-full ${getInsightColor(insight.priority)}`}>
                      ...
                      <CardContent>
                        ...
                        {insight.suggestedActions && insight.suggestedActions.length > 0 && (
                          <div className="space-y-2">
                            <p className="text-sm font-medium">{t('family.suggestedActions')}:</p>
        ...
        <TabsContent value="features" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{t('family.recommendedFeaturesTitle')}</CardTitle>
              <CardDescription>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">{t('family.communicationStyle')}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="p-4 bg-blue-50 rounded-lg">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="font-medium">
                    {t(`family.communication.${familySituation.communicationStyle}`)}
                  </span>
                </div>
                <p className="text-sm text-gray-600">
                  {t(`family.communication.${familySituation.communicationStyle}Desc`)}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <TabsContent value="cultural" className="space-y-4">
          {showCulturalSetup ? (
            ...
            <>
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">{t('family.culturalConsiderations')}</CardTitle>
                  <CardDescription>
                    {t('family.culturalDesc')}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {familySituation.culturalConsiderations.length > 0 ? (
                    <div className="space-y-3">
                      {familySituation.culturalConsiderations.map((consideration, index) => (
                        <div
                          key={index}
                          className="flex items-center gap-3 p-3 bg-purple-50 rounded-lg"
                        >
                          <Globe className="h-5 w-5 text-purple-600" />
                          <span className="text-sm">{t(`family.cultural.${consideration}`)}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
