...
export const CreateTimeCapsuleModal: React.FC<CreateTimeCapsuleModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim()) {
      setError(t("common:createTimeCapsuleModal.errors.noTitle"));
      return;
    }

    if (selectedRecipients.length === 0) {
      setError(t("common:createTimeCapsuleModal.errors.noRecipients"));
      return;
    }

    if (unlockCondition === "date" && !unlockDate) {
      setError(t("common:createTimeCapsuleModal.errors.noUnlockDate"));
      return;
    }

    if (messageType === "text" && !textContent.trim()) {
      setError(t("common:createTimeCapsuleModal.errors.noMessage"));
      return;
    }
    ...
    if (
      ...
      setError(t("common:createTimeCapsuleModal.errors.noFile"));
      return;
    }
    ...
    try {
      ...
      if (editingCapsule && !attachment) {
        ...
        if (!response.ok) {
          throw new Error(
            t("common:createTimeCapsuleModal.errors.updateFailed"),
          );
        }
      } else {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error ||
              t("common:createTimeCapsuleModal.errors.saveFailed"),
          );
        }
      }
      ...
      toast.success(
        editingCapsule
          ? t("common:createTimeCapsuleModal.success.updated")
          : t("common:createTimeCapsuleModal.success.created"),
      );
      resetForm();
      onSuccess();
    } catch (err) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      setError(t("common:createTimeCapsuleModal.errors.fileTooLarge"));
      return;
    }
    ...
    if (
      ...
      setError(
        t("common:createTimeCapsuleModal.errors.invalidFileType", {
          messageType,
        }),
      );
      return;
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>
              {editingCapsule
                ? t("common:createTimeCapsuleModal.titleEdit")
                : t("common:createTimeCapsuleModal.titleCreate")}
            </DialogTitle>
            <DialogDescription>
                  ...
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      {unlockDate
                        ...
                        : t(
                            "common:createTimeCapsuleModal.placeholders.selectDate",
                          )}
                    </Button>
                  </PopoverTrigger>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={loading}>
              {loading
                ? t("common:createTimeCapsuleModal.buttons.saving")
                : editingCapsule
                  ? t("common:createTimeCapsuleModal.buttons.update")
                  : t("common:createTimeCapsuleModal.buttons.create")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
