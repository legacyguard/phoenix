...
interface EmpatheticLoaderProps {
  ...
  action?: 'saving' | 'loading' | 'processing' | 'uploading' | 'analyzing' | 'generating' | 'updating';
  emotionalState?: 'first_time' | 'returning' | 'complex_task' | 'sensitive_area' | 'overwhelmed' | 'confident';
  progress?: number;
  className?: string;
...
const EmpatheticLoader: React.FC<EmpatheticLoaderProps> = ({
  ...
  const getContextualMessage = useCallback((): LoadingMessage => {
    ...
    if (context === 'general') {
      ...
      message.primary = t(contextualKey, { defaultValue: t(`general.${action}`) });
    }
    ...
    if (emotionalState) {
      if (emotionalState === 'overwhelmed' || emotionalState === 'confident') {
        ...
        message.encouragement = encouragementMessages[Math.floor(Math.random() * encouragementMessages.length)];
      } else {
    ...
    if (progress !== undefined) {
      if (progress < 25) {
        message.secondary = t('progress.starting');
      } else if (progress < 50) {
        message.secondary = t('progress.quarter');
      } else if (progress < 75) {
        message.secondary = t('progress.half');
      } else if (progress < 100) {
        message.secondary = t('progress.three_quarters');
      } else {
        message.secondary = t('progress.complete');
      }
    }
