...
export function WillVersionHistory({
  ...
  const renderChangesSummary = (changes?: WillChanges) => {
    ...
    if (addedCount > 0) {
      summaryItems.push(t("common:willVersion.added", { count: addedCount }));
    }
    if (removedCount > 0) {
      summaryItems.push(
        t("common:willVersion.removed", { count: removedCount }),
      );
    }
    if (modifiedCount > 0) {
      summaryItems.push(
        t("common:willVersion.modified", { count: modifiedCount }),
      );
    }
