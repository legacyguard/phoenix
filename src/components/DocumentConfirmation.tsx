...
const DocumentConfirmation: React.FC<DocumentConfirmationProps> = ({
  ...
  const [suggestedArea, setSuggestedArea] = useState(
    documentData.suggestedArea,
  );
  const [linkedPossessionId, setLinkedPossessionId] = useState(
    documentData.relationships.linkedPossessionId || "",
  );
  const [linkedPersonId, setLinkedPersonId] = useState(
    documentData.relationships.linkedPersonId || "",
  );
  ...
  return (
    <div className="document-confirmation">
      ...
      <div className="section">
        ...
        <p>
          {t("documentConfirmation.we_detected_this_is_an_3")}
          <strong>{category}</strong>
          {t("documentConfirmation.is_that_correct_4")}
        </p>
      ...
      <div className="section">
        ...
        <p>
          {t("documentConfirmation.we_suggest_filing_this_under_7")}
          <strong>{suggestedArea}</strong>.
        </p>
        <select
          value={suggestedArea}
          onChange={(e) => setSuggestedArea(e.target.value)}
        >
          {["home", "savings", "business", "valuables", "personal"].map(
            (area) => (
      ...
      <div className="section">
        ...
        {linkedPossessionId && (
          <div className="relationship">
            ...
            <select
              value={linkedPossessionId}
              onChange={(e) => setLinkedPossessionId(e.target.value)}
            >
