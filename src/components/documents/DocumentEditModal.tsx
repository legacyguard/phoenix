...
export const DocumentEditModal: React.FC<DocumentEditModalProps> = ({
  ...
  const [contractNumber, setContractNumber] = useState(
    document.metadata?.contractNumber || "",
  );
  const [contactPerson, setContactPerson] = useState(
    document.metadata?.contactPerson || "",
  );
  const [contactPhone, setContactPhone] = useState(
    document.metadata?.contactPhone || "",
  );
  const [contactEmail, setContactEmail] = useState(
    document.metadata?.contactEmail || "",
  );
  const [renewalDate, setRenewalDate] = useState(
    document.metadata?.renewalDate || "",
  );
  const [renewalAction, setRenewalAction] = useState(
    document.metadata?.renewalAction || "",
  );
  const [importanceLevel, setImportanceLevel] = useState(
    document.metadata?.importanceLevel || "medium",
  );
  const [relatedAssets, setRelatedAssets] = useState(
    document.metadata?.relatedAssets?.join(", ") || "",
  );
  ...
  const [renewalCost, setRenewalCost] = useState(
    document.renewal_cost?.toString() || "",
  );
  const [autoRenewal, setAutoRenewal] = useState(
    document.auto_renewal || false,
  );
  const [providerPhone, setProviderPhone] = useState(
    document.provider_contact_info?.phone || "",
  );
  const [providerEmail, setProviderEmail] = useState(
    document.provider_contact_info?.email || "",
  );
  const [providerWebsite, setProviderWebsite] = useState(
    document.provider_contact_info?.website || "",
  );
  ...
  const updateDocumentMutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        ...
        if (relatedAssets) {
          metadata.relatedAssets = relatedAssets
            .split(",")
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            provider_contact_info:
              Object.keys(providerContactInfo).length > 0
                ? providerContactInfo
                : null,
            cancellation_notice_period: cancellationNoticePeriod
              ? parseInt(cancellationNoticePeriod)
              : null,
        ...
        toast.success(t("common:assets.updateSuccess"));
        queryClient.invalidateQueries({ queryKey: ["documents"] });
        onClose();
      } catch (error) {
        console.error("Error updating document:", error);
        toast.error(t("common:assets.updateError"));
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title || !category) {
      toast.error(t("common:assets.validation.requiredFields"));
      return;
    }
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="contractNumber">
                    {t("assets.metadata.contractNumber")}
                  </Label>
                  <Input
                    id="contractNumber"
                ...
                <div>
                  <Label htmlFor="importanceLevel">
                    {t("assets.metadata.importanceLevel")}
                  </Label>
                  <Select
                ...
                <div>
                  <Label htmlFor="contactPerson">
                    {t("assets.metadata.contactPerson")}
                  </Label>
                  <Input
                    id="contactPerson"
                    value={contactPerson}
                ...
                <div>
                  <Label htmlFor="contactPhone">
                    {t("assets.metadata.contactPhone")}
                  </Label>
                  <Input
                    id="contactPhone"
                    type="tel"
                ...
                <div>
                  <Label htmlFor="contactEmail">
                    {t("assets.metadata.contactEmail")}
                  </Label>
                  <Input
                    id="contactEmail"
                    type="email"
                ...
                <div className="col-span-2">
                  <Label htmlFor="renewalAction">
                    {t("assets.metadata.renewalAction")}
                  </Label>
                  <Input
                    id="renewalAction"
                    value={renewalAction}
                ...
                <div className="col-span-2">
                  <Label htmlFor="relatedAssets">
                    {t("assets.metadata.relatedAssets")}
                  </Label>
                  <Input
                    id="relatedAssets"
                    value={relatedAssets}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("assets.subscription.typePlaceholder")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  <Label htmlFor="renewalCost">
                    {t("assets.subscription.renewalCost")}
                  </Label>
                  <Input
                    id="renewalCost"
                    type="number"
                    ...
                    placeholder={t(
                      "assets.subscription.renewalCostPlaceholder",
                    )}
                  />
                </div>
                ...
                <div className="col-span-2 flex items-center space-x-2">
                  ...
                  <Label htmlFor="autoRenewal">
                    {t("assets.subscription.autoRenewal")}
                  </Label>
                </div>

                <div>
                  <Label htmlFor="providerPhone">
                    {t("assets.subscription.providerPhone")}
                  </Label>
                  <Input
                    id="providerPhone"
                    type="tel"
                    ...
                    placeholder={t(
                      "assets.subscription.providerPhonePlaceholder",
                    )}
                  />
                </div>
                ...
                <div>
                  <Label htmlFor="providerEmail">
                    {t("assets.subscription.providerEmail")}
                  </Label>
                  <Input
                    id="providerEmail"
                    type="email"
                    ...
                    placeholder={t(
                      "assets.subscription.providerEmailPlaceholder",
                    )}
                  />
                </div>

                <div>
                  <Label htmlFor="providerWebsite">
                    {t("assets.subscription.providerWebsite")}
                  </Label>
                  <Input
                    id="providerWebsite"
                    type="url"
                    ...
                    placeholder={t(
                      "assets.subscription.providerWebsitePlaceholder",
                    )}
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setCancellationNoticePeriod(e.target.value)
                    }
                    placeholder={t(
                      "assets.subscription.cancellationNoticePlaceholder",
                    )}
                  />
                </div>
              </div>
