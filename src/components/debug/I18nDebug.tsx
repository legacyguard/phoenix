  ...
  return (
    <div className="fixed bottom-4 right-4 bg-black/90 text-white p-4 rounded-lg text-xs max-w-md z-50">
      <h3 className="font-bold mb-2">
        {t("help.i18nDebug.i18n_debug_info_1")}
      </h3>
      <div className="space-y-1">
        ...
        <div>
          {t("help.i18nDebug.resolved_language_3")}
          <span className="text-green-400">{resolvedLanguage}</span>
        </div>
        <div>
          {t("help.translationTest.available_languages_3")}
          <span className="text-blue-400">{availableLanguages.join(", ")}</span>
        </div>
        <div>
          {t("help.i18nDebug.test_translation_5")}
          {testKey}): <span className="text-yellow-400">{translatedValue}</span>
        </div>
        <div>
          {t("help.i18nDebug.has_resources_6")}
          <span className="text-purple-400">{resources ? "Yes" : "No"}</span>
        </div>
        {resources && currentLanguage && resources[currentLanguage] && (
          <div>
            {t("help.i18nDebug.current_lang_resources_7")}
            <span className="text-orange-400">
              {Object.keys(resources[currentLanguage]).join(", ")}
            </span>
          </div>
        )}
      </div>
