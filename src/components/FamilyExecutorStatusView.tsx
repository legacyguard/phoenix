...
export const FamilyExecutorStatusView: React.FC<
  FamilyExecutorStatusViewProps
> = ({ deceasedUserId }) => {
  const { user } = useAuth();
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const { data: commData, error: commError } = await supabase
        ...
        .select(
          "id, beneficiary_name, communication_type, subject, communication_date",
        )
      ...
      const { data: statusData } = await supabase.rpc(
        "get_latest_executor_status",
        { p_deceased_user_id: deceasedUserId },
      );

      if (statusData && statusData[0]) {
        setExecutorName(
          statusData[0].executor_name ||
            t("common:familyExecutorView.executor"),
        );
      }
    } catch (err) {
        ...
        <CardHeader>
          ...
          <CardDescription>
            {t("familyExecutorView.communications.description", {
              executor:
                executorName || t("common:familyExecutorView.theExecutor"),
            })}
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("familyExecutorView.communications.to", {
                        name: comm.beneficiary_name,
                      })}{" "}
                      •{" "}
                      {t(
