...
export const JustInTimeAccess: React.FC<JustInTimeAccessProps> = ({
  ...
  const handleReauthentication = async () => {
    try {
      ...
      if (authError) {
        setError(t("common:justInTimeAccess.errors.incorrectPassword"));
        return;
      }
      ...
      setError(t("common:justInTimeAccess.errors.verificationFailed"));
    } finally {
      setLoading(false);
      setPassword("");
    }
  ...
  const fetchSensitiveData = async () => {
    try {
      ...
      if (error || !data) {
        setError(t("common:justInTimeAccess.errors.noDataFound"));
        return;
      }
      ...
      setError(t("common:justInTimeAccess.errors.retrievalFailed"));
    }
  };
  ...
  const logAccess = async () => {
    try {
      const { error } = await supabase.from("access_logs").insert({
        ...
        action: t("common:justInTimeAccess.logAction", {
          name:
            user?.user_metadata?.full_name ||
            t("common:justInTimeAccess.executor"),
          taskTitle,
        }),
        target_id: taskId,
        metadata: {
        ...
        {!isAuthenticated ? (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {loading
                  ? t("common:justInTimeAccess.actions.verifying")
                  : t("common:justInTimeAccess.actions.verify")}
              </Button>
            </div>
          </div>
        ) : (
              ...
              <Card>
                <CardContent className="pt-6 space-y-4">
                  {renderSensitiveField(
                    t("common:justInTimeAccess.fields.accountName"),
                    sensitiveData.accountName,
                  )}
                  {renderSensitiveField(
                    t("common:justInTimeAccess.fields.accountNumber"),
                    sensitiveData.accountNumber,
                  )}
                  {renderSensitiveField(
                    t("common:justInTimeAccess.fields.username"),
                    sensitiveData.username,
                  )}
                  {renderSensitiveField(
                    t("common:justInTimeAccess.fields.passwordField"),
                    sensitiveData.password,
                    true,
                  )}
                  {renderSensitiveField(
                    t("common:justInTimeAccess.fields.pin"),
                    sensitiveData.pin,
                    true,
                  )}
