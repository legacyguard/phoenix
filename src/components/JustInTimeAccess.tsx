...
export const JustInTimeAccess: React.FC<JustInTimeAccessProps> = ({
    ...
    try {
      const { error } = await supabase.from('access_logs').insert({
        ...
        action: t('justInTimeAccess.logAction', { 
          name: user?.user_metadata?.full_name || t('justInTimeAccess.executor'), 
          taskTitle 
        }),
        target_id: taskId,
        metadata: {
  ...
  const renderSensitiveField = (label: string, value: string | undefined, isPassword: boolean = false) => {
        ...
        {!isAuthenticated ? (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="password">{t('justInTimeAccess.fields.password')}</Label>
              <Input
                id="password"
                type="password"
                value={password}
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {loading ? t('justInTimeAccess.actions.verifying') : t('justInTimeAccess.actions.verify')}
              </Button>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            {sensitiveData ? (
              <Card>
                <CardContent className="pt-6 space-y-4">
                  {renderSensitiveField(t('justInTimeAccess.fields.accountName'), sensitiveData.accountName)}
                  {renderSensitiveField(t('justInTimeAccess.fields.accountNumber'), sensitiveData.accountNumber)}
                  {renderSensitiveField(t('justInTimeAccess.fields.username'), sensitiveData.username)}
                  {renderSensitiveField(t('justInTimeAccess.fields.passwordField'), sensitiveData.password, true)}
                  {renderSensitiveField(t('justInTimeAccess.fields.pin'), sensitiveData.pin, true)}
                  
                  {sensitiveData.notes && (
                    <div className="space-y-1">
                      <Label className="text-sm font-medium">{t('justInTimeAccess.fields.additionalNotes')}</Label>
            ...
            <div className="text-xs text-muted-foreground">
              ...
              <p>{t('justInTimeAccess.accessTime', { time: new Date().toLocaleString() })}</p>
            </div>
