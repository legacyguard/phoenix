...
export const GuardianPlaybook: React.FC<GuardianPlaybookProps> = ({
  ...
  const [templateSection, setTemplateSection] =
    useState<keyof PlaybookTemplates>("funeral_wishes");
  ...
  const loadPlaybook = useCallback(async () => {
    try {
      ...
      toast.error(t("common:family.errors.loadFailed"));
    } finally {
      setIsLoading(false);
    }
  }, [guardianId, t]);
  ...
  const autoSave = useDebouncedCallback(async () => {
    ...
    try {
      ...
      toast.success(t("common:family.messages.autoSaved"), { duration: 1000 });
    } catch (error) {
  ...
  const handleRemoveContact = (contactId: string) => {
    setPlaybook((prev) => ({
      ...prev,
      important_contacts: prev.important_contacts.filter(
        (c) => c.id !== contactId,
      ),
    }));
    autoSave();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t("common:family.messages.saved"));
      onClose();
    } catch (error) {
      toast.error(t("common:family.errors.saveFailed"));
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold">
              {isReadOnly
                ? t("common:playbook.title.view", { name: guardianName })
                : t("common:playbook.title.edit", { name: guardianName })}
            </h2>
            <p className="text-muted-foreground mt-1">
              {isReadOnly
                ? t("common:family.subtitle.view")
                : t("common:family.subtitle.edit")}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            {!isReadOnly && (
              ...
              >
                ...
                {isPreview
                  ? t("common:family.edit")
                  : t("common:family.preview")}
              </Button>
            )}
          </div>
        </div>
      ...
      {!isReadOnly && (
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleSave} disabled={isSaving}>
            ...
            {isSaving ? t("common:family.saving") : t("common:family.save")}
          </Button>
        </div>
      )}
