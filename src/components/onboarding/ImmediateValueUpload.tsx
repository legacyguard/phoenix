...
const ImmediateValueUpload: React.FC<ImmediateValueUploadProps> = ({
  ...
  const categories: Array<{
    ...
    },
    {
      ...
      icon: <Users className="w-5 h-5" />,
      color: 'indigo',
      examples: ['Marriage Certificate', 'Adoption Papers', 'Custody Agreements']
    },
    {
      id: 'other',
  ...
  const generateInsights = (file: File, category: DocumentCategory): DocumentInsight[] => {
    ...
    switch (category) {
      case 'identity':
        insights.push({
          type: 'expiration',
          title: t('respectful.upload.insights.checkExpiration'),
          description: t('respectful.upload.insights.identityExpiration'),
          priority: 'medium',
          action: t('respectful.upload.insights.setReminder')
        });
        break;
      ...
      case 'insurance':
        insights.push({
          type: 'renewal',
          title: t('respectful.upload.insights.renewalNeeded'),
          description: t('respectful.upload.insights.insuranceReview'),
          priority: 'high',
          action: t('respectful.upload.insights.reviewCoverage')
        });
        insights.push({
          type: 'family_share',
          title: t('respectful.upload.insights.shareBeneficiary'),
          description: t('respectful.upload.insights.beneficiaryAccess'),
          priority: 'medium',
          action: t('respectful.upload.insights.addBeneficiary')
        });
        break;
      ...
      case 'legal':
        insights.push({
          type: 'security',
          title: t('respectful.upload.insights.highSecurity'),
          description: t('respectful.upload.insights.legalProtection'),
          priority: 'high',
          action: t('respectful.upload.insights.enableEncryption')
        });
        insights.push({
          type: 'family_share',
          title: t('respectful.upload.insights.executorAccess'),
          description: t('respectful.upload.insights.executorNeeds'),
          priority: 'high',
          action: t('respectful.upload.insights.designateExecutor')
        });
        break;
      ...
      case 'financial':
        insights.push({
          type: 'security',
          title: t('respectful.upload.insights.financialSecurity'),
          description: t('respectful.upload.insights.protectFinancial'),
          priority: 'high',
          action: t('respectful.upload.insights.enableTwoFactor')
        });
        break;
      ...
      case 'property':
        insights.push({
          type: 'missing_info',
          title: t('respectful.upload.insights.propertyDetails'),
          description: t('respectful.upload.insights.addPropertyInfo'),
          priority: 'low',
          action: t('respectful.upload.insights.completeDetails')
        });
        break;
      ...
      default:
        insights.push({
          type: 'security',
          title: t('respectful.upload.insights.documentSecured'),
          description: t('respectful.upload.insights.safelyStored'),
          priority: 'low'
        });
    }
  ...
  const generateSuggestedActions = (category: DocumentCategory): string[] => {
    const baseActions = [
      t('respectful.upload.actions.addDescription'),
      t('respectful.upload.actions.setAccessPermissions')
    ];
    
    const categoryActions: Record<DocumentCategory, string[]> = {
      identity: [
        t('respectful.upload.actions.setExpirationReminder'),
        t('respectful.upload.actions.addEmergencyContact')
      ],
      financial: [
        t('respectful.upload.actions.linkAccounts'),
        t('respectful.upload.actions.addAccountNumbers')
      ],
      property: [
        t('respectful.upload.actions.addPropertyValue'),
        t('respectful.upload.actions.linkInsurance')
      ],
      insurance: [
        t('respectful.upload.actions.addBeneficiaries'),
        t('respectful.upload.actions.setPremiumReminder')
      ],
      legal: [
        t('respectful.upload.actions.designateExecutor'),
        t('respectful.upload.actions.addWitnesses')
      ],
      medical: [
        t('respectful.upload.actions.addEmergencyContacts'),
        t('respectful.upload.actions.listMedications')
      ],
      family: [
        t('respectful.upload.actions.addFamilyMembers'),
        t('respectful.upload.actions.setSharing')
      ],
      other: baseActions
    };
  ...
  const handleConfirmUpload = () => {
    ...
    const newDocument: UploadedDocument = {
      ...
      importance: selectedCategory === 'legal' || selectedCategory === 'insurance' ? 'critical' : 'important',
  ...
  useEffect(() => {
    if (uploadedDocuments.length > 0) {
      localStorage.setItem('immediateValueDocuments', JSON.stringify(uploadedDocuments));
    }
  }, [uploadedDocuments]);
    ...
    <div className="immediate-value-upload max-w-6xl mx-auto p-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">
          {t('respectful.upload.description')}
        </p>
      </div>
        ...
        <div className="mb-6">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              ...
              style={{ width: `${(uploadedDocuments.length / maxDocuments) * 100}%` }}
            />
          </div>
        </div>
      )}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="order-2 md:order-1">
            <div
              ...
              `}
              onClick={() => uploadedDocuments.length < maxDocuments && fileInputRef.current?.click()}
            >
              <input
                ref={fileInputRef}
                type="file"
                ...
                <>
                  ...
                  <p className="text-lg font-medium text-gray-700 mb-2">
                    {isDragging 
                      ...
                      : t('respectful.upload.dragOrClick')
                    }
                  </p>
              ...
              <div className="mt-6 space-y-3">
                ...
                {uploadedDocuments.map((doc) => (
                  <div key={doc.id} className="bg-white border border-gray-200 rounded-lg p-3 flex items-center gap-3">
                    <div className={`p-2 rounded-lg bg-${getCategoryColor(doc.category)}-100`}>
                      <FileText className={`w-5 h-5 text-${getCategoryColor(doc.category)}-600`} />
                    </div>
                    <div className="flex-grow">
                      <p className="font-medium text-gray-900 text-sm">{doc.name}</p>
                      <p className="text-xs text-gray-500">
                        {categories.find(c => c.id === doc.category)?.label} • {formatFileSize(doc.size)}
                      </p>
                    </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl p-6">
              ...
              <div className="mt-6 pt-6 border-t border-blue-100">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['Insurance Policy', 'Will', 'Property Deed', 'Bank Statement'].map((example) => (
                    <div key={example} className="flex items-center gap-2 text-xs text-gray-600">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {generateInsights(currentDocument!, selectedCategory).map((insight, index) => (
                    <div key={index} className="bg-gray-50 rounded-lg p-3 flex items-start gap-3">
                      <Info className={`w-5 h-5 mt-0.5 text-${getPriorityColor(insight.priority)}-600`} />
                      <div className="flex-grow">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {generateSuggestedActions(selectedCategory).slice(0, 3).map((action, index) => (
                    <button
                      key={index}
