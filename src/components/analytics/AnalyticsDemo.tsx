...
export const AnalyticsDemo: React.FC = () => {
  ...
  const handleRandomEvents = () => {
    ...
    for (let i = 0; i < count; i++) {
      setTimeout(() => {
        const randomAction =
          DEMO_ACTIONS[Math.floor(Math.random() * DEMO_ACTIONS.length)];
      ...
      <Card className="border-amber-200 bg-amber-50 dark:bg-amber-950 dark:border-amber-800">
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            ...
            <span>
              {t("analytics.analyticsDemo.analytics_demo_tracking_disabl_1")}
            </span>
          </CardTitle>
          <CardDescription>
            {t("analytics.analyticsDemo.enable_analytics_tracking_in_y_2")}
          </CardDescription>
        </CardHeader>
      </Card>
    );
    ...
    <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center space-x-2">
              ...
              <span>
                {t("analytics.analyticsDemo.analytics_event_generator_3")}
              </span>
            </CardTitle>
            <CardDescription>
              {t("analytics.analyticsDemo.generate_test_events_to_see_re_4")}
            </CardDescription>
          </div>
          <Badge variant="secondary">
            {eventCount}
            {t("analytics.analyticsDemo.events_sent_5")}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3 pt-4 border-t">
          <Button onClick={handleRandomEvents} variant="secondary">
            <Shuffle className="mr-2 h-4 w-4" />
            {t("analytics.analyticsDemo.generate_random_events_6")}
          </Button>
          ...
          >
            {activeTimer
              ? t("common:analytics.demo.stopTimer")
              : t("common:analytics.demo.startTimer")}
          </Button>

          <Button onClick={handleMilestone} variant="default">
            <Target className="mr-2 h-4 w-4" />
            {t("analytics.analyticsDemo.track_milestone_7")}
          </Button>
        </div>
