import React, { useState } from 'react';import type { useTranslation } from "react-i18next";
...
const crisisScenarios: CrisisScenario[] = [
{
  ...
  description: 'Without proper documentation, your family will struggle to access essential accounts and services during an already difficult time.',
  ...
  preventionSteps: [
  {
    ...
    familyBenefit: 'Your spouse will have immediate access to pay household expenses',
    preventionStatus: 'vulnerable'
  },
  {
    id: '1-2',
    crisisScenario: 'Your children cannot find insurance policies when needed',
    actionRequired: 'Record all insurance policy details',
    timeEstimate: 5,
    privacyLevel: 'local_only',
    familyBenefit: 'Your family can quickly file claims and access benefits',
    preventionStatus: 'vulnerable'
  },
  {
    id: '1-3',
    crisisScenario: 'Your family doesn\'t know who to contact for legal matters',
    actionRequired: 'List attorney and legal advisor contacts',
    timeEstimate: 2,
    privacyLevel: 'local_only',
    familyBenefit: 'Your family will have immediate access to legal guidance',
    preventionStatus: 'vulnerable'
  },
  {
    id: '1-4',
...
},
{
  ...
  description: 'Missing documents and unclear financial situations can lead to months of legal complications and financial stress.',
  familyImpact: 'Severe - Extended legal battles and financial uncertainty',
  preventionSteps: [
  {
    id: '2-1',
    crisisScenario: 'Your family doesn\'t understand your financial situation',
    actionRequired: 'Create a financial overview document',
    timeEstimate: 10,
    privacyLevel: 'encrypted_cloud',
    familyBenefit: 'Your family will have a clear picture of assets and obligations',
    preventionStatus: 'vulnerable'
  },
  {
    id: '2-2',
    ...
    timeEstimate: 15,
    privacyLevel: 'encrypted_cloud',
    familyBenefit: 'Your family can quickly access wills, trusts, and legal papers',
    preventionStatus: 'vulnerable'
  },
  {
    id: '2-3',
    crisisScenario: 'Your business operations halt because no one has access',
    actionRequired: 'Document business continuity information',
    timeEstimate: 20,
    privacyLevel: 'encrypted_cloud',
    familyBenefit: 'Your business can continue operating without interruption',
    preventionStatus: 'vulnerable'
  }],
...
},
{
  ...
  description: 'Without clear guidance, your family may make decisions that go against your wishes or harm their interests.',
  ...
  preventionSteps: [
  {
    ...
    familyBenefit: 'Your family can make medical decisions aligned with your wishes',
    preventionStatus: 'vulnerable'
  },
  {
    id: '3-2',
    crisisScenario: 'Your children don\'t understand your wishes for their future',
    actionRequired: 'Write guidance letters for children',
    timeEstimate: 30,
    privacyLevel: 'family_shared',
    familyBenefit: 'Your children will have your wisdom and guidance when needed',
    preventionStatus: 'vulnerable'
  },
  {
    id: '3-3',
...
const FamilyCrisisPrevention: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'partially_protected':return '#D97706';
      case 'fully_protected':return '#059669';
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: '32px' }}>
        <h1 style={{ fontSize: '32px', fontWeight: '700', color: '#111827', marginBottom: '8px' }}>{t("familyCrisisPrevention.family_crisis_prevention_1")}

        </h1>
        <p style={{ fontSize: '18px', color: '#6B7280' }}>{t("familyCrisisPrevention.protect_your_family_from_chaos_2")}

        </p>
      </div>

      {scenarios.map((scenario) => {
        const protectedSteps = scenario.preventionSteps.filter((step) => step.preventionStatus === 'fully_protected').length;
          ...
          <div key={scenario.id} style={{
            ...
            <div style={{ padding: '24px', borderBottom: '1px solid #E5E7EB' }}>
              ...
              <div style={{ display: 'flex', gap: '24px', fontSize: '14px' }}>
                <div>
                  <span style={{ color: '#6B7280' }}>{t("familyCrisisPrevention.impact_3")}</span>
                  <span style={{ color: '#DC2626', fontWeight: '600' }}>{scenario.familyImpact}</span>
                </div>
                <div>
                  <span style={{ color: '#6B7280' }}>{t("familyCrisisPrevention.affects_4")}</span>
                  <span style={{ color: '#111827', fontWeight: '600' }}>{scenario.affectedFamilyMembers.join(', ')}</span>
                </div>
                <div>
                  <span style={{ color: '#6B7280' }}>{t("familyCrisisPrevention.protection_status_5")}</span>
                  <span style={{ color: allProtected ? '#059669' : '#DC2626', fontWeight: '600' }}>
                    {protectedSteps}/{totalSteps}{t("familyCrisisPrevention.steps_completed_6")}
                  </span>
                </div>
              </div>
            </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginBottom: '12px' }}>
                        <p style={{ color: '#111827', fontWeight: '500', marginBottom: '4px' }}>{t("familyCrisisPrevention.prevention_step_8")}
                        {step.actionRequired}
                        </p>
                        <p style={{ color: '#059669', fontSize: '14px' }}>
                          <strong>{t("familyCrisisPrevention.family_benefit_9")}</strong> {step.familyBenefit}
                        </p>
                      </div>
                      <div style={{ display: 'flex', gap: '16px', fontSize: '14px', color: '#6B7280' }}>
                        <span>⏱️ {step.timeEstimate} minutes</span>
                        <span>{t("familyCrisisPrevention.privacy_10")}{step.privacyLevel.replace('_', ' ')}</span>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#DC2626'}>{t("familyCrisisPrevention.prevent_this_crisis_11")}
                    ...
                    <div style={{
                      ...
                      fontSize: '14px'
                          ...
                          <span style={{ fontSize: '20px' }}>✓</span>{t("familyCrisisPrevention.crisis_prevented_12")}
