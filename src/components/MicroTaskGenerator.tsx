...
import { ResponsibilityArchetype } from "./PersonalizationEngine";
...
const MicroTaskGenerator: React.FC = () => {
  ...
  const taskData: MicroTask[] = [
    {
      id: "1",
      title: t("common:microTaskGenerator.tasks.driversLicense.title"),
      description: t(
        "common:microTaskGenerator.tasks.driversLicense.description",
      ),
      estimatedTime: 2,
      difficultyLevel: "easy",
      familyImpact: t("common:microTaskGenerator.familyImpactLevels.high"),
      completionMessage: t(
        "common:microTaskGenerator.tasks.driversLicense.completionMessage",
      ),
      scoring: { urgency: 70, impact: 90, ease: 80, momentum: 85 },
      triggers: {
        archetype: ["FamilyAnchor"],
        preparednessLevel: 1,
        familyComplexity: 1,
        contextualTriggers: [],
      },
  ...
  const startTask = (taskId: string) => {
    setTaskState((prevState) =>
      prevState.map((task) =>
        task.id === taskId
          ? { ...task, status: "in_progress", startedAt: new Date() }
          : task,
      ),
    );
  };
        ...
        return (
          <div key={task.id}>
            ...
            <p>
              ...
              {t("microTaskGenerator.minutes_1")}
              {task.familyImpact}
              {t("microTaskGenerator.family_impact_2")}
            </p>
            {taskStatus?.status === "available" && (
              <button onClick={() => startTask(task.id)}>
                {t("microTaskGenerator.start_task_3")}
              </button>
            )}
            {taskStatus?.status === "in_progress" && (
              <button onClick={() => completeTask(task.id)}>
                {t("microTaskGenerator.complete_task_4")}
              </button>
            )}
            {taskStatus?.status !== "completed" && (
              <button onClick={() => skipTask(task.id)}>
                {t("microTaskGenerator.skip_for_now_5")}
              </button>
            )}
            {taskStatus?.status === "completed" && (
