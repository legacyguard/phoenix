...
describe('Time Capsule System', () => {
  ...
  describe('Encryption/Decryption', () => {
    ...
    it('should decrypt file content after retrieval', () => {
      ...
      const decrypted = CryptoJS.AES.decrypt(encryptedContent, 'secret-key').toString();
  ...
  describe('API Endpoints', () => {
    describe('POST /api/time-capsule', () => {
      it('should create a time capsule with text content', async () => {
        const mockRequest = new NextRequest('http://localhost:3000/api/time-capsule', {
          method: 'POST',
          headers: {
  ...
  describe('Delivery Service', () => {
    it('should check and unlock time capsules based on date', async () => {
        ...
        from: vi.fn((table: string) => {
          ...
          if (table === 'profiles') {
            return {
              select: vi.fn().mockReturnThis(),
              eq: vi.fn().mockReturnThis(),
              single: vi.fn().mockResolvedValue({ data: mockSender, error: null })
            };
          }
      ...
      (supabaseMock.from('time_capsule_messages') as any).select = vi.fn().mockReturnValueOnce({
        eq: vi.fn().mockResolvedValue({ data: mockLockedCapsules, error: null })
      });
        ...
        from: vi.fn((table: string) => {
          ...
          if (table === 'profiles') {
            return {
              select: vi.fn().mockReturnThis(),
              eq: vi.fn().mockReturnThis(),
              single: vi.fn().mockResolvedValue({ data: mockDeceasedUser, error: null })
            };
          }
          return {};
        })
  ...
  describe('Security', () => {
    ...
    it('should encrypt sensitive content', () => {
      ...
      const encrypted = CryptoJS.AES.encrypt(sensitiveData, 'secret-key').toString();
