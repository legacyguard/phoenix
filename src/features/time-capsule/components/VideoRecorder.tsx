...
export function VideoRecorder({
  ...
  useEffect(() => {
    const initializeMedia = async () => {
      try {
        ...
        setError(
          t(
            "common:TimeCapsule.videoRecorder.unable_to_access_camera_or_mic_1",
          ),
        );
      }
    };
  ...
  const handleSaveVideo = useCallback(async () => {
    ...
    if (fileSizeMB > maxFileSizeMB) {
      setError(
        `Video file is too large (${fileSizeMB.toFixed(1)}MB). Maximum allowed is ${maxFileSizeMB}MB.`,
      );
      return;
    }
    ...
    try {
      ...
      setError(
        t("common:TimeCapsule.videoRecorder.failed_to_process_video_please_2"),
      );
      ...
      <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-2xl font-semibold text-gray-900">
            {t("TimeCapsule.videoRecorder.record_video_message_3")}
          </h2>
          <Button
            variant="ghost"
            size="icon"
        ...
        <div className="p-6">
            ...
            {isRecording && (
              <div className="absolute top-4 left-4 flex items-center space-x-2 bg-red-600 text-white px-3 py-1.5 rounded-full">
                ...
                <span className="text-sm font-medium">
                  {tMicro("statusMessages.loading.processing")}
                </span>
              </div>
            )}
          ...
          <div className="text-center mb-6">
            <p className="text-sm text-gray-600">
              {t("TimeCapsule.videoRecorder.maximum_recording_duration_4")}
              {formatDuration(maxDurationSeconds)}
            </p>
            <p className="text-sm text-gray-600">
              {t("TimeCapsule.videoRecorder.maximum_file_size_5")}
              {maxFileSizeMB}MB
            </p>
          </div>
