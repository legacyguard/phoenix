...
export const LegacyLetters: React.FC = () => {
  ...
  const fetchCapsules = useCallback(async () => {
    ...
    try {
      ...
      setError(t("common:legacyLetters.failed_to_load_your_time_capsu_1"));
    } finally {
      setLoading(false);
    }
  }, [user, t]);
  ...
  const handleDelete = async (capsuleId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t("common:legacyLetters.errors.deleteFailed"));
      }

      toast.success(t("common:legacyLetters.messages.deleteSuccess"));
      fetchCapsules();
    } catch (err) {
      console.error("Error deleting time capsule:", err);
      toast.error(t("common:legacyLetters.messages.deleteFailed"));
    }
  };
