...
export const AssetDetail: React.FC = () => {
  ...
  const loadAsset = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        financial_institution:
          ((data.metadata as Record<string, unknown>)
            ?.financial_institution as string) || "",
        account_number:
          ((data.metadata as Record<string, unknown>)
            ?.account_number as string) || "",
        login_credentials:
          ((data.metadata as Record<string, unknown>)
            ?.login_credentials as string) || "",
      });
    } catch (error) {
  ...
  const handleSave = async () => {
    if (!formData.name.trim()) {
      toast.error(t("common:validation.nameRequired"));
      return;
    }
    ...
    try {
      ...
      const assetData = {
        ...
        property_registry_number:
          formData.property_registry_number.trim() || null,
      ...
      if (isNewAsset) {
        ...
        toast.success(t("common:ui.assetAdded"));
      } else {
        const { error } = await supabaseWithRetry
          .from("assets")
          .update(assetData)
        ...
        toast.success(t("common:ui.assetUpdated"));
      }
  ...
  const getAssetTypeLabel = (type: string) => {
    switch (type) {
      case "RealEstate":
        return t("common:types.primaryResidence");
      case "Business":
        return t("common:types.businessOwnership");
      case "Vehicle":
        return t("common:types.automobile");
      case "FinancialAccount":
        return t("common:types.checkingAccount");
      default:
        return t("common:form.name");
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">
              {isNewAsset ? t("common:form.addAsset") : formData.name}
            </h1>
            <p className="text-muted-foreground">
              {isNewAsset
                ? t("common:form.assetDetails")
                : t("common:form.editAsset")}
            </p>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("common:form.assetDetails")}</CardTitle>
        </CardHeader>
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="RealEstate">
                      {t("common:types.primaryResidence")}
                    </SelectItem>
                    <SelectItem value="Business">
                      {t("common:types.businessOwnership")}
                    </SelectItem>
                    <SelectItem value="Vehicle">
                      {t("common:types.automobile")}
                    </SelectItem>
                    <SelectItem value="FinancialAccount">
                      {t("common:types.checkingAccount")}
                    </SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
                  <p className="text-xs text-yellow-800">
                    ⚠️ <strong>{t("form.securityNote")}</strong>{" "}
                    {t("form.accountHelper")}
                  </p>
                </div>
              </div>
            ...
            <div>
              {liabilities.length === 0 ? (
                ...
                liabilities.map((liability) => (
                  ...
                  >
                    <p>
                      <strong>{t("form.liens")}:</strong>{" "}
                      {liability.liability_type}
                    </p>
                    <p>
                      <strong>{t("form.institution")}:</strong>{" "}
                      {liability.provider_name}
                    </p>
                    <p>
                      <strong>{t("form.accountNumber")}:</strong>{" "}
