...
export const SubscriptionDashboard: React.FC = () => {
  ...
  const [preferences, setPreferences] =
    useState<SubscriptionPreferences | null>(null);
  ...
  const loadSubscriptionData = useCallback(async () => {
    try {
      ...
      toast.error(t("dashboard-main:subscription.loadError"));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const updatePreference = async (
    ...
    try {
      ...
      toast.success(t("dashboard-main:subscription.preferencesUpdated"));
    } catch (error) {
      console.error("Error updating preferences:", error);
      toast.error(t("dashboard-main:subscription.updateError"));
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {t("subscription.dashboard.subtitle")}
          </p>
        </div>
      ...
      {upcomingRenewals.length > 0 && (
        <Alert className="border-primary">
          ...
          <AlertDescription>
            {t("subscription.upcomingRenewalsDesc", {
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="upcoming">
            {t("subscription.tabs.upcoming")}
          </TabsTrigger>
          <TabsTrigger value="settings">
            {t("subscription.tabs.settings")}
          </TabsTrigger>
        </TabsList>
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant={sub.auto_renewal ? "default" : "secondary"}
                          >
                            {sub.auto_renewal
                              ? t("dashboard-main:subscription.autoRenewal")
                              : t("dashboard-main:subscription.manual")}
                          </Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {sub.subscription_type === "monthly"
                            ? t("dashboard-main:subscription.types.monthly")
                            : t("dashboard-main:subscription.types.yearly")}
        ...
        <TabsContent value="upcoming" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("subscription.next90Days")}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          ...
                          {sub.cancellation_notice_period > 0 &&
                            daysUntil <= sub.cancellation_notice_period && (
                              <p className="text-sm text-destructive flex items-center gap-1">
                                <AlertCircle className="h-3 w-3" />
                                {t("subscription.cancellationNoticeWarning", {
                                  days: sub.cancellation_notice_period,
                                })}
                              </p>
                            )}
                        </div>
                        <div className="flex gap-2">
                          ...
                          {!sub.auto_renewal && (
                            <Button size="sm">{t("subscription.renew")}</Button>
                          )}
                        </div>
                      </div>
                    </Alert>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    updatePreference("tracking_enabled", checked)
                  }
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    updatePreference("cost_optimization_enabled", checked)
                  }
                />
              </div>
