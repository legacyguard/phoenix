...
export const FamilyHub: React.FC = () => {
  ...
  const [emergencyProtocolEnabled, setEmergencyProtocolEnabled] =
    useState(false);
  ...
  useEffect(() => {
    const fetchFamilyHubData = async () => {
      try {
        ...
        if (!token) {
          throw new Error(t("common:familyHub.errors.authenticationRequired"));
        }
        ...
        if (!response.ok) {
          throw new Error(t("common:familyHub.errors.failedToFetchData"));
        }
        ...
        const avgPreparedness =
          data.familyMembers.length > 0
            ? data.familyMembers.reduce(
                (sum: number, member: FamilyMember) =>
                  sum + member.preparednessScore,
                0,
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : t("common:familyHub.errors.failedToLoadMembers");
  ...
  const handleAccessLevelChange = async (
    ...
    try {
      ...
      toast.success(t("common:familyHub.messages.accessLevelUpdated"));
    } catch (error) {
      toast.error(t("common:familyHub.errors.failedToLoadMembers"));
    }
  };
  ...
  const handleSendUpdate = async (member: FamilyMember) => {
    try {
      ...
      toast.success(
        t("common:familyHub.messages.updateSentTo", { name: member.name }),
      );
      ...
      setFamilyMembers((prev) =>
        prev.map((m) =>
          m.id === member.id
            ? { ...m, lastCommunicated: new Date().toISOString() }
            : m,
        ),
      );
    } catch (error) {
      toast.error(
        t("common:familyHub.messages.failedToSendUpdate", {
          name: member.name,
        }),
      );
    }
  ...
  const getAccessLevelDescription = (level: AccessLevel): string => {
    switch (level) {
      case "none":
        return t("common:familyHub.accessLevels.none.description");
      case "emergency_only":
        return t("common:familyHub.accessLevels.emergencyOnly.fullDescription");
      case "limited_info":
        return t("common:familyHub.accessLevels.limitedInfo.fullDescription");
      case "full_access":
        return t("common:familyHub.accessLevels.fullAccess.fullDescription");
      default:
        return "";
    }
  };
            ...
            <CardContent className="text-center p-4 md:p-6">
              ...
              <div className="relative inline-flex items-center justify-center">
                ...
                <div className="absolute">
                  <p
                    className={`text-3xl md:text-5xl font-bold ${getPreparednessColor(overallPreparedness)}`}
                  >
                    {overallPreparedness}%
                  </p>
                </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                >
                  {emergencyProtocolEnabled
                    ? t("common:familyHub.emergencyProtocol.configured")
                    : t("common:familyHub.emergencyProtocol.configure")}
                </Button>
              </div>
            </CardHeader>
                ...
                >
                  ...
                  <CardContent className="p-4 md:p-6 pt-0 md:pt-0 space-y-4">
                    <div>
                        ...
                        <SelectContent>
                          <SelectItem value="none" className="py-3">
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                {t(
                                  "common:familyHub.accessLevels.none.description",
                                )}
                              </p>
                            </div>
                          </SelectItem>
                          <SelectItem value="emergency_only" className="py-3">
                            <div>
                              <p className="font-medium text-sm">
                                {t(
                                  "familyHub.accessLevels.emergencyOnly.title",
                                )}
                              </p>
                              <p className="text-xs text-gray-500">
                                {t(
                                  "familyHub.accessLevels.emergencyOnly.description",
                                )}
                              </p>
                            </div>
                          </SelectItem>
                          <SelectItem value="limited_info" className="py-3">
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                {t(
                                  "familyHub.accessLevels.limitedInfo.description",
                                )}
                              </p>
                            </div>
                          </SelectItem>
                          <SelectItem value="full_access" className="py-3">
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                {t(
                                  "familyHub.accessLevels.fullAccess.description",
                                )}
                              </p>
                            </div>
                          </SelectItem>
                      ...
                      <p
                        className={`text-xs mt-1 ${getAccessLevelColor(member.accessLevel)}`}
                      >
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="text-xs md:text-sm text-gray-600">
                          {t("familyHub.memberManagement.preparedness")}
                        </span>
                        <span
                          className={`text-xs md:text-sm font-bold ${getPreparednessColor(member.preparednessScore)}`}
                        >
                    ...
                    <div className="pt-3 md:pt-4 border-t">
                      <p className="text-xs text-gray-500 mb-3">
                        {t("familyHub.memberManagement.lastContacted")}:{" "}
                        {new Date(member.lastCommunicated).toLocaleDateString()}
                        (
                        {t("familyHub.memberManagement.daysAgo", {
                          days: getDaysSinceContact(member.lastCommunicated),
                        })}
                        )
                      </p>
                      <Button
                      ...
                      >
                        ...
                        <span className="sm:hidden">
                          {t("familyHub.memberManagement.sendUpdate")}
                        </span>
                      </Button>
                    </div>
                  </CardContent>
      ...
      {!loading && (
        <PersonalAssistant
          ...
          contextData={{
            ...
            overallPreparedness,
            emergencyProtocolEnabled,
            membersNeedingAttention: familyMembers.filter(
              (m) =>
