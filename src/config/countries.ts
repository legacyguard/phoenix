...
export type CountryCode = 'DE' | 'GB' | 'FR' | 'CZ' | 'SK' | 'PL' | 'AT' | 'CH' | 'IT' | 'ES' | 
  'NL' | 'BE' | 'LU' | 'LI' | 'DK' | 'SE' | 'FI' | 'NO' | 'IS' | 'IE' | 'PT' | 'GR' | 
  'HU' | 'RO' | 'BG' | 'HR' | 'SI' | 'EE' | 'LV' | 'LT' | 'MT' | 'CY' | 'MD' | 'UA' | 
  'RS' | 'AL' | 'MK' | 'ME' | 'BA';

export type LanguageCode = 'en' | 'de' | 'fr' | 'es' | 'it' | 'pl' | 'cs' | 'sk' | 'hu' | 
  'ro' | 'bg' | 'hr' | 'sl' | 'et' | 'lv' | 'lt' | 'el' | 'nl' | 'da' | 'sv' | 'fi' | 
  'no' | 'is' | 'pt' | 'mt' | 'sq' | 'mk' | 'sr' | 'uk' | 'ru' | 'tr' | 'ga' | 'bs' | 'cy';

export const SUPPORTED_COUNTRIES: CountryCode[] = [
  'DE', 'GB', 'FR', 'CZ', 'SK', 'PL', 'AT', 'CH', 'IT', 'ES', 'NL', 'BE', 'LU', 'LI',
  'DK', 'SE', 'FI', 'NO', 'IS', 'IE', 'PT', 'GR', 'HU', 'RO', 'BG', 'HR', 'SI', 'EE',
  'LV', 'LT', 'MT', 'CY', 'MD', 'UA', 'RS', 'AL', 'MK', 'ME', 'BA'
];
