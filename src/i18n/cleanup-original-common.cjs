...
const languages = fs
  ...
  .filter(
    (entry) => entry.isDirectory() && entry.name !== "_backup_original_common",
  )
  ...
  if (fs.existsSync(originalCommonPath)) {
    try {
      ...
      if (isLargeOriginal) {
        ...
        console.log(
          `✓ Backed up and deleted: ${lang}/common.json (${(stats.size / 1024).toFixed(1)}KB)`,
        );
        processedCount++;
      } else {
        console.log(
          `- Skipped: ${lang}/common.json (appears to be new modular version)`,
        );
      }
    } catch (error) {
