...
def fix_json_syntax(content):
    ...
    # Fix missing commas between objects (looking for pattern }newline"key")
    content = re.sub(r'}\s*\n\s*"', '},\n      "', content)
...
def fix_locale_file(locale_code):
    ...
    try:
        ...
        try:
            ...
            print(f"❌ {locale_code}: Still has JSON errors after fix attempt: {e}")
            return False
        ...
        # Check if they exist but might be misplaced
        if 'personalMessages' in data['dashboard'] and 'personalMessages' not in tasks:
            # Move it to tasks
            tasks['personalMessages'] = data['dashboard']['personalMessages']
            del data['dashboard']['personalMessages']
            has_personal = True
        
        if 'familyMeeting' in data['dashboard'] and 'familyMeeting' not in tasks:
            # Move it to tasks
            tasks['familyMeeting'] = data['dashboard']['familyMeeting']
            del data['dashboard']['familyMeeting']
            has_family = True
        
        if has_personal and has_family:
            # Make sure they're in the right order after emergencyContacts
            if 'emergencyContacts' in tasks:
                ...
                for key, value in tasks.items():
                    ...
                    if key == 'emergencyContacts':
                        new_tasks['personalMessages'] = tasks.get('personalMessages', translations.get('personalMessages', {}))
                        new_tasks['familyMeeting'] = tasks.get('familyMeeting', translations.get('familyMeeting', {}))
                ...
                print(f"✅ {locale_code}: Fixed! Tasks reordered correctly")
                return True
            else:
...
def main():
    ...
    print("🔧 Fixing locale files with JSON syntax errors and proper dashboard.tasks structure\n")
