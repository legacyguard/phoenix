...
const commonPath = '/Users/luborfedak/Documents/Github/phoenix/src/i18n/locales/en/common.json';
...
const moduleMapping = {
  ...
  'common': {
    keys: ['common', 'action', 'cta', 'validation', 'errors', 'app', 'index', 'notfound', 'countryLanguage'],
    description: 'Shared translations used across the application'
  },
  
  // Authentication & user management
  'auth': {
    keys: ['consent', 'privacyControlPanel', 'privacy', 'securityStatus', 'inviteAcceptance', 'invite'],
  ...
  // Dashboard & analytics
  'dashboard': {
    keys: ['dashboard', 'analytics', 'notifications', 'notificationSettings', 'notificationPreferences'],
    description: 'Dashboard and analytics features'
  },
  
  // Document management
  'documents': {
    keys: ['documentTypes', 'documentUpload', 'documentUploader', 'vault', 'upload', 'createTimeCapsuleModal'],
    description: 'Document management and vault features'
  },
  
  // Assets & possessions
  'assets': {
    keys: ['assets', 'assetCard', 'assetDetail', 'assetType', 'dynamicAssetForm', 'myPossessions', 'addLiabilityModal', 'calculator'],
    description: 'Asset and possession management'
  },
  
  // Family features
  'family': {
    keys: ['familyHub', 'familyPreparedness', 'familyPreparednessTools', 'relationships', 'beneficiaryCommunications', 'emergencyAccess', 'emotionalContextSystem', 'lifeEvents', 'logLifeEvent', 'contacts'],
    description: 'Family hub and preparedness features'
  },
  
  // Guardian features
  'guardians': {
    keys: ['guardian', 'guardianUpload', 'guardianView', 'executor', 'executorManagement'],
    description: 'Guardian and executor management'
  },
  
  // Will & planning
  'wills': {
    keys: ['will', 'willGenerator', 'will_generator', 'playbook', 'scenarioPlanner', 'scenarios', 'annualReview', 'complexityReduction', 'microTaskGenerator', 'editStoryModal'],
...
Object.entries(moduleMapping).forEach(([moduleName, config]) => {
  ...
  if (Object.keys(moduleData).length > 0) {
    const outputPath = path.join('/Users/luborfedak/Documents/Github/phoenix/src/i18n/locales/en', `${moduleName}.json`);
    ...
    console.log(`Created ${moduleName}.json with ${Object.keys(moduleData).length} top-level keys`);
  }
});
...
const unmappedKeys = Object.keys(allTranslations).filter(key => !allMappedKeys.has(key));
if (unmappedKeys.length > 0) {
  console.log('\nWarning: The following keys were not mapped to any module:', unmappedKeys);
}
