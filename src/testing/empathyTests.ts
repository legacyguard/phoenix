  ...
  describe("Call-to-Action Tests", () => {
    test("All CTAs focus on family benefit", () => {
      ...
      ctas.forEach((cta) => {
        ...
        expect(cta.text).toMatch(
          /family|loved ones|protect|help|prepare|peace|legacy/i,
        );

        // Should NOT contain technical action words
        expect(cta.text).not.toMatch(
          /add|create|manage|configure|edit|delete|submit/i,
        );
      });
    });
    ...
    test("Error messages provide emotional support", () => {
      ...
      errorMessages.forEach((message) => {
        ...
        expect(message).toMatch(
          /don't worry|safe|help|try again|it's okay|no problem/i,
        );

        // Should NOT contain technical error terms
        expect(message).not.toMatch(
          /error|failed|invalid|wrong|incorrect|exception/i,
        );
      });
    });
    ...
    test("Empty states provide gentle encouragement", () => {
      ...
      emptyStates.forEach((state) => {
        expect(state).toMatch(
          /when you're ready|whenever you'd like|I'm here to help/i,
        );
  ...
  test("Every action clearly explains family benefit", () => {
    ...
    allActions.forEach((action) => {
      ...
      expect(action.description).toMatch(
        /your family|loved ones|family members/i,
      );

      // Should avoid technical language
      expect(action.description).not.toMatch(
        /system|database|configuration|record/i,
      );
    });
  });
...
describe("Assistant Personality Consistency Tests", () => {
  ...
  test("Maintains caring personality throughout", () => {
    ...
    messages.forEach((message) => {
      ...
      expect(message.content).toMatch(
        /understand|help|support|important|care/i,
      );
  ...
  test("Provides specific, actionable guidance", () => {
    const guidanceMessages = getAllAssistantMessages().filter(
      (m) => m.type === "guidance",
    );
...
async function measureEmotionalResponse(
  context: string,
