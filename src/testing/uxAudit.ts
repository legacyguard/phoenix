...
const TECHNICAL_TERMS = [
  'asset', 'configure', 'manage', 'optimize', 'process', 'system',
  'database', 'user', 'interface', 'functionality', 'feature',
  'workflow', 'module', 'component', 'integration', 'implementation'
];

// Family-focused terms to encourage
const FAMILY_TERMS = [
  'family', 'loved ones', 'children', 'spouse', 'parents',
  'protect', 'care', 'help', 'support', 'prepare', 'peace of mind',
  'legacy', 'wishes', 'important to you', 'your story'
];

// Empathetic phrases
const EMPATHETIC_PHRASES = [
  "I understand", "I'm here to help", "don't worry", "take your time",
  "this is important", "your family", "when you're ready", "let me help"
];
...
export const auditFamilyFocus = async (): Promise<UXAuditResult> => {
  ...
  const familyFocusRatio = familyReferences / (familyReferences + featureReferences + 1);
...
export const auditPersonalAssistantPersonality = async (): Promise<UXAuditResult> => {
  ...
  messages.forEach(message => {
    ...
    const caringIndicators = ['help', 'support', 'understand', 'care', 'important'];
  ...
  const score = (consistencyScore + (personalityScore / messages.length * 10)) / 2;
...
export const auditErrorHandling = async (): Promise<UXAuditResult> => {
  ...
  errorMessages.forEach(error => {
    ...
    const technicalErrorTerms = ['error', 'failed', 'invalid', 'incorrect', 'wrong'];
