...
const simulateJourneyStep = async (
  ...
  return {
    ...
    behavioralResponse,
    success:
      emotionalResponse === step.expectedEmotion &&
...
async function measureBehavioralResponse(
  user: TestUser,
  step: JourneyStep,
): Promise<string> {
...
export const validateEmpatheticJourney = async (): Promise<{
  ...
  for (const [scenarioName, scenario] of Object.entries(journeyScenarios)) {
    const user = createTestUser({
      familySituation: scenarioName.includes("blended")
        ? "blended_family"
        : "young_family",
    });
  ...
  const overallSuccess = Array.from(scenarioResults.values()).every(
    (result) => result,
  );
...
async function testScenarioJourney(
  user: TestUser,
