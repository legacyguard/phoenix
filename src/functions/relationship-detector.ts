...
export async function detectRelationships(
  ...
  if (propertyAddress) {
    const linkedHome = possessions.find(
      (p) => p.details?.address === propertyAddress,
    );
  ...
  const hasPropertyInsurance = possessions.some(
    (p) => p.type === "property_insurance",
  );
  if (hasMortgage && !hasPropertyInsurance) {
    result.missingDocumentSuggestion =
      "It looks like you have a mortgage but no property insurance linked. Consider adding your policy.";
  }
