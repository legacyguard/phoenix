...
export async function suggestCategory(
  ...
  const suggestion = analyzeDocumentForSuggestion(
    docCategory,
    metadata,
    extractedText,
  );
...
function analyzeDocumentForSuggestion(
  ...
  const hasFinancialInfo =
    metadata.accountNumber || metadata.balance !== undefined;
  ...
  switch (docCategory) {
    case "insurance_policy": {
      ...
      if (
        ...
        policyType.includes("property") ||
        hasPropertyInfo
      ) {
        return {
          area: "home",
          reasoning:
            "This is a homeowners insurance policy that protects your property",
        };
      }
      if (
        policyType.includes("auto") ||
        ...
        return {
          area: "home",
          reasoning:
            "Vehicle insurance is typically managed alongside home and property",
        };
      }
      if (policyType.includes("life") || policyType.includes("disability")) {
        return {
          area: "personal",
          reasoning:
            "Life and disability insurance are personal protection documents",
        };
      }
    ...
    case "property_deed":
      return {
        area: "home",
        reasoning:
          "Property deeds belong with your home and real estate documents",
      };
    ...
    case "vehicle_title":
      return {
        area: "home",
        reasoning:
          "Vehicle titles are typically stored with home and property documents",
      };
    ...
    case "bank_statement": {
      ...
      if (
        ...
        accountType.includes("money market")
      ) {
        return {
          area: "savings",
          reasoning:
            "Savings account statements belong with your financial assets",
        };
      }
      return {
        area: "personal",
    ...
    case "investment_statement":
      return {
        area: "savings",
        reasoning:
          "Investment accounts are part of your savings and future planning",
      };
    ...
    case "will_or_trust":
      return {
        area: "personal",
        reasoning:
          "Estate planning documents are crucial personal legal documents",
      };
  ...
  if (docCategory === "insurance_policy") {
    if (primarySuggestion !== "personal") {
      alternatives.push({
        ...
        confidence: 0.6,
        reasoning:
          "Insurance policies can also be filed with personal documents",
      });
    }
    if (
      primarySuggestion !== "home" &&
      (metadata.policyType === "home" || metadata.propertyAddress)
    ) {
      alternatives.push({
        area: "home",
        confidence: 0.7,
        reasoning:
          "Property-related insurance could be filed with home documents",
      });
    }
  }
...
export function getAreaDescription(areaId: PossessionAreaId): string {
  const descriptions: Record<PossessionAreaId, string> = {
    ...
    savings:
      "Your Savings & Investments - Financial accounts and investment documents",
    business: "Your Business - Business ownership and operational documents",
    valuables:
      "Your Valuables & Collections - Documentation for valuable items",
    personal:
      "Personal & Legal - IDs, medical records, estate planning, and personal documents",
  };
