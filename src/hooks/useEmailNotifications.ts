...
export const useEmailNotifications = () => {
  ...
  const sendTaskReminder = async (
    ...
    try {
      ...
      if (
        ...
        const notificationChannels = channels.filter((c) => c !== "email") as (
          | "push"
          | "sms"
          | "inApp"
        )[];
  ...
  const sendSubscriptionNotification = async (
    ...
    try {
      ...
      // Send email notification
      if (channels.includes("email")) {
        results.email = await emailService.sendSubscriptionEmail(
          user,
          type,
          data,
        );
  ...
  const sendFamilyInvitation = async (
    ...
    try {
      const success = await emailService.sendFamilyInvitationEmail(
        recipientEmail,
        senderName,
        data,
      );
  ...
  const sendSecurityCode = async (phoneNumber: string, code: string) => {
    try {
      const success = await notificationService.sendSecurityCode(
        phoneNumber,
        code,
      );
      if (success) {
  ...
  const sendLoginAlert = async (user: User, location: string, time: string) => {
    try {
      ...
      const success = await notificationService.sendLoginAlert(
        user.id,
        user.phoneNumber,
        location,
        time,
  ...
  const sendEmergencyAccessNotification = async (
    phoneNumber: string,
    familyName: string,
  ) => {
    try {
      const success = await notificationService.sendEmergencyAccessNotification(
        phoneNumber,
        familyName,
      );
      if (success) {
  ...
  const sendCriticalReminder = async (user: User) => {
    try {
      ...
      const success = await notificationService.sendCriticalReminder(
        user.id,
        user.phoneNumber,
      );
      if (success) {
  ...
  const getComplianceInfo = () => {
    return {
      dataProtection: t("common:wills.compliance.dataProtection"),
      encryption: t("common:wills.compliance.encryption"),
      retention: t("common:wills.compliance.retention"),
      access: t("common:wills.compliance.access"),
      portability: t("common:wills.compliance.portability"),
    };
  };
  ...
  const getTermsAndConditions = () => {
    return {
      serviceTerms: t("common:wills.terms.serviceTerms"),
      privacyPolicy: t("common:wills.terms.privacyPolicy"),
      dataProcessing: t("common:wills.terms.dataProcessing"),
      userAgreement: t("common:wills.terms.userAgreement"),
      acceptanceRequired: t("common:wills.terms.acceptanceRequired"),
      lastUpdated: t("common:legal.terms.lastUpdated", {
        date: new Date().toLocaleDateString(),
      }),
      effectiveDate: t("common:legal.terms.effectiveDate", {
        date: new Date().toLocaleDateString(),
      }),
    };
  };
  ...
  const getExecutionRequirements = (jurisdiction: string) => {
    return {
      witnessRequirements: t(
        "common:legal.executionRequirements.witnessRequirements",
        { jurisdiction },
      ),
      notaryRequirements: t(
        "common:wills.executionRequirements.notaryRequirements",
      ),
      signingInstructions: t(
        "common:wills.executionRequirements.signingInstructions",
      ),
      storageGuidelines: t(
        "common:wills.executionRequirements.storageGuidelines",
      ),
      legalCompliance: t("common:wills.executionRequirements.legalCompliance"),
    };
  };
