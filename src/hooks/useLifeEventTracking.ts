...
export const useLifeEventTracking = (): UseLifeEventTrackingReturn => {
  ...
  const trackProfileChange = useCallback(
    async (change: Omit<ProfileChange, "timestamp">): Promise<void> => {
      if (!userId) return;
  ...
  const trackExternalIndicator = useCallback(
    async (indicator: ExternalIndicator): Promise<void> => {
      if (!userId) return;
  ...
  const getRecentProfileChanges = async (
    userId: string,
  ...
  const getRecentIndicators = async (
    userId: string,
  ): Promise<ExternalIndicator[]> => {
    try {
      const q = query(
