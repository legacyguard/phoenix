...
export function useAnalytics({
  ...
  useEffect(() => {
    const checkConsent = () => {
      const consentPrefs = localStorage.getItem(
        "legacyguard-consent-preferences",
      );
      if (consentPrefs) {
  ...
  const trackAction = useCallback(
    (
      ...
      const context =
        emotionalContext || inferEmotionalContext(action, timing, retryCount);
  ...
  const trackFeature = useCallback(
    (
      ...
      if (details) {
        trackAction(
          `feature_${feature}_${accepted ? "accepted" : "rejected"}`,
          details,
        );
      }
    },
  ...
  const endTimer = useCallback(
    (
      ...
      trackAction(`${action}_${success ? "completed" : "failed"}`, {
        ...properties,
  ...
  const trackMilestone = useCallback(
    (
      ...
      if (
        milestone === "first_peace_of_mind" ||
        milestone === "family_protection_realized" ||
        milestone === "procrastination_overcome"
      ) {
