...
import {
  detectAcknowledgmentMoments,
  getNextProtectionAreas,
} from "@/services/acknowledgmentTriggers";
...
export const useAcknowledgmentManager = (userProgress: UserProgress) => {
  const [currentAcknowledgment, setCurrentAcknowledgment] =
    useState<AcknowledgmentMoment | null>(null);
  const [acknowledgedTriggers, setAcknowledgedTriggers] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    ...
    const newMoment = moments.find(
      (moment) => !acknowledgedTriggers.has(moment.trigger),
    );
  ...
  const acknowledgeCurrentMoment = () => {
    if (currentAcknowledgment) {
      setAcknowledgedTriggers(
        (prev) => new Set([...prev, currentAcknowledgment.trigger]),
      );
    ...
    return [
      {
        ...
        benefit:
          "Your family can manage finances without searching for information",
      },
      {
      ...
      },
      {
        ...
        benefit:
          "Your children's care continues seamlessly with chosen guardians",
      },
      {
