...
const EXECUTOR_TASK_TEMPLATES: TaskTemplate[] = [
  // Immediate tasks (First 48 hours)
  {
    ...
    priority: 1,
    detailsTemplate:
      "You will need these for banks, government agencies, and insurance companies. The funeral home can usually help you with this. Keep multiple copies as many institutions require originals.",
  },
  {
    ...
    priority: 2,
    detailsTemplate:
      "Call 1-800-772-1213 or visit https://www.ssa.gov to report the death. You'll need the deceased's Social Security number and date of birth.",
  },
  {
    ...
    priority: 3,
    detailsTemplate:
      "The policy is with {{insuranceProvider}}. Policy number: {{policyNumber}}. Contact: {{agentName}} at {{agentPhone}}.",
  ...
  },
  {
    ...
    priority: 4,
    detailsTemplate:
      "Change locks if necessary. Ensure valuable items are secure. The primary residence is at {{primaryAddress}}.",
  },
  {
    ...
    priority: 5,
    detailsTemplate:
      "Contact {{employerName}} HR department at {{employerPhone}}. Request information about final paycheck, benefits, and life insurance.",
  },
  ...
  // First week tasks
  {
    ...
    daysFromNow: 3,
    detailsTemplate:
      "Accounts to notify: {{bankAccounts}}. Bring death certificate and executor documentation.",
  ...
  },
  {
    ...
    daysFromNow: 5,
    detailsTemplate:
      "Contact the probate court in {{county}} County. The will is located in {{willLocation}}.",
  ...
  },
  {
    ...
    daysFromNow: 4,
    detailsTemplate:
      "Cancel or transfer the following cards: {{creditCards}}. This prevents fraudulent charges.",
  },
  {
    ...
    daysFromNow: 7,
    detailsTemplate:
      "Transfer or cancel: {{utilityProviders}}. Digital subscriptions: {{digitalSubscriptions}}.",
  },
  ...
  // Ongoing tasks
  {
    ...
    daysFromNow: 30,
    detailsTemplate:
      "File final income tax return by April 15th. Tax preparer: {{taxPreparer}}. Previous returns located: {{taxDocLocation}}.",
  ...
  },
  {
    ...
    daysFromNow: 60,
    detailsTemplate:
      "Review will and trust documents. Primary beneficiaries: {{beneficiaries}}. Work with estate attorney if needed.",
  ...
  },
  {
    ...
    daysFromNow: 14,
    detailsTemplate:
      "Important accounts: {{digitalAccounts}}. Use legacy contact features where available.",
  },
];
...
export class ExecutorTaskService {
  ...
  static async generateExecutorTasks(
    ...
    try {
      ...
      const { error } = await supabase.from("executor_tasks").insert(tasks);
      ...
      console.log(
        `Generated ${tasks.length} executor tasks for user ${deceasedUserId}`,
      );
    } catch (error) {
   ...
   */
  private static async fetchUserDocuments(
    userId: string,
  ...
  private static async personalizeTaskDetails(
    ...
    if (template.requiresDocuments?.includes("life_insurance")) {
      const insuranceDoc = documents.find(
        (d) => d.category === "insurance" && d.metadata?.type === "life",
      );
      if (insuranceDoc?.extracted_data) {
        details = details
          .replace(
            "{{insuranceProvider}}",
            insuranceDoc.extracted_data.provider || "[Insurance Provider]",
          )
          .replace(
            "{{policyNumber}}",
            insuranceDoc.extracted_data.policy_number || "[Policy Number]",
          )
          .replace(
            "{{agentName}}",
            insuranceDoc.extracted_data.agent_name || "[Agent Name]",
          )
          .replace(
            "{{agentPhone}}",
            insuranceDoc.extracted_data.agent_phone || "[Agent Phone]",
          );
      }
    }
    ...
    details = details
      .replace(
        "{{primaryAddress}}",
        userData.primary_address || "[Primary Address]",
      )
      .replace("{{employerName}}", userData.employer_name || "[Employer]")
      .replace(
        "{{employerPhone}}",
        userData.employer_phone || "[Employer Phone]",
      )
    ...
    if (details.includes("{{bankAccounts}}")) {
      const bankDocs = documents.filter(
        (d) =>
          d.category === "financial" && d.metadata?.type === "bank_statement",
      );
      const banks = [
        ...new Set(
          bankDocs.map((d) => d.extracted_data?.bank_name).filter(Boolean),
        ),
      ];
      details = details.replace(
        "{{bankAccounts}}",
        banks.join(", ") || "[Bank Accounts]",
      );
    }

    if (details.includes("{{beneficiaries}}")) {
      const willDoc = documents.find(
        (d) => d.category === "legal" && d.metadata?.type === "will",
      );
      const beneficiaries =
        willDoc?.extracted_data?.beneficiaries?.join(", ") || "[Beneficiaries]";
        ...
        switch (docType) {
          ...
          case "bank_statements":
            return (
              ...
              d.metadata?.type === "bank_statement"
            );
          case "tax_returns":
            return (
              d.category === "financial" && d.metadata?.type === "tax_return"
            );
          default:
            return false;
        }
