...
export class FamilySituationAnalysisService {
  ...
  static analyzeFamilySituation(
    ...
    const recommendedFeatures = this.recommendFeatures(
      structure,
      specialNeeds,
      familyData,
    );
    const sensitivities = this.identifySensitivities(
      familyData,
      structure,
      culturalContext,
    );
  ...
  private static assessComplexity(
    ...
    const dependentCount = familyData.members.filter(
      (m) => m.isDependent,
    ).length;
    if (dependentCount > 3) {
      complexityScore += 1;
  ...
  private static identifyCulturalConsiderations(
    ...
    if (
      familyData.members.some(
        (m) => m.culturalRole === "elder" && culturalContext.primaryCulture,
      )
    ) {
  ...
  private static identifySensitivities(
    ...
    if (
      culturalContext.familyHierarchy &&
      culturalContext.familyHierarchy !== "egalitarian"
    ) {
  ...
  private static hasMultipleGenerations(members: FamilyMember[]): boolean {
    ...
    const hasGrandparents = relationships.some((r) =>
      r.includes("grandparent"),
    );
  ...
  static generateFamilyInsights(
    ...
    if (situation.complexity === "simple") {
      insights.push({
        ...
        description:
          "Your family situation allows for straightforward planning. We can help you protect your family efficiently.",
        priority: "medium",
        actionable: true,
        suggestedActions: [
          "Use quick setup wizard",
          "Complete basic protection plan",
        ],
      });
    }
   ...
   */
  private static getStructureInsightTitle(
    structure: FamilySituation["structure"],
  ): string {
    const titles = {
  ...
  private static getStructureInsightDescription(
    structure: FamilySituation["structure"],
  ): string {
    const descriptions = {
      nuclear:
        "Your traditional family structure benefits from standard protection planning with focus on children and spouse.",
      single_parent:
        "As a single parent, creating a strong support network and detailed guardian plans is essential.",
      blended:
        "Blended families need extra care in beneficiary designations and clear communication about intentions.",
      multigenerational:
        "Planning across generations requires balancing different needs and cultural expectations.",
      childless:
        "Focus on protecting each other and considering legacy beyond immediate family.",
      empty_nest:
        "With children grown, it's time to update plans for this new life phase.",
      non_traditional:
        "Your unique family structure deserves customized planning that fits your specific needs.",
    };
    return descriptions[structure];
  }

  private static getStructureActions(
    structure: FamilySituation["structure"],
  ): string[] {
    const actions = {
      nuclear: ["Create will", "Name guardians", "Set up life insurance"],
      single_parent: [
        "Build guardian network",
        "Create detailed care instructions",
        "Establish emergency fund",
      ],
      blended: [
        "Clarify beneficiaries",
        "Address step-relationships",
        "Create family communication plan",
      ],
      multigenerational: [
        "Balance generation needs",
        "Plan elder care",
        "Set up family meetings",
      ],
      childless: [
        "Focus on spouse protection",
  ...
  private static getSpecialNeedDescription(need: string): string {
    const descriptions: Record<string, string> = {
      special_needs_planning:
        "Protecting a family member with special needs requires specialized trusts and detailed care planning.",
      elder_care_planning:
        "Caring for aging parents involves financial planning and healthcare directives.",
      military_benefit_planning:
        "Military families have unique benefits and planning considerations during service.",
      business_succession:
        "Your business needs succession planning to protect your family and employees.",
      young_child_guardianship:
        "Young children need carefully chosen guardians and detailed care instructions.",
    };
    return (
      descriptions[need] || "This situation requires specialized planning."
    );
  }
  ...
  private static getSpecialNeedActions(need: string): string[] {
    const actions: Record<string, string[]> = {
      ...
      elder_care_planning: [
        "Create elder care fund",
        "Set up healthcare proxies",
        "Discuss wishes with parents",
      ],
      military_benefit_planning: [
  ...
  private static getSensitivityDescription(sensitivity: string): string {
    const descriptions: Record<string, string> = {
      step_family_dynamics:
        "Navigate step-family relationships with clear communication and fair planning.",
      family_conflict:
        "Low family harmony requires extra clarity and potentially neutral third parties.",
      disability_planning_stress:
        "Planning for disabilities can be emotionally challenging but is critically important.",
      traditional_role_expectations:
        "Balance cultural traditions with legal requirements and personal wishes.",
    };
    return (
      descriptions[sensitivity] || "This area requires careful consideration."
    );
  }
