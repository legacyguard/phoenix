...
export async function generateNudgesForUser(userId: string): Promise<void> {
  ...
  try {
    ...
    const [assets, guardians, wills] = await Promise.all([
      supabase
        .from("assets")
        .select("type, estimated_value")
        .eq("user_id", userId),
      supabase
        .from("guardians")
        .select("full_name, invitation_status")
        .eq("user_id", userId),
    ...
    const hasLifeInsurance = assets.data.some(
      (a: { type: string }) => a.type === "life_insurance",
    );
    if (!hasLifeInsurance) {
      await createNotification(userId, {
        type: "nudge_social_proof",
        message:
          "Did you know? 85% of fathers in their 40s have life insurance to protect their family's financial future.",
        urgency: "low",
      });
    }
    ...
    const totalAssetValue = assets.data.reduce(
      (sum: number, a: { estimated_value?: number }) =>
    ...
    if (totalAssetValue > 50000 && !hasWill) {
      await createNotification(userId, {
        ...
        message:
          "Without clear instructions, a significant portion of your assets could be lost to legal fees or taxes. Secure your legacy by outlining your wishes.",
        urgency: "medium",
      });
    }

    // Nudge 3: Progress Momentum
    const preparednessScore = calculateOverallPreparedness({
      assets: assets.data,
    ...
    const unpreparedGuardians = guardians.data.filter(
      (g: { invitation_status?: string; full_name: string }) =>
        g.invitation_status !== "accepted",
    );
...
async function createNotification(
  userId: string,
  {
    ...
    message,
    urgency,
  }: { type: string; message: string; urgency: "low" | "medium" | "high" },
): Promise<void> {
  try {
    ...
    if (typeof window !== "undefined") {
      ...
      if (userNotifications.length > 0) {
        ...
        const lastNotificationDate = new Date(
          lastNotification.timestamp as string,
        );
        const dateDiff =
          (new Date().getTime() - lastNotificationDate.getTime()) /
          (1000 * 60 * 60 * 24);
...
function calculateOverallPreparedness(userData: {
  assets?: unknown[];
  guardians?: unknown[];
  wills?: unknown[];
}): number {
  ...
  if (
    userData.assets &&
    Array.isArray(userData.assets) &&
    userData.assets.length > 0
  )
    score += 30;
  if (
    userData.guardians &&
    Array.isArray(userData.guardians) &&
    userData.guardians.length > 0
  )
    score += 30;
  if (
    userData.wills &&
    Array.isArray(userData.wills) &&
    userData.wills.length > 0
  )
    score += 40;
