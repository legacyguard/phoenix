  ...
  if (lifeAnswers.familyStructure) {
    switch (lifeAnswers.familyStructure) {
      case 'young_children':
        ...
        specificNeeds.push('guardianship', 'education_planning', 'life_insurance');
        break;
      case 'adult_children':
        familyType = 'empty_nesters';
        specificNeeds.push('inheritance_planning', 'healthcare_directives', 'legacy_planning');
        break;
      case 'blended_family':
        familyType = 'blended_family';
        specificNeeds.push('clear_inheritance', 'conflict_prevention', 'explicit_documentation');
        break;
      case 'extended_care':
        familyType = lifeAnswers.primaryResponsibility === 'aging_parents' ? 'empty_nesters' : 'young_family';
        specificNeeds.push('care_instructions', 'special_needs_planning', 'medical_documentation');
        break;
    }
  }
  ...
  if (lifeAnswers.financialComplexity) {
    switch (lifeAnswers.financialComplexity) {
      ...
      case 'business_owner':
        ...
        specificNeeds.push('business_succession', 'key_employee_planning', 'buy_sell_agreements');
        break;
      case 'complex_estate':
        assetComplexity = 'complex';
        specificNeeds.push('trust_planning', 'tax_optimization', 'professional_advisors');
        break;
    }
  }
...
export const generatePersonalizedDashboard = (profile: UserProfile): DashboardConfig => {
  ...
  if (profile.familyType === 'young_family') {
    assistantMessages.push({
      type: 'welcome',
      content: "Since you have young children, let's start with the most important protections for them. We'll focus on guardianship and making sure they're taken care of no matter what.",
      contextual: true
    });
  } else if (profile.familyType === 'empty_nesters') {
    assistantMessages.push({
      type: 'welcome',
      content: "Let's focus on organizing your accumulated assets and healthcare wishes. You've built a lot over the years - now let's make sure it's protected.",
      contextual: true
    });
  } else if (profile.familyType === 'blended_family') {
    assistantMessages.push({
      type: 'welcome',
      content: "With a blended family, clear documentation is essential. We'll help you create explicit plans that protect everyone and prevent misunderstandings.",
      contextual: true
    });
  }

  // Generate priority tasks based on profile
  if (profile.familyType === 'young_family' || profile.specificNeeds.includes('guardianship')) {
    priorityTasks.push({
      id: 'guardian-selection',
      title: 'Choose and Document Guardians',
      description: 'Select trusted people to care for your children and document your wishes clearly.',
      estimatedTime: '30 minutes',
      impact: 'high',
      category: 'Family Protection',
      link: '/guardians',
      personalizedReason: 'Critical for protecting your young children if something happens to you.'
    });
  }

  if (profile.assetComplexity === 'complex' || profile.specificNeeds.includes('business_succession')) {
    priorityTasks.push({
      id: 'business-succession',
      title: 'Create Business Succession Plan',
      description: 'Document how your business should be managed or transferred to protect your family\'s income.',
      estimatedTime: '2 hours',
      impact: 'high',
      category: 'Business Protection',
      link: '/business-planning',
      personalizedReason: 'Your business is a major family asset that needs proper succession planning.'
    });
  }
  ...
  if (profile.emotionalState === 'overwhelmed') {
    assistantMessages.push({
      type: 'encouragement',
      content: "I know this feels like a lot. Let's take it one small step at a time. You don't have to do everything today.",
      contextual: true
    });
    ...
    quickWins.push({
      ...
      link: '/contacts/add',
      description: 'Start small - add just one person your family should contact.'
    });
  }
  ...
  if (profile.emotionalState === 'anxious') {
    assistantMessages.push({
      type: 'guidance',
      content: "We'll address your concerns step by step. Every task we've created directly addresses what's worrying you.",
      contextual: true
    });
  }
  ...
  if (profile.urgencyLevel === 'high') {
    assistantMessages.push({
      type: 'warning',
      content: "Since time is critical, we've prioritized the most important tasks that will have immediate impact.",
      contextual: true
    });
  ...
  if (profile.primaryConcerns.includes('family_confusion')) {
    personalizedInsights.push({
      ...
      description: 'Every step we take will include clear instructions your family can follow easily.',
      icon: 'Users',
      color: 'blue'
    });
  }
  ...
  if (profile.primaryConcerns.includes('business_continuity')) {
    personalizedInsights.push({
      ...
      description: 'We\'ll ensure your business continues to support your family.',
      icon: 'Briefcase',
      color: 'purple'
    });
  }
  ...
  if (profile.familyType === 'young_family') {
    relevantSections.push({
      ...
      priority: 2,
      prefilledData: {
        suggestedGuardians: ['Consider siblings', 'Close family friends', 'Grandparents'],
  ...
  if (profile.assetComplexity !== 'simple') {
    relevantSections.push({
      ...
      priority: 3,
      prefilledData: {
        assetCategories: profile.assetComplexity === 'complex' 
          ? ['Real Estate', 'Business Interests', 'Investments', 'Retirement Accounts']
...
export const generateTaskRecommendations = (
  ...
  if (profile.familyType === 'young_family' && !existingTasks.find(t => t.id === 'life-insurance')) {
    recommendations.push({
      ...
      personalizedReason: 'Young children need financial protection until they can support themselves.'
    });
  }
  
  // Complexity-specific recommendations
  if (profile.assetComplexity === 'complex' && !existingTasks.find(t => t.id === 'advisor-team')) {
    recommendations.push({
      id: 'advisor-team',
      title: 'Document Your Professional Advisors',
      description: 'List all attorneys, accountants, and advisors your family should contact.',
      estimatedTime: '20 minutes',
      impact: 'medium',
      category: 'Professional Network',
      link: '/advisors',
      personalizedReason: 'Complex estates need professional guidance - make sure your family knows who to call.'
    });
  }
...
export const scoreTaskPriority = (
  ...
  if (task.category === 'guardianship' && profile.familyType === 'young_family') {
    score += 100;
  }
