...
export const generateSmartSuggestions = (
  ...
  if (userProfile.hasYoungChildren && !currentProgress.hasGuardianship) {
    suggestions.push({
      ...
      reasoning: 'This ensures your children will be cared for by people you trust',
      familyBenefit: 'Your children will be protected by guardians you\'ve chosen, not appointed by a court',
  ...
  if (userProfile.hasYoungChildren && !currentProgress.hasLifeInsurance) {
    suggestions.push({
      ...
      reasoning: 'Life insurance provides crucial financial protection for your children',
      familyBenefit: 'Your children\'s education and living expenses will be covered',
  ...
  if (userProfile.hasAgingParents && !currentProgress.hasHealthcareDirective) {
    suggestions.push({
      ...
      reasoning: 'Clear medical preferences prevent difficult decisions during emergencies',
      familyBenefit: 'Your family will know your medical wishes without guessing',
      timeEstimate: '20-30 minutes',
      action: () => console.log('Navigate to healthcare directives')
    });
  }
  
  if (userProfile.isBusinessOwner && !currentProgress.completedAreas.includes('business')) {
    suggestions.push({
      ...
      reasoning: 'Your business needs continuity planning to protect employees and value',
      familyBenefit: 'Your family will know how to handle your business interests',
...
const generateQuickWinSuggestions = (progress: UserProgress): SmartSuggestion[] => {
  ...
  if (progress.documentsCount === 0) {
    quickWins.push({
      ...
      reasoning: 'This is the most essential financial information your family needs',
...
const generateAdvancedSuggestions = (progress: UserProgress): SmartSuggestion[] => {
  const advanced: SmartSuggestion[] = [];
  
  if (progress.hasWill && !progress.completedAreas.includes('personal_messages')) {
    advanced.push({
      ...
      reasoning: 'Personal messages provide comfort and guidance beyond legal documents',
      familyBenefit: 'Your family will have your personal thoughts and love to comfort them',
      timeEstimate: '20-30 minutes',
  ...
  if (progress.documentsCount > 10) {
    advanced.push({
      ...
      reasoning: 'Well-organized documents save crucial time during emergencies',
      familyBenefit: 'Your family can find exactly what they need when they need it',
...
const generateTimeBasedSuggestions = (progress: UserProgress): SmartSuggestion[] => {
  const timeBased: SmartSuggestion[] = [];
  const currentMonth = new Date().getMonth();
  const daysSinceLastActivity = Math.floor((Date.now() - progress.lastActivityDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  if (currentMonth >= 0 && currentMonth <= 3) {
    timeBased.push({
      ...
      familyBenefit: 'Your family will have all financial information organized and accessible',
      timeEstimate: '30-45 minutes',
...
export const categorizeSuggestionsByTime = (suggestions: SmartSuggestion[]): {
