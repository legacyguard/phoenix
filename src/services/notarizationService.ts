...
export class NotarizationService {
  ...
  async findNotaries(
    ...
    try {
      ...
      await supabase.from("notary_searches").insert({
  ...
  private async archiveNotarizedWill(willId: string): Promise<void> {
    try {
      ...
      await supabase.from("archived_wills").insert({
        will_id: willId,
  ...
  private async sendNotarizationConfirmation(
    appointmentId: string,
  ): Promise<void> {
    // Email notification implementation
    console.log(
      `Sending notarization confirmation for appointment ${appointmentId}`,
    );
  }
}
