...
class SuggestionLearningService {
  ...
  private updatePatterns(feedback: SuggestionFeedback): void {
    ...
    const relevantFeedback = this.feedbackHistory.filter(
      (f) =>
        f.suggestionType === feedback.suggestionType &&
        f.userProfile.familyComposition ===
          feedback.userProfile.familyComposition,
    );

    const acceptedCount = relevantFeedback.filter(
      (f) => f.response === "accepted",
    ).length;
    pattern.successRate = acceptedCount / relevantFeedback.length;

    // Update average time to accept
    const acceptedFeedback = relevantFeedback.filter(
      (f) => f.response === "accepted",
    );
    if (acceptedFeedback.length > 0) {
      const totalTime = acceptedFeedback.reduce(
        (sum, f) => sum + f.timeToResponse,
        0,
      );
  ...
  rankSuggestions<
    T extends { type: string; priority: "high" | "medium" | "low" },
  >(suggestions: T[], userProfile: UserProfile): T[] {
    return suggestions.sort((a, b) => {
      const aPattern = this.patterns.get(
        `${a.type}_${userProfile.familyComposition}`,
      );
      const bPattern = this.patterns.get(
        `${b.type}_${userProfile.familyComposition}`,
      );
  ...
  getSimilarUserPatterns(userProfile: UserProfile): SuggestionPattern[] {
    ...
    this.patterns.forEach((pattern) => {
      ...
      const isSimilar =
        this.calculateProfileSimilarity(userProfile, pattern.userProfileMatch) >
        0.7;
