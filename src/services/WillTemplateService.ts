...
export class WillTemplateService {
   ...
   */
  static async getTemplate(countryCode: string, languageCode?: string): Promise<WillTemplate | null> {
    try {
      let query = supabase
        .from('will_templates')
        .select('*')
   ...
   */
  static async createTemplate(templateData: WillTemplateFormData): Promise<WillTemplate | null> {
    try {
   ...
   */
  static async updateTemplate(templateId: string, updates: Partial<WillTemplateFormData>): Promise<WillTemplate | null> {
    try {
      const { data: userData } = await supabase.auth.getUser();
   ...
   */
  static async getAvailableCountries(): Promise<Array<{ code: string; name: string; languages: string[] }>> {
    try {
      const { data, error } = await supabase
        .from('will_templates')
      ...
      const countries = Array.from(countriesMap.entries()).map(([code, languages]) => ({
        code,
  ...
  static fillTemplate(template: WillTemplate, userData: Record<string, unknown>): string {
    ...
    filledTemplate = filledTemplate.replace(eachRegex, (match, arrayName, content) => {
