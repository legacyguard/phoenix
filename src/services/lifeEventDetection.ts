...
export class LifeEventDetectionService {
  ...
  static async detectLifeEvents(
    ...
    const marriageEvent = this.detectMarriageOrDivorce(
      userActivity,
      profileChanges,
      externalIndicators,
    );
    if (
      marriageEvent &&
      marriageEvent.confidence >= this.MIN_CONFIDENCE_THRESHOLD
    ) {
      detectedEvents.push({ ...marriageEvent, userId });
    }

    // Check for new child/birth
    const birthEvent = this.detectNewChild(
      userActivity,
      profileChanges,
      externalIndicators,
    );
    ...
    const deathEvent = this.detectDeathInFamily(
      userActivity,
      profileChanges,
      externalIndicators,
    );
    ...
    const jobEvent = this.detectJobChange(
      userActivity,
      profileChanges,
      externalIndicators,
    );
    ...
    const moveEvent = this.detectRelocation(
      userActivity,
      profileChanges,
      externalIndicators,
    );
    ...
    const retirementEvent = this.detectRetirement(
      userActivity,
      profileChanges,
      externalIndicators,
    );
    if (
      retirementEvent &&
      retirementEvent.confidence >= this.MIN_CONFIDENCE_THRESHOLD
    ) {
      detectedEvents.push({ ...retirementEvent, userId });
    }

    // Check for major purchases
    const purchaseEvent = this.detectMajorPurchase(
      userActivity,
      profileChanges,
      externalIndicators,
    );
    if (
      purchaseEvent &&
      purchaseEvent.confidence >= this.MIN_CONFIDENCE_THRESHOLD
    ) {
  ...
  private static detectMarriageOrDivorce(
    ...
    const nameChange = profileChanges.find(
      (change) =>
        change.field === "lastName" && this.isRecent(change.timestamp, 90),
    );
    if (nameChange) {
    ...
    const beneficiaryUpdates = activities.filter(
      (activity) =>
    ...
    const spouseUpdate = activities.find(
      (activity) =>
    ...
    const marriageIndicator = external.find(
      (ind) =>
        ind.type === "marriage_certificate" || ind.type === "divorce_decree",
    );
  ...
  private static detectNewChild(
    ...
    const childAddition = activities.find(
      (activity) =>
        activity.type === "guardian_update" &&
        (activity.details.relationship === "child" ||
          activity.details.relationship === "dependent") &&
    ...
    const guardianshipUpdate = activities.filter(
      (activity) =>
    ...
    // Check external indicators
    const birthCertificate = external.find(
      (ind) =>
  ...
  private static detectDeathInFamily(
    ...
    const familyRemoval = activities.find(
      (activity) =>
        activity.type === "guardian_update" &&
    ...
    const beneficiaryRemovals = activities.filter(
      (activity) =>
        activity.type === "beneficiary_update" &&
        activity.details.action === "remove" &&
        this.isRecent(activity.timestamp, 30),
    ...
    // Check external indicators
    const deathCertificate = external.find(
      (ind) => ind.type === "death_certificate",
    );
    if (deathCertificate) {
  ...
  private static detectJobChange(
    ...
    const employmentChange = profileChanges.find(
      (change) =>
    ...
    const incomeChange = profileChanges.find(
      (change) =>
    ...
    const retirementUpdate = activities.filter(
      (activity) =>
  ...
  private static detectRelocation(
    ...
    const addressChange = profileChanges.find(
      (change) =>
    ...
    const contactUpdates = activities.filter(
      (activity) =>
        activity.type === "guardian_update" &&
    ...
    const propertyChange = activities.find(
      (activity) =>
        activity.type === "asset_change" &&
  ...
  private static detectRetirement(
    ...
    const employmentStatus = profileChanges.find(
      (change) =>
    ...
    const retirementActivity = activities.filter(
      (activity) =>
        activity.type === "asset_change" &&
        activity.details.assetType === "retirement_account" &&
    ...
    const ageCheck = profileChanges.find(
      (change) => change.field === "age" && change.newValue >= 62,
    );
    if (ageCheck) {
  ...
  private static detectMajorPurchase(
    ...
    const majorAsset = activities.find(
      (activity) =>
        activity.type === "asset_change" &&
        activity.details.action === "add" &&
        (activity.details.value > 50000 ||
          activity.details.assetType === "real_estate") &&
        this.isRecent(activity.timestamp, 90),
    );
    if (majorAsset) {
    ...
    const loanDoc = activities.find(
      (activity) =>
        activity.type === "document_upload" &&
        (activity.details.documentType === "mortgage" ||
          activity.details.documentType === "loan") &&
        this.isRecent(activity.timestamp, 60),
    );
    if (loanDoc) {
