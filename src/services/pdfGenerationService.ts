...
export class PDFGenerationService {
  generateWillPDF(
    ...
    addText(
      identityText +
        (requirements.legal_language.soundMind
          ? `, ${requirements.legal_language.soundMind}`
          : ""),
      12,
    );
    ...
    willContent.beneficiaries.forEach((beneficiary, index) => {
      ...
      addText(
        `${index + 1}. ${beneficiary.name} (${beneficiary.relationship})${beneficiary.identification ? ` - ${beneficiary.identification}` : ""}`,
        12,
      );
    ...
    if (willContent.executor) {
      ...
      if (willContent.executor.alternativeExecutor) {
        ...
        addText(
          `${willContent.executor.alternativeExecutor.name} (${willContent.executor.alternativeExecutor.relationship})`,
          11,
        );
      }
    }
    ...
    // Date and place
    doc.text(
      `${requirements.legal_language.date}: ______________`,
      margin,
      yPosition,
    );
    ...
    if (requirements.witness_count > 0) {
      ...
      for (let i = 0; i < requirements.witness_count; i++) {
        ...
        doc.text(
          `${requirements.legal_language.witness} ${i + 1}:`,
          xPos,
          baseY,
        );
    ...
    if (includeSignature && signatureData) {
      ...
      const svgBase64 =
        ...
        btoa(unescape(encodeURIComponent(signatureSVG)));
      ...
      try {
        ...
        addText(
          `Signer: ${signatureData.signerName || willContent.testator.name}`,
          10,
          "center",
        );
        addText(`Date: ${new Date().toLocaleDateString()}`, 10, "center");
        addText(
          `Document ID: ${signatureData.documentId?.substring(0, 8) || "N/A"}`,
          10,
          "center",
        );
      }
      ...
      // Add legal text
      addText(
        "This document has been digitally signed and authenticated by the LegacyGuard platform.",
        10,
        "center",
      );
      addText(
        "The digital signature confirms the identity of the signer and the integrity of the document.",
        10,
        "center",
      );
    }
