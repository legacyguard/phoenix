...
export const createAssetFormSchema = (t: (key: string, params?: any) => string) => z.object({
  ...
  property_registry_number: z.string()
    .max(100, { message: t("validation.errors.registryNumberMaxLength", { max: 100 }) })
    .optional(),
    
  // Specific fields for financial accounts
  account_type: z.string().optional(),
  financial_institution: z.string()
    .max(200, { message: t("validation.errors.institutionNameMaxLength", { max: 200 }) })
    .optional(),
  account_number: z.string()
    .max(50, { message: t("validation.errors.accountNumberMaxLength", { max: 50 }) })
    .optional(),
  login_credentials: z.string()
    .max(1000, { message: t("validation.errors.credentialsMaxLength", { max: 1000 }) })
    .optional()
});
