...
export const AssistantProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [emotionalState, setEmotionalState] =
    useState<EmotionalState>("starting");
    ...
    (action: string, area?: string) => {
      setUserProgress((prev) => {
        const updated = {
          ...
          completedAreas:
            area && !prev.completedAreas.includes(area)
  ...
  const getPersonalizedMessage = useCallback((): AssistantMessage => {
    ...
    if (totalActions === 0) {
      return {
        type: "welcome",
        content: t("common:welcome.firstTime"),
        icon: Heart,
      };
    }
    ...
    if (lastActivity) {
      ...
      if (daysSince > 14) {
        return {
          type: "welcome",
          content: t("common:welcome.afterBreak"),
          icon: Heart,
        };
      }
    }
    ...
    switch (currentContext) {
      case "dashboard":
        if (totalActions < 3) {
          return {
            type: "guidance",
            content: t("common:contextual.dashboard.empty"),
            icon: Home,
          };
        } else if (userProgress.completedAreas.length >= 2) {
          return {
            type: "encouragement",
            content: t("common:contextual.dashboard.progress"),
            icon: Heart,
          };
        }
        break;
      ...
      case "assets":
        if (assetsCount === 0) {
          return {
            type: "guidance",
            content: t("common:contextual.assets.firstAsset"),
            icon: Briefcase,
          };
        } else if (assetsCount < 3) {
          return {
            type: "encouragement",
            content: t("common:contextual.assets.addingMore"),
            icon: Briefcase,
          };
        }
        break;
      ...
      case "family":
        if (trustedPeopleCount === 0) {
          return {
            type: "guidance",
            content: t("common:contextual.family.noTrustedPeople"),
            icon: Users,
          };
        } else {
          return {
            type: "encouragement",
            content: t("common:contextual.family.addingPeople"),
            icon: Heart,
          };
        }
        break;
      ...
      case "documents":
        if (documentsCount === 0) {
          return {
            type: "guidance",
            content: t("common:contextual.documents.firstUpload"),
            icon: FileText,
          };
        }
        break;
    ...
    if (emotionalState === "overwhelmed") {
      return {
        type: "support",
        content: t("common:emotional.overwhelmed.message"),
        icon: Coffee,
        actionSuggestion: {
          text: t("common:actions.supportive.takeBreak"),
          action: () => {},
          priority: "high",
        },
      };
    ...
    return {
      type: "encouragement",
      content: t("common:welcome.returning"),
      icon: Heart,
    };
  ...
  const getSuggestions = useCallback((): Recommendation[] => {
    ...
    const { assetsCount, documentsCount, trustedPeopleCount, willCompleted } =
      userProgress;
    ...
    if (trustedPeopleCount === 0) {
      suggestions.push({
        ...
        description: t("common:suggestions.smart.basedOnAnswers"),
        timeEstimate: t("common:guidance.timeEstimates.quick"),
        priority: "high",
        familyBenefit: t("common:guidance.familyBenefit.immediate"),
    ...
    if (documentsCount === 0) {
      suggestions.push({
        ...
        timeEstimate: t("common:guidance.timeEstimates.moderate"),
        priority: "high",
        familyBenefit: t("common:guidance.familyBenefit.immediate"),
    ...
    if (assetsCount < 3) {
      suggestions.push({
        ...
        familyBenefit: t("common:guidance.familyBenefit.planning"),
    ...
    if (!willCompleted && userProgress.totalActions > 5) {
      suggestions.push({
        ...
        timeEstimate: t("common:guidance.timeEstimates.involved"),
        priority: "medium",
        familyBenefit: t("common:guidance.familyBenefit.peace"),
