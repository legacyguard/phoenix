...
export function AIDemo() {
  ...
  const [currentQuestion, setCurrentQuestion] = React.useState<Record<
    string,
    unknown
  > | null>(null);
  const [currentSuggestion, setCurrentSuggestion] = React.useState<Record<
    string,
    unknown
  > | null>(null);
            ...
            <CardContent className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-muted-foreground">
                  {t("demo.ai.profile.label")}
                </p>
                <p className="font-medium">
                  {demoProfile.name}, {demoProfile.age} years old
                </p>
                <p className="text-sm">
                  {demoProfile.documentsCount}{" "}
                  {t("demo.ai.profile.documentsCount")}
                </p>
              </div>
                ...
                <div className="space-y-4 mt-4">
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="capitalize">
                      {t("demo.ai.question.category")}{" "}
                      {currentQuestion.category}
                    </span>
                    <span>
                      {t("demo.ai.question.time", {
                        minutes: currentQuestion.estimatedTimeMinutes,
                      })}
                    </span>
                  </div>
        ...
        <TabsContent value="suggestions" className="mt-6">
          <Card className="max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-muted-foreground">
                  {t("demo.ai.context.label")}
                </p>
                <p className="text-sm">
                  {t("demo.ai.context.completion")}{" "}
                  {demoContext.completionPercentage}%
                </p>
                <p className="text-sm">
                  {t("demo.ai.context.recent")}{" "}
                  {demoContext.recentActions.join(", ")}
                </p>
                <p className="text-sm">
                  {t("demo.ai.context.mood")} {demoContext.mood}
                </p>
              </div>
              ...
              {currentSuggestion && (
                <div className="space-y-4 mt-4">
                  ...
                  <div className="flex justify-between text-sm">
                    <span>
                      {t("demo.ai.suggestion.priority")}{" "}
                      <span className="font-medium">
                        {currentSuggestion.priority}
                      </span>
                    </span>
                    <span>
                      {t("demo.ai.suggestion.timeNeeded", {
                        time: currentSuggestion.estimatedTime,
                      })}
                    </span>
                  </div>
                </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <Button
                  variant="outline"
                  onClick={() =>
                    handleGenerateMessage("User needs motivation", "gentle")
                  }
                >
