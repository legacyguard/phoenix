...
export const InviteAcceptance: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadInvitationData = async () => {
      if (!inviteToken) {
        if (mounted) {
          toast.error(t("common:inviteAcceptance.errors.invalidLink"));
          navigate("/");
        }
        return;
      }
      ...
      try {
        ...
        if (error || !data) {
          toast.error(t("common:inviteAcceptance.errors.notFound"));
          navigate("/");
          return;
        }

        if (data.invitation_status !== "sent") {
          toast.error(t("common:inviteAcceptance.errors.alreadyProcessed"));
          navigate("/");
          return;
        }
        ...
        setInviterName(t("common:inviteAcceptance.defaultInviterName"));
        ...
        let userMessage = t("common:inviteAcceptance.errors.loadingFailed");
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      toast.success(t("common:inviteAcceptance.messages.accepted"));
      ...
      // User-friendly message
      let userMessage = t("common:inviteAcceptance.errors.acceptFailed");
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      toast.success(t("common:inviteAcceptance.messages.declined"));
      navigate("/");
    } catch (error: Record<string, unknown>) {
      const timestamp = new Date().toISOString();
      const errorMessage = error?.message || t("errors:errors.unknown");
      ...
      // User-friendly message
      let userMessage = t("common:inviteAcceptance.errors.declineFailed");
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            ...
            >
              ...
              {processing
                ? t("common:inviteAcceptance.buttons.processing")
                : t("common:inviteAcceptance.buttons.acceptRole")}
            </Button>
            <Button
              variant="outline"
