...
export const Manual: React.FC = () => {
  ...
  const loadContacts = async () => {
    try {
      ...
      const errorMessage = error?.message || t("help:help.errors.unknown");
      ...
      if (error?.code === "PGRST116") {
        ...
        userMessage = t("help:help.errors.network");
  ...
  const handleSaveContact = async () => {
    ...
    try {
      ...
      if (editingContact) {
        ...
        toast.success(t("help:help.contacts.messages.updated"));
      } else {
        const { error } = await supabaseWithRetry
          .from("contacts")
          .insert([contactData]);
        ...
        if (error) throw error;
        toast.success(t("help:help.contacts.messages.added"));
      }
  ...
  const handleDeleteContact = async (id: string) => {
    try {
      ...
      toast.success(t("help:help.contacts.messages.deleted"));
    } catch (error: Record<string, unknown>) {
      const timestamp = new Date().toISOString();
      const errorMessage = error?.message || t("help:help.errors.unknown");
      const errorCode = error?.code || "UNKNOWN_ERROR";
  ...
  const loadKeyDocuments = async () => {
    try {
      ...
      // Detailné logovanie pre debugging
      console.error(
        "[Manuál a kontakty] Chyba pri načítanie kľúčových dokumentov:",
        {
          timestamp,
  ...
  const getExpirationStatus = (expirationDate: string | null) => {
    ...
    if (diffDays < 0) {
      return {
        text: t("help:help.documents.expiration.expired"),
        variant: "destructive" as const,
      };
    } else if (diffDays <= 30) {
      return {
        text: t("help.documents.expiration.expiresSoon"),
        variant: "destructive" as const,
      };
    } else if (diffDays <= 90) {
      return {
        text: t("help:help.documents.expiration.expiring"),
    ...
    <ErrorBoundary>
      <div className="container mx-auto px-4 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">{t("help:help.title")}</h1>
            <p className="text-muted-foreground">{t("help:help.subtitle")}</p>
          </div>
        </div>
        ...
        <Tabs defaultValue="contacts" className="space-y-6">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="contacts">
              {t("help:help.tabs.contacts")}
            </TabsTrigger>
            <TabsTrigger value="documents">
              {t("help:help.tabs.documents")}
            </TabsTrigger>
            <TabsTrigger value="instructions">
              {t("help:help.tabs.instructions")}
            </TabsTrigger>
          </TabsList>
            ...
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl font-semibold">
                  {t("help:help.contacts.title")}
                </h2>
                <p className="text-muted-foreground">
                  {t("help:help.contacts.description")}
                </p>
              </div>
              <Sheet
              ...
              >
                ...
                <SheetContent>
                  <SheetHeader>
                    <SheetTitle>
                      {editingContact
                        ? t("help.contacts.form.editTitle")
                        : t("help.contacts.form.addTitle")}
                    </SheetTitle>
                    <SheetDescription>
                      {t("help:help.contacts.form.description")}
                    </SheetDescription>
                  </SheetHeader>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <Label htmlFor="contactName">
                        {t("help.contacts.form.nameLabel")}
                      </Label>
                      <Input
                        id="contactName"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="role">
                        {t("help.contacts.form.roleLabel")}
                      </Label>
                      <Input
                        id="role"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="phone">
                        {t("help.contacts.form.phoneLabel")}
                      </Label>
                      <Input
                        id="phone"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="email">
                        {t("help.contacts.form.emailLabel")}
                      </Label>
                      <Input
                        id="email"
                        type="email"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="notes">
                        {t("help.contacts.form.notesLabel")}
                      </Label>
                      <Textarea
                        id="notes"
              ...
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <h3 className="text-lg font-semibold mb-2">
                    {t("help:help.contacts.empty.title")}
                  </h3>
                  <p className="text-muted-foreground mb-4">
                    {t("help:help.contacts.empty.description")}
                  </p>
                  <Button onClick={handleAddContact}>
                    <Plus className="mr-2 h-4 w-4" />
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex gap-2 pt-2">
                        ...
                        >
                          ...
                          {t("help:help.contacts.card.edit")}
                        </Button>
                        <Button
                          variant="outline"
                          size="sm"
          ...
          <TabsContent value="documents" className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl font-semibold">
                  {t("help:help.documents.title")}
                </h2>
                <p className="text-muted-foreground">
                  {t("help:help.documents.description")}
                </p>
              </div>
            </div>
            ...
            {isDocumentsLoading ? (
              ...
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <h3 className="text-lg font-semibold mb-2">
                    {t("help:help.documents.empty.title")}
                  </h3>
                  <p className="text-muted-foreground mb-4">
                    {t("help:help.documents.empty.description")}
                  </p>
                  <Badge variant="outline" className="text-xs">
                    <Star className="mr-1 h-3 w-3" />
                    {t("help:help.documents.empty.tip")}
                  </Badge>
                </CardContent>
              </Card>
            ) : (
          ...
          <TabsContent value="instructions" className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-xl font-semibold">
                  {t("help:help.instructions.title")}
                </h2>
                <p className="text-muted-foreground">
                  {t("help:help.instructions.description")}
                </p>
              </div>
            </div>
            ...
            {isInstructionsLoading ? (
              <div className="flex items-center justify-center py-12">
                <div className="text-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto mb-4"></div>
                  <p className="text-muted-foreground">
                    {t("help:help.instructions.loading")}
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      {t("help:help.instructions.funeral.title")}
                    </CardTitle>
                    <CardDescription>
                      {t("help:help.instructions.funeral.description")}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Textarea
                      placeholder={t(
                        "help:help.instructions.funeral.placeholder",
                      )}
                      ...
                      onChange={(e) => {
                        ...
                        setInstructions((prev) =>
                          prev
                            ? { ...prev, funeral_wishes: value }
                            : { id: "", funeral_wishes: value },
                        );
                ...
                <Card>
                  ...
                  <CardContent>
                    <Textarea
                      placeholder={t(
                        "help.instructions.digitalAccounts.placeholder",
                      )}
                      ...
                      onChange={(e) => {
                        ...
                        setInstructions((prev) =>
                          prev
                            ? { ...prev, digital_accounts_shutdown: value }
                            : { id: "", digital_accounts_shutdown: value },
                        );
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      {t("help:help.instructions.messages.title")}
                    </CardTitle>
                    <CardDescription>
                      {t("help:help.instructions.messages.description")}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Textarea
                      placeholder={t(
                        "help:help.instructions.messages.placeholder",
                      )}
                      ...
                      onChange={(e) => {
                        ...
                        setInstructions((prev) =>
                          prev
                            ? { ...prev, messages_to_loved_ones: value }
                            : { id: "", messages_to_loved_ones: value },
                        );
                        debouncedSaveInstructions(
