...
export const Manual: React.FC = () => {
  ...
  const loadContacts = async () => {
    try {
      ...
      const errorMessage = error?.message || t("common:help.errors.unknown");
      ...
      let userMessage = t("common:help.errors.loadContacts");

      // Špecifické správy podľa typu chyby
      if (error?.code === "PGRST116") {
        userMessage = t("common:help.errors.dataNotFound");
      } else if (error?.message?.includes("network")) {
        userMessage = t("common:help.errors.network");
      } else if (error?.message?.includes("permission")) {
        userMessage = t("common:help.errors.noPermission");
      } else if (error?.message?.includes("duplicate")) {
        userMessage = t("common:help.errors.duplicateEntry");
      }
  ...
  const handleSaveContact = async () => {
    if (!formData.name.trim() || !formData.role.trim()) {
      toast.error(t("common:help.contacts.messages.nameAndRoleRequired"));
      return;
    }
    ...
    try {
      ...
      if (!user) {
        toast.error(t("common:help.contacts.messages.loginRequired"));
        return;
      }
      ...
      if (editingContact) {
        ...
        toast.success(t("common:help.contacts.messages.updated"));
      } else {
        const { error } = await supabaseWithRetry
          .from("contacts")
          .insert([contactData]);
        ...
        if (error) throw error;
        toast.success(t("common:help.contacts.messages.added"));
      }
      ...
      // Používateľsky prívetivá správa
      let userMessage = t("common:help.errors.saveContact");
  ...
  const handleDeleteContact = async (id: string) => {
    try {
      ...
      toast.success(t("common:help.contacts.messages.deleted"));
    } catch (error: Record<string, unknown>) {
      const timestamp = new Date().toISOString();
      const errorMessage = error?.message || t("common:help.errors.unknown");
      const errorCode = error?.code || "UNKNOWN_ERROR";
      ...
      // Používateľsky prívetivá správa
      let userMessage = t("common:help.errors.deleteContact");
  ...
  const loadKeyDocuments = async () => {
    try {
      ...
      // Používateľsky prívetivá správa
      let userMessage = t("common:help.errors.loadKeyDocuments");
  ...
  const loadInstructions = async () => {
    try {
      ...
      // Používateľsky prívetivá správa
      let userMessage = t("common:help.errors.loadInstructions");
  ...
  const debouncedSaveInstructions = useCallback(
    async (field: keyof Instructions, value: string) => {
      try {
        ...
        // Používateľsky prívetivá správa
        let userMessage = t("common:help.errors.saveInstructions");
  ...
  const getExpirationStatus = (expirationDate: string | null) => {
    ...
    if (diffDays < 0) {
      return {
        text: t("common:help.documents.expiration.expired"),
        variant: "destructive" as const,
      };
    } else if (diffDays <= 30) {
      return {
        text: t("common:help.documents.expiration.expiresSoon"),
        variant: "destructive" as const,
      };
    } else if (diffDays <= 90) {
      return {
        text: t("common:help.documents.expiration.expiring"),
        ...
        <Tabs defaultValue="contacts" className="space-y-6">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="contacts">
              {t("help.tabs.contacts")}
            </TabsTrigger>
            <TabsTrigger value="documents">
              {t("help.tabs.documents")}
            </TabsTrigger>
            <TabsTrigger value="instructions">
              {t("help.tabs.instructions")}
            </TabsTrigger>
          </TabsList>
              ...
              >
                ...
                <SheetContent>
                  <SheetHeader>
                    <SheetTitle>
                      {editingContact
                        ? t("common:help.contacts.form.editTitle")
                        : t("common:help.contacts.form.addTitle")}
                    </SheetTitle>
                    <SheetDescription>
                    ...
                    <div className="flex gap-3 pt-4">
                      ...
                      >
                        {isSaving ? (
                          ...
                          t("common:help.contacts.form.updateButton")
                        ) : (
                          t("common:help.contacts.form.addButton")
                        )}
                      </Button>
                    </div>
                  </div>
