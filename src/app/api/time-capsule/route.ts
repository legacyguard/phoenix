  ...
  try {
    ...
    if (!authHeader) {
      return NextResponse.json(
        { error: t("common:ui.errors.unauthorized") },
        { status: 401 },
      );
    }
    ...
    if (authError || !user) {
      return NextResponse.json(
        { error: t("common:ui.errors.invalidAuthentication") },
        { status: 401 },
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { error: t("common:ui.errors.missingRequiredFields") },
        { status: 400 },
      );
    }
    ...
    if (capsuleData.messageType === "text") {
      if (!capsuleData.textContent) {
        return NextResponse.json(
          { error: t("common:timeCapsule.errors.textContentRequired") },
          { status: 400 },
        );
      }
    ...
    if (capsuleData.unlockCondition === "date") {
      if (!capsuleData.unlockDate) {
        return NextResponse.json(
          { error: t("common:timeCapsule.errors.unlockDateRequired") },
          { status: 400 },
        );
      }
    ...
    if (
      attachment &&
      ["photo", "video", "audio"].includes(capsuleData.messageType)
    ) {
      ...
      if (uploadError) {
        ...
        return NextResponse.json(
          { error: t("common:timeCapsule.errors.failedToUploadAttachment") },
          { status: 500 },
        );
      }
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: t("common:timeCapsule.errors.failedToCreate") },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      { error: t("common:ui.errors.internalServerError") },
      { status: 500 },
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Database error:", error);
      return NextResponse.json(
        { error: t("common:timeCapsule.errors.failedToFetch") },
        { status: 500 },
      );
    }
