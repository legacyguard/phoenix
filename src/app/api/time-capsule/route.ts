...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: t("ui.errors.missingRequiredFields") },
        { status: 400 },
      );
    }
    ...
    if (capsuleData.messageType === "text") {
      if (!capsuleData.textContent) {
        return NextResponse.json(
          { error: t("timeCapsule.errors.textContentRequired") },
          { status: 400 },
        );
      }
    ...
    if (capsuleData.unlockCondition === "date") {
      if (!capsuleData.unlockDate) {
        return NextResponse.json(
          { error: t("timeCapsule.errors.unlockDateRequired") },
          { status: 400 },
        );
      }
    ...
    if (
      ...
      if (uploadError) {
        ...
        return NextResponse.json(
          { error: t("timeCapsule.errors.failedToUploadAttachment") },
          { status: 500 },
        );
      }
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: t("timeCapsule.errors.failedToCreate") },
        { status: 500 },
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Database error:", error);
      return NextResponse.json(
        { error: t("timeCapsule.errors.failedToFetch") },
        { status: 500 },
      );
    }
