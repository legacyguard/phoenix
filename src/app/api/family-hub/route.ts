...
export async function GET(request: NextRequest) {
  try {
    ...
    const familyMembers: FamilyMember[] = (trustedPeople || []).map(person => ({
      id: person.id,
      name: person.name,
      email: person.email,
    ...
    const overallScore = familyMembers.length > 0
    ...
    const response: FamilyHubResponse = {
      overallScore,
      familyMembers,
      emergencyProtocolEnabled: userProfile?.emergency_protocol_enabled || false
    };
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const validAccessLevels: AccessLevel[] = ['none', 'emergency_only', 'limited_info', 'full_access'];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to update communication timestamp' }, { status: 500 });
    }
...
async function sendUpdateEmail(personId: string, trustedPerson: Pick<TrustedPerson, 'name' | 'email'>) {
  // TODO: Implement actual email sending logic
  console.log(`[Email Update] Would send email to ${trustedPerson.name} (${trustedPerson.email}) - Person ID: ${personId}`);
