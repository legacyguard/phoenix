...
async function isAdmin(supabase: SupabaseClient): Promise<boolean> {
  ...
  const isAdminUser = user.email?.endsWith('@legacyguard.com') || 
...
async function getUserJourneyStats(supabase: SupabaseClient) {
  ...
  const onboardingCompleted = users?.filter(u => u.onboarding_completed).length || 0;
  ...
  return {
    ...
    firstTrustedPersonAdded: Math.round(((withTrustedPeople || 0) / total) * 100),
...
async function getFeatureAdoptionStats(supabase: SupabaseClient) {
  ...
  const total = users?.length || 0;
  const localOnlyMode = users?.filter(u => u.privacy_mode === 'local').length || 0;
  const pushEnabled = users?.filter(u => u.push_notifications_enabled).length || 0;
...
async function getPreparednessStats(supabase: SupabaseClient) {
  ...
  const avgScore = scores?.reduce((sum, s) => sum + (s.preparedness_score || 0), 0) / (scores?.length || 1);
  ...
  return {
    ...
    documentsUploaded: Math.round(((totalDocs || 0) / (totalUsers || 1)) * 10) / 10,
    trustedPeopleAdded: Math.round(((totalTrusted || 0) / (totalUsers || 1)) * 10) / 10,
  };
}
