...
function validateCzechWill(
  ...
  if (!willContent.requiresNotarization) {
    recommendations.push(
      "Czech typed wills require notarization - ensure this is completed",
    );
  }
...
function validateMoldovanWill(
  ...
  recommendations.push(
    "Ensure the will is registered with the notarial chamber for additional security",
  );
}
...
function validateUkrainianWill(
  ...
  recommendations.push(
    "Consider registering your will in the Hereditary Register of Ukraine",
  );
  recommendations.push(
    "Ukrainian law allows for secret wills - consult a notary for this option",
  );
}
...
function validateSerbianWill(
  ...
  // Check required clauses
  if (
    !willContent.testator?.name ||
    !willContent.testator?.birthDate ||
  ...
  recommendations.push(
    "Consider depositing your will with the court for safekeeping",
  );
  recommendations.push(
    "Serbian law recognizes oral wills only in exceptional circumstances",
  );
}
...
function validateAlbanianWill(
  ...
  recommendations.push(
    "Beneficiaries cannot serve as witnesses under Albanian law",
  );
}
...
function validateMacedonianWill(
  ...
  recommendations.push(
    "The will must be read aloud in the presence of witnesses before signing",
  );
}
...
function validateMontenegrinWill(
  ...
  recommendations.push(
    "Consider court deposition for additional legal protection",
  );
...
function validateCommonRequirements(
  ...
  const totalAllocation =
    willContent.beneficiaries?.reduce(
      (sum: number, b: Record<string, unknown>) => {
        const percentageAllocation =
          b.allocation
            ?.filter(
              (a: Record<string, unknown>) => a.assetType === "percentage",
            )
            ?.reduce(
              (s: number, a: Record<string, unknown>) => s + (a.value || 0),
              0,
            ) || 0;
  ...
  if (willContent.specialBequests) {
    willContent.specialBequests.forEach(
      (bequest: Record<string, unknown>, index: number) => {
