...
export async function POST(request: NextRequest) {
  try {
    ...
    const { willContent, requirements, countryCode, documentType } = await request.json();
    ...
    const userIP = request.headers.get('x-forwarded-for') || request.headers.get('remote-addr');

    // Add graphical stamp
    const signerFullName = `${user.user_metadata?.first_name || ''} ${user.user_metadata?.last_name || ''}`.trim() || user.email;
    ...
    if (documentType === 'will') {
      const skribbleResponse = await eSignatureService.requestSkribbleSignature(savedWill.id, base64);

      if (skribbleResponse.error) {
        console.error('Skribble signing error:', skribbleResponse.error);
        return NextResponse.json({
          error: t('will.errors.failed_to_sign'),
