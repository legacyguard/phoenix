    ...
    const settings: PrivacySettings = userProfile
      ? {
          defaultProcessingMode:
            userProfile.privacy_mode || defaultSettings.defaultProcessingMode,
          autoDeleteAfter:
            userProfile.auto_delete_after || defaultSettings.autoDeleteAfter,
          aiFeatureToggles:
            userProfile.ai_feature_toggles || defaultSettings.aiFeatureToggles,
        }
      : defaultSettings;
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (
      ...
      !["hybrid", "local_only"].includes(settings.defaultProcessingMode)
    ) {
      return NextResponse.json(
    ...
    if (
      typeof settings.autoDeleteAfter !== "number" ||
      settings.autoDeleteAfter < 0
    ) {
      return NextResponse.json(
        { error: "Invalid auto delete period" },
        { status: 400 },
      );
    }
