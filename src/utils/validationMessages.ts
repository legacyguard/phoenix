...
export const getValidationMessage = (key: string, params?: Record<string, any>) => {
  ...
  const messages: Record<string, string> = {
    ...
    'name.max': t('validation.errors.nameMaxLength', { max: params?.max || 100 }),
    
    // Required fields
    'field.required': t('validation.errors.fieldRequired', { field: params?.field }),
    ...
    // Selections
    'select.min': t('validation.errors.selectAtLeastOne', { field: params?.field }),
    'select.max': t('validation.errors.selectUpTo', { max: params?.max, field: params?.field }),
    ...
    // Character limits
    'field.maxLength': t('validation.errors.maxCharacters', { field: params?.field, max: params?.max }),
    ...
    // Files
    'file.size': t('validation.errors.fileSizeTooLarge', { maxSize: params?.maxSize }),
    'file.type': t('validation.errors.invalidFileType', { types: params?.types }),
