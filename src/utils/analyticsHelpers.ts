...
function identifyDropOffPoints(
  sessions: Record<string, AnalyticsEvent[]>,
): string[] {
...
function analyzeEmotionalTrend(
  ...
  const recentEvents = events.filter((e) => e.emotional_context).slice(-20); // Last 20 events with emotional context

  if (recentEvents.length < 5) return "stable";

  const emotionScores: Record<
    NonNullable<AnalyticsEvent["emotional_context"]>,
    number
  > = {
    frustrated: -2,
    confused: -1,
  ...
  const firstScore =
    firstHalf.reduce(
      (sum, e) => sum + (emotionScores[e.emotional_context!] || 0),
      0,
    ) / firstHalf.length;
...
export function generateInsights(
  ...
  if (metrics.onboarding_completion_rate < 50) {
    insights.push(
      "Over 50% of users are not completing onboarding. Consider simplifying the process.",
    );
  }

  // Time to value insights
  if (metrics.time_to_first_value > 1440) {
    // 24 hours
    insights.push(
      "Users take over 24 hours to reach first value. Add quick wins in onboarding.",
    );
  }
  ...
  if (metrics.document_completion_percentage < 70) {
    insights.push(
      "Many uploaded documents lack categorization. Improve AI suggestions or UX.",
    );
  }
  ...
  if (metrics.family_preparedness_score < 30) {
    insights.push(
      "Family features have low adoption. Consider highlighting benefits more prominently.",
    );
  }
  ...
  if (patterns.emotionalTrend === "declining") {
    insights.push(
      "User satisfaction is trending downward. Review recent changes and user feedback.",
    );
  }

  if (patterns.dropOffPoints.length > 0) {
    insights.push(
      `High drop-off at: ${patterns.dropOffPoints[0]}. This step may need improvement.`,
    );
  }
