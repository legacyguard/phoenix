...
const ProfessionalDashboard: React.FC<ProfessionalDashboardProps> = ({
  ...
  const getSecurityStatus = (): SecurityStatus => {
    if (completionScore < 25) {
      return {
        ...
        description: t("dashboard-main:dashboard.status.gettingStarted"),
        nextMilestone: t(
  ...
  const getGreeting = () => {
    ...
    if (hour < 12)
      return t("dashboard-main:dashboard.greeting.morning", { name });
    if (hour < 17)
      return t("dashboard-main:dashboard.greeting.afternoon", { name });
    return t("dashboard-main:dashboard.greeting.evening", { name });
  };
      ...
      default:
        return (
          <Badge variant="secondary" className="text-xs">
            {t("dashboard.priority.recommended")}
          </Badge>
        );
    }
  };
