...
export const FamilyPreparednessCalculator: React.FC = () => {
  ...
  useEffect(() => {
    const calculateSavings = () => {
      ...
      const timeWithout = baseTime + accountTime + propertyTime + businessTime + dependentTime;
      const timeWith = 0.083; // 5 minutes in hours
      setTimeSaved(Math.round(timeWithout - timeWith));

      // Estimate money savings
      const legalFees = 2000 + (hasProperty ? 2000 : 0) + (hasBusiness ? 4000 : 0);
      const lostWages = Math.round(income / 160 * timeSaved);
      const penalties = 1000 + (hasProperty ? 2000 : 0) + (hasBusiness ? 2000 : 0);
    ...
    <section className="py-20 bg-muted/30">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex items-center space-x-3 p-4 bg-muted/50 rounded-lg">
                <Checkbox
                  id="property"
                  checked={hasProperty}
                  onCheckedChange={(checked) => setHasProperty(checked as boolean)} />
              ...
              <div className="flex items-center space-x-3 p-4 bg-muted/50 rounded-lg">
                <Checkbox
                  id="business"
                  checked={hasBusiness}
                  onCheckedChange={(checked) => setHasBusiness(checked as boolean)} />
          ...
          {/* Results Card */}
          <Card id="calculator-results" className={`transition-opacity duration-500 ${showResults ? 'opacity-100' : 'opacity-50'}`}>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                <div className="space-y-2 pl-7">
                  ...
                  <div className="pt-2 border-t">
                    <p className="text-primary font-semibold">
                      {t('calculator.results.timeSavings.saved', { hours: timeSaved })}
                    </p>
                  </div>
                </div>
              </div>
