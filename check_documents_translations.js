...
console.log('Checking documents.json translations in all language directories...\n');
  ...
  try {
    ...
    expectedKeys.forEach(keyPath => {
      if (!checkObjectHasKeys(json, keyPath)) {
        ...
        if (found && (!current || typeof current !== 'string' || current.trim().length === 0)) {
...
console.log(`Valid translations: ${results.filter(r => r.status === 'OK').length}`);
