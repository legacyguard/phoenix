...
export class OCRService {
  ...
  private detectDocumentType(text: string): DetectedDocumentType {
    ...
    for (const [type, pattern] of Object.entries(documentPatterns)) {
      ...
      if (matchCount >= pattern.requiredMatches) {
        const maxPossibleWeight = pattern.patterns.reduce(
          (sum, p) => sum + p.weight,
          0,
        );
  ...
  async extractStructuredData(
    ...
    switch (documentType) {
      ...
      case "property_deed":
        ...
        data.propertyAddress = this.extractPattern(
          text,
          /(?:adresa|address):?\s*([^\n]+)/i,
        );
        break;
  ...
  anonymizeText(
    ...
    const { text: anonymized, removedCount } =
      DocumentPreprocessor.anonymizeText(text, options);
  ...
  private createError(
    ...
    const userMessages: Record<OCRError["code"], string> = {
      initialization_failed:
        "Having trouble starting the document reader. Please try again.",
      processing_failed:
        "Could not read your document. Please ensure the image is clear.",
      unsupported_format:
        "This file type is not supported. Please upload an image.",
