...
export function useDocumentUpload(): UseDocumentUploadReturn {
  ...
  const [errors, setErrors] = useState<Array<{ id: string; error: UploadError }>>([]);
  ...
  const overallProgress = uploadQueue.length === 0 ? 0 : Math.round(
  ...
  const processUpload = async (item: UploadQueueItem) => {
    ...
    setUploadQueue(prev => prev.map(i =>
      i.id === item.id ? { ...i, status: 'processing', startedAt: new Date() } : i
    ));
