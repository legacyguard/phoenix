...
export function useOCR(): UseOCRReturn {
  ...
  const processDocument = useCallback(
    async (file: File, options: OCROptions = {}): Promise<OCRResult | null> => {
      // Reset state
      setError(null);
      setResult(null);
  ...
  const anonymizeText = useCallback(
    (text: string, options: AnonymizationOptions = {}): AnonymizedText => {
  ...
  try {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(storeName)) {
        ...
        store.createIndex("documentType", "documentType.type", {
          unique: false,
        });
      }
    };
...
export function useStoredOCRResults() {
  ...
  const loadStoredResults = async () => {
    try {
      ...
      const storedResults = await new Promise<OCRResult[]>(
        (resolve, reject) => {
