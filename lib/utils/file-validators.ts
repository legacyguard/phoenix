import type { ValidationResult, ValidationError, ValidationWarning } from '../services/document-upload.types';
...
export function validateDocument(file: File): ValidationResult {
  ...
  if (file.size > MAX_FILE_SIZE) {
    errors.push({
      ...
      userMessage: 'This file is quite large. Would you like me to compress it for you?',
      recoverable: true,
    });
    ...
    warnings.push({
      ...
      suggestion: 'This might take a moment to process. I can compress it to speed things up.',
    });
  }
  ...
  if (file.type.startsWith('image/') && file.size < MIN_IMAGE_SIZE) {
    warnings.push({
      ...
      suggestion: 'This image might be low quality. Try taking a clearer photo if text is hard to read.',
    });
  }
  ...
  if (!ALL_SUPPORTED_TYPES.includes(file.type)) {
    ...
    if (file.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
      errors.push({
        ...
        userMessage: 'I can\'t read Word documents directly. Could you save it as a PDF first?',
        recoverable: false,
      });
      ...
      errors.push({
        ...
        userMessage: 'I can only process images (JPEG, PNG) and PDFs right now.',
        recoverable: false,
      });
    }
  }
  ...
  if (hasSupiciousFileName(file.name)) {
    errors.push({
      ...
      userMessage: 'This file name looks suspicious. Please check it\'s from a trusted source.',
      recoverable: false,
    });
  }
  ...
  if (file.size === 0) {
    errors.push({
      ...
      userMessage: 'This file seems to be empty or corrupted. Try uploading it again?',
      recoverable: true,
    });
  }
...
function hasSupiciousFileName(fileName: string): boolean {
  ...
  return suspicious.some(ext => lowerName.endsWith(ext)) || 
         lowerName.includes('virus') || 
         lowerName.includes('malware');
}

// Check if file might be a duplicate
export async function checkForDuplicate(file: File, existingFiles: { name: string; checksum: string }[]): Promise<boolean> {
...
export async function validateImageDimensions(file: File): Promise<{ valid: boolean; width?: number; height?: number; error?: string }> {
    ...
    img.onload = () => {
      ...
      if (img.width < MIN_WIDTH || img.height < MIN_HEIGHT) {
        resolve({
          ...
          width: img.width,
          height: img.height,
          error: 'Image is too small for reliable text extraction. Try a higher resolution photo.',
        });
      } else {
        resolve({
          valid: true,
