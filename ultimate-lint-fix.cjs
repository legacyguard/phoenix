  ...
  filesToFix.forEach((relativePath) => {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        console.error(
          `❌ Error fixing encoding in ${relativePath}:`,
          error.message,
        );
      }
    }
...
function main() {
  console.log(
    "🚀 Starting ultimate lint fix to resolve all GitHub Actions issues...\n",
  );
  ...
  try {
    ...
    execSync(
      'npm run lint -- --max-warnings=200 --ignore-pattern "backup-before-*/**"',
      { stdio: "inherit" },
    );
    ...
    try {
      const result = execSync(
        'npm run lint 2>&1 | grep -E "error|warning" | grep -c "error" || echo "0"',
        { encoding: "utf8" },
      );
      ...
      if (errorCount < 10) {
        console.log(
          "🎯 Error count is now very low - GitHub Actions should pass!",
        );
      } else {
        console.log(
          "⚠️  Some errors remain, but significantly reduced from original 103 errors",
        );
      }
    } catch (e) {
