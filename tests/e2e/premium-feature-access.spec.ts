...
test.describe('Premium Feature Access - Minimum User State Coverage', () => {
  test('Logged out user is redirected to login when accessing premium feature', async ({ page }) => {
    await page.goto('/executor-toolkit');
    await expect(page).toHaveURL(/auth|login/i);
    await expect(page.locator('text=Sign In')).toBeVisible();
  });

  test('Free user attempting to access premium feature should see upgrade', async ({ page }) => {
  ...
  test('Premium feature badge is visible to free users but not to premium', async ({ page }) => {
    await signIn(page, testUsers.freeUser);
    await page.goto('/dashboard');
    const freeBadges = await page.locator('.premium-badge, [data-premium="true"]').count();
    expect(freeBadges).toBeGreaterThan(0);
    await signOut(page);

    await signIn(page, testUsers.premiumUser);
    await page.goto('/dashboard');
    const premiumBadges = await page.locator('.premium-badge, [data-premium="true"]').count();
