...
test.describe("Core Vault Setup Journey", () => {
  test("New user completes core vault setup", async ({ page }) => {
    ...
    const initialStrength = await page
      .locator('[data-testid="plan-strength-value"]')
      .textContent();
    ...
    await expect(
      page.locator("text=Trusted person added successfully"),
    ).toBeVisible();

    // Verify plan strength increased
    await page.waitForTimeout(1000); // Wait for animation
    const strengthAfterTrusted = await page
      .locator('[data-testid="plan-strength-value"]')
      .textContent();
    ...
    await page.fill(
      ...
      "Main household checking account at Local Bank",
    );
    ...
    const strengthAfterAsset = await page
      .locator('[data-testid="plan-strength-value"]')
      .textContent();
    ...
    await page.fill(
      'textarea[name="notes"]',
      "Updated will signed on January 1, 2024",
    );
    ...
    await page.click('a[href="/dashboard"]');
    const finalStrength = await page
      .locator('[data-testid="plan-strength-value"]')
      .textContent();
    ...
    await expect(
      page.locator(
        '[data-testid="step-trusted-person"][data-completed="true"]',
      ),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="step-assets"][data-completed="true"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="step-documents"][data-completed="true"]'),
    ).toBeVisible();
  });
