...
const locales = fs
  .readdirSync(LOC)
  .filter((d) => fs.statSync(path.join(LOC, d)).isDirectory());
const missingByLocale = readJSON(path.join(ADIR, "missing_by_locale.json"));
const unusedInBase =
  readJSON(path.join(ADIR, "unused_in_base.json")).unused || [];
let placeholderMismatch = [];
try {
  placeholderMismatch = readJSON(path.join(ADIR, "placeholder_mismatch.json"));
} catch {}
